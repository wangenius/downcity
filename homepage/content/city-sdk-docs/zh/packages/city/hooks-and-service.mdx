...
所以你看到的 usage、账号、支付，并不是“额外开一套系统”，而是作为扩展点进入同一套 Federation。
...
- `new AccountsService()` 给 City 增加 `accounts` service
- `AccountsService` 会自动挂载注册、登录、OAuth start/result、better-auth passthrough 与 OAuth callback 入口，不需要在 worker 里手动调用 `router()`
- `new PaymentService()` 给 City 增加统一支付入口，用来暴露当前可用的支付方式
- `new UsageService()` 给 City 增加 usage 记录能力和相关查询入口
- `stripePaymentProvider()` 给统一 Payment 服务增加 Stripe Checkout、webhook 和支付结果同步
...
service 负责把能力挂进 City，真正的产品调用入口仍然是 [@downcity/city](/zh/city-sdk-docs/packages/city/federation)。

## 继续阅读

- 想看 city 端怎么统一调用 service，读 [@downcity/city](/zh/city-sdk-docs/packages/city/federation)
