...
- a project-level assistant that holds sessions, reads project config, calls tools, and runs plugins
...
- binding the project to its selected Federation AIService model
- creating and maintaining sessions
- exposing project-level HTTP and CLI entry points
- starting enabled project plugins
...
- it does not own the Federation AIService model catalog
- it does not persist City-level channel accounts
- it is not the Console UI or the global control plane

Those concerns are split between Downcity and City: Downcity manages local Agents and Agent plugins, while City manages models, channel accounts, and Service capabilities.
...
- chat with a project through Downcity
...
Downcity provides the user-facing local runtime path. This path is best when your goal is "turn this repository into an agent runtime and use it directly."
...
- you already have a Node.js app or application runtime
- you want agent capability inside an existing workflow
- you want to control tools, plugins, and session lifecycle programmatically
...
3. [Agent CLI product docs](/en/docs/reference/cli)
4. [Agent SDK overview](/en/agent-sdk-docs)
