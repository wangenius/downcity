...
description: Call City from the studio with UserClient.
---

Studio clients use `UserClient`. The common paths are `client.ai.*` for AIService and `client.service(id).action(name).invoke()` for custom services.
...
const result = await client.ai.text({
  ...
  prompt: "Write a welcome message for the studio",
});
```
