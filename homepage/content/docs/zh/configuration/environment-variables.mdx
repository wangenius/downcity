...
3. 当宿主是 `bay` 时，会先从 `downcity.db` 读取 Console 全局 env，再显式传给 `Agent`。
...
当你使用 `bay start -p` 或 `bay console start -p` 时，CLI 会尝试在启动结果里打印 `Public URL`。
如果你使用 `bay public on`，后续普通 `bay start` 和 `bay console start` 也会沿用这份已保存的公网绑定模式。

这些值通常由 `bay start` 或部署环境注入，不是 `contact link` 要求用户手动填写的参数：

1. `DOWNCITY_PUBLIC_URL`：完整公网 URL，例如 `https://console.example.com`
2. `DOWNCITY_PUBLIC_HOST`：公网 host，CLI 会自动补成 `http://<host>:5315`

`bay start` 会自动探测公网 IP，并把结果写入全局 Console Env 的 `DOWNCITY_PUBLIC_HOST`。这个值也会被 agent 的 `contact link` 用来生成可转交的联系码。
