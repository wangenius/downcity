...
import { resolve_loop_selectable_index, resolve_next_loop_selectable_index, } from "../tui/SelectableList.js";
...
export async function run_agent_chat_tui(params) {
    ...
    const build_title = () => `Agent chat · ${params.agent_id} · ${current_session_id}`;
    const history_lines = [
        `system> ${build_title()}`,
    ...
    await new Promise((resolve) => {
        ...
        const reset_session_view = (session_id) => {
            current_session_id = session_id;
            history_lines.splice(0, history_lines.length, `system> ${build_title()}`, "system> Type /help for shortcuts, /session to switch, /new to create, /quit to exit.");
            refresh_view();
        };
        const finish = () => {
            if (closed)
...
async function open_session_picker(params) {
    ...
    return await new Promise((resolve) => {
        ...
        list.key(["up", "k"], () => {
            selected_index = resolve_next_loop_selectable_index(choices, selected_index, -1);
        ...
        list.key(["enter", "return"], () => {
            selected_index = resolve_loop_selectable_index(choices, list.selected, selected_index);
            finish(choices[selected_index] || null);
        });
        list.on("select", (_item, index) => {
            selected_index = resolve_loop_selectable_index(choices, index, selected_index);
            finish(choices[selected_index] || null);
        });
        list.select(selected_index);
        list.focus();
...
function format_session_choice_label(session) {
