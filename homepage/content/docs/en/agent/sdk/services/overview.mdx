...
- `agent.http.start()` and `agent.rpc.start()` call `ensureServicesStarted()` first
...
The current local SDK session prompt path does not automatically inject `service.system()`.

The default SDK path currently injects:
...
So in the SDK today, the primary value of services is:

- lifecycle
- long-lived state
- channel and runtime capability ownership

not "write a service and it automatically changes every model prompt."

## Recommended reading

1. [Which built-in services exist today](/en/docs/agent/sdk/services/built-in-services)
2. [ChatService](/en/docs/agent/sdk/services/chat-service)
3. [Custom service](/en/docs/agent/sdk/services/custom-service)
4. [Service lifecycle](/en/docs/agent/sdk/services/service-lifecycle)
