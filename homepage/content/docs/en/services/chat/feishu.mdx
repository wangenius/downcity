...
The same config is now stored in console `~/.ship/ship.db` and managed in Console UI under `Agent / Authorization`, instead of being written to project `ship.json` in plaintext. New users fall back to default roles, so no pairing flow is required; group/channel usage depends only on the speaking user's role.
...
- text / links / mentions inside `post` are normalized into plain text, while `post` images / media are downloaded into `.ship/.cache/feishu/` and injected as `<file ...>`
- inbound image, video, file, and audio messages are downloaded into `.ship/.cache/feishu/` and injected as `<file ...>`
...
- outbound replies support `<file ...>` conversion; multiline text, links, and images are merged into Feishu `post` when possible
