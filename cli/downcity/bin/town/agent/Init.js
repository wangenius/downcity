/**
 * `town agent create`：在目标目录生成最小可用的 Downcity 工程骨架与配置文件。
 *
 * 目标
...
export async function initCommand(cwd = ".", options = {}) {
    ...
    const default_agent_id = normalizeDefaultAgentId(projectBaseName) || projectBaseName;
    ...
    if (existingShipJson) {
        if (!allowOverwrite) {
            const confirmResponse = (await prompts({
                type: "confirm",
                name: "overwrite",
                message: "downcity.json already exists. Overwrite existing downcity.json and continue?",
                initial: false,
            }));
