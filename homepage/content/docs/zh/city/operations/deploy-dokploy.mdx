...
`/data` 必须是持久化 volume。bay、model、用户、session、usage、payment 和 City env 都会保存在这个 SQLite 数据库里。
...
可信后端应该用 `Admin Visa` 携带 `DOWNCITY_CITY_ADMIN_SECRET_KEY`：
...
完整 SDK 能力继续看 [User Visa](/docs/city/reference/visa-user) 和 [Admin Visa](/docs/city/reference/visa-admin)。
