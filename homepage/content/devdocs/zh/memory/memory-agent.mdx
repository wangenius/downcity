...
description: 解释 memoryAgent 在 Downcity 里的真实定位、职责、触发方式、工作节奏和与 SessionAgent 的关系
---
...
| 角色 | 主要目标 | 时间尺度 |
| --- | --- | --- |
| `SessionAgent` | 把当前这一步做好 | 秒级 |
| `memoryAgent` | 把过去整理成长期状态 | 分钟、小时、天 |

一句话：

```text
SessionAgent 负责现在，memoryAgent 负责时间。
```
...
```mermaid
flowchart LR
  DAY["白天<br/>SessionAgent 持续执行"] --> CAP["memory service 轻量 capture"]
  CAP --> NIGHT["夜里 / 空闲时"]
