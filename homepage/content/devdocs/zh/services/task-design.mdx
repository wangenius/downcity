...
`chat` 更多是通过现成的 `AgentContext.session.prompt()` 进入宿主执行链。
`task` 在 run 阶段则会额外创建一个 task 专用 agent runtime：

- 单独的 task session runtime
- 单独的 `SessionHistoryStore`
- 单独的 run 目录
- 独立的 task profile system

这意味着：

- `task` 不只是“调用一下现成 session.prompt”
