...
| 包 / 目录 | 作用 |
| --- | --- |
| `downcity` | 官方 CLI 聚合包，安装后提供 `town` 本机 Agent 宿主命令与 `city` 可部署 City 管理命令。 |
| `@downcity/agent` | 单 Agent runtime 与 SDK，负责 session、tool loop、service、plugin、HTTP/RPC、sandbox 与宿主接入。 |
| `@downcity/city` | City 运行时与 City 访问 SDK，负责 service 注册、action、auth、env、town 访问边界与 HTTP 调用。 |
...
- Town 控制面：通过 `town start` 或 `town console` 托管本机 Agent，并进入浏览器控制面。
- Agent 生命周期管理：创建、启动、停止、重启、诊断、对话、查看历史。
- 全局模型池：通过 `town model` 管理 provider 与 model，并把项目绑定到模型 ID。
- City runtime：通过 `@downcity/city`、`@downcity/services` 和可部署的 `cities/*`，让多个 town 复用同一座服务城市。
...
`town` 负责本机 Agent 宿主，`city` 负责可部署 City 管理。使用 `town update` 更新全局 CLI。
...
`town` 会把 `downcity.json.execution.modelId` 解析成运行时 `LanguageModel`，再注入到 Agent session 中。
