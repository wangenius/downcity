---
title: User Gate
description: How studio clients read the model directory and call AIService or custom services.
---

`User Gate` is the runtime client for studio-facing calls.
...
The most important point is not the method list. `User Gate` unifies three kinds of studio-side calls:
...
So from the studio side, `User Gate` is not only "the thing that calls models." It is the unified user-context entry into City.
...
After receiving `session.user_token`, create a User Gate with `studio_id + user_token` for AI service calls.
...
Pass `city_url + studio_id + user_token`. This is for:
