...
```ts
const session = await remoteAgent.sessions.create();
const turn = await session.prompt({ query: "继续" });
await turn.finished;
```
...
- 用 `prompt()` 追加新的用户输入
- 用 `stop()` 停止当前 turn，并取消尚未被吸收的排队 prompt
- 用 `subscribe()` 观察统一实时 Mutation
- 用 `messages()` 读取初始快照；重新连接后重新读取快照并订阅
...
- [stop](/zh/agent-sdk-docs/sessions/stop)
