...
const RESERVED_PORTS = new Set([5315]);
 ...
 */
async function canListenPort(port, host) {
 ...
 * 在给定范围内分配一个可用端口。
 */
export async function allocateAvailablePort(params = {}) {
    ...
    for (let port = start; port <= end; port += 1) {
        ...
        if (await canListenPort(port, host))
            return port;
    }
