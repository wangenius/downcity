...
当执行 `sma agent start`（后台 daemon）时：
...
`sma console agents` / `sma console status` 在展示时会再次读每个项目的 daemon pid 并判活；判定为 stale 的记录会被清理。
...
因此 shell 中再执行 `sma ...` 命令时，默认会优先命中当前 runtime 对应 server。

## 命令桥接

shell 对 `sma|shipmyagent` 命令支持桥接协议：
...
```mermaid
flowchart TD
  A[sma agent start] --> B[startDaemonProcess]
  B --> C[write ship debug files]
  B --> D[upsert console registry]
  D --> E[console registry]

  F[sma service] --> G[api services endpoints]
