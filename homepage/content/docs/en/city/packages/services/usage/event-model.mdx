...
## How studio-side users read their own usage
...
- self-serve usage awareness inside the studio

## Common scenarios

### Scenario 1: track first, price later

First persist the facts, then later decide which models are billable, which calls count toward quotas, and which studios or flows are exempt.

### Scenario 2: debugging and operations

If you need to know whether calls really happened, whether one studio is consuming abnormally, or whether one model is being hit far more than expected, the usage fact layer is often the first place to look.
...
- For read surfaces, read [Querying and Summary](/en/docs/city/packages/services/usage/querying-and-summary)
- For how usage combines with billing logic, read [Billing Integration](/en/docs/city/packages/services/usage/billing-integration)
