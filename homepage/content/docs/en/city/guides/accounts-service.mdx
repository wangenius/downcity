...
```txt
bay frontend
  -> guest User Gate with city_url only
...
If you already have your own backend, you can skip the service for login itself and let your backend validate users first, then use `Admin Gate` to issue `user_token`. The accounts service is the official ready-made service path for a minimal auth system.
...
Both patterns converge to the same end state: the frontend gets `bay_id + user_token`, then uses a normal `User Gate` to call City.
...
The response includes `user_token`, `user_id`, and `email`. The bay client can then create a normal `User Gate` and call AI services.
...
## Switching from guest User Gate to normal User Gate
...
- guest Actions still run through `User Gate`
- but at that stage the `User Gate` only needs `city_url`
...
So the service provides the auth capability, while the SDK still provides the calling surface through `User Gate` and `Admin Gate`.
