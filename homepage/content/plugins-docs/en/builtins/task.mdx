...
`TaskPlugin` is exported from `@downcity/plugins`. It can be attached directly in SDK usage, although the most common path is still through the agent runtime and `town task`.
