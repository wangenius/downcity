...
图片任务的后台 `image/fetch` 会先原子领取任务，同一时间只有一个 worker 调用 Provider。Provider 返回成功后，AIService 先用稳定的任务幂等键完成扣费，再写入 `status: "succeeded"` 和 `result_json`；扣费失败时任务保持可重试，重复执行不会重复扣款。`image_create()` 不扣费；用户 `image_result()` 只读 `async_jobs` 缓存。如果 Queue 消费没有当前用户上下文，`bill()` 应该从已保存任务归属里返回 charge line 的 `user_id`。
