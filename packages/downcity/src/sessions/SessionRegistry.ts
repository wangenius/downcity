/**
 * SessionRegistry：会话生命周期编排器（main 层）。
 *
 * 关键职责（中文）
 * - 负责 request scope 绑定与统一运行入口。
 * - 负责用户消息与助手消息入库。
 * - 通过 `SessionRuntimeRegistry` 访问 SessionRuntime/Persistor。
 */
...
export class SessionRegistry {
   ...
   *
   * 关键点（中文）
   * - 收敛 `runtimeRegistry.getRuntime + withRequestContext + sessionRuntime.run`。
  ...
  async run(params: {
    ...
    if (!sessionId) {
      throw new Error("SessionRegistry.run requires a non-empty sessionId");
    }
    ...
    const wrappedOnAssistantStepCallback = async (step: {
      ...
      await this.appendAssistantMessage({
        ...
        extra: {
          ...
          persistedBy: "session_registry_run",
        },
      });
