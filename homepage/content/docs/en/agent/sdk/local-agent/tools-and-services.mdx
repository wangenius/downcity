...
The local SDK currently wires the plugin registry and `AgentContext.plugins`.

It does not automatically inject `plugin.system()` into session prompts, and it does not automatically mount `plugin.http.runtime` onto the SDK HTTP server started by `agent.http.start()`.
