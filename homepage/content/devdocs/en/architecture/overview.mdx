...
- `cli/town/`: local Agent host, Console gateway, model pool, project orchestration, and daemon control
- `packages/city/`: City runtime plus City access SDK for services, actions, auth, env, town-scoped access, routing, and HTTP calls
...
- `packages/city` owns service access over HTTP through City exports
...
packages/city build -> runtime and City SDK package
packages/services build -> public services package
cli/city build -> City management command
cli/town build -> local Town host command
