...
`User Gate` is the runtime client for bay-facing calls.
...
The most important point is not the method list. `User Gate` unifies three kinds of bay-side calls:
...
So from the bay side, `User Gate` is not only "the thing that calls models." It is the unified user-context entry into City.
...
After receiving `session.user_token`, create a User Gate with `bay_id + user_token` for AI service calls.

## Guest calls vs logged-in user calls

It helps to think about `User Gate` in two phases:
...
From the perspective of `User Gate`, both are called the same way.
...
When `User Gate` receives a non-2xx HTTP response, it throws an `Error` with two extra fields:
...
## When not to use `User Gate`

Do not use `User Gate` for:
...
Those are trusted-side actions and belong in [Admin Gate](/en/docs/city/reference/gate-admin) or your own backend.
