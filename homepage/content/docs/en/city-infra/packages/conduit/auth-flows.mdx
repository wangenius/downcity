...
Every login flow around `@downcity/conduit` converges to one goal:

> get `product_id + user_token` into the product frontend, then enter normal `UserClient` calls.
...
At that point the product enters real user context:
