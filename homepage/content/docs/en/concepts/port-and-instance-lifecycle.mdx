...
description: How console, agent, runtime plugins, and plugins coordinate on port allocation, routing, and runtime lifecycle
---
...
- runtime-plugin and plugin commands resolve target agent first, then endpoint.
...
- Runtime HTTP APIs include `/api/plugins/runtime/*` and `/api/plugins/*`
- Debug metadata is persisted to `.downcity/.debug`

### runtime plugin / plugin

- `city service ...` calls runtime plugin routes
