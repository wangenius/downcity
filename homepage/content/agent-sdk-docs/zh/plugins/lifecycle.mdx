...
ActionSchedule 不是 plugin；它是 Agent 构造时自动启动的 Agent 级 action 调度 loop（与 plugin lifecycle 同时起来）。

## runtime state 不等于 enablement

下面这些是不同事实：

- `enabled`：当前项目配置是否允许插件开启
...
本地 SDK 中，plugin lifecycle 在 `new Agent(...)` 构造时自动启动；调用方可以 `await agent.ready()` 等待启动完成，`await agent.dispose()` 停止。

更底层的 runtime 代码可以直接使用这些导出 helper：
