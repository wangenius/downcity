---
title: 上下文工程
description: “Repo is the Agent” 在 ShipMyAgent 里的工程落地方式（PROFILE.md / .ship / skills / 按需历史）
---
...
1) **项目静态上下文（Repo-level）**
   - `PROFILE.md`：项目“宪法/角色/边界/行为规范”（系统提示词来源之一）
...
### 1) system prompt：PROFILE.md（可选 SOUL.md / USER.md）+ 内置提示词 + Skills 概览

每次执行都会构建一份 system prompt（示意）：

- 读取项目 `PROFILE.md`（必需）与 `SOUL.md` / `USER.md`（可选，存在则注入）
- 拼上内置默认提示词（行为约束 / 输出规则）
