 ...
 * 渲染侧边栏选项列表。
 */
export function render_nav(shell, title, options, selected_index) {
 ...
 * 渲染当前选项详情到内容区。
 */
export function render_option_detail(shell, title, option) {
 ...
 * 渲染 loading 状态。
 */
export function render_loading(shell, title) {
 ...
 * 在内容区展示一段文本。
 */
export async function show_content(shell, input) {
 ...
 * 渲染底部提示条。
 */
export function render_sidebar_hint(shell, options, selected) {
...
export function format_sidebar_option(option) {
    if (is_disabled_option(option)) {
        return `── ${option.label} ──`;
    }
    return option.label;
}
export function format_option_detail(title, option) {
    if (!option) {
        return t({
            zh: "未选择项目",
            en: "No item selected",
...
export function option_description(option) {
...
export function is_disabled_option(option) {
    return option?.disabled === true;
}
export function format_table(rows) {
    if (rows.length === 0) {
        return "";
    }
...
export function message_title(kind) {
    if (kind === "success")
 ...
 * 输入态 footer 文案。
 */
export function text_footer_text(secret) {
    return secret
        ? t({
...
export function message_accent(kind) {
    if (kind === "success")
        return "green";
    if (kind === "error")
        return "red";
...
//# sourceMappingURL=AdminTuiRender.js.map