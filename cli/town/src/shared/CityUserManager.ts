...
import {
  ...
  readTownCitySessionForBase,
} from "./CityStateStore.js";
import type {
  ResolvedTownCityUser,
  ResolveTownCityUserInput,
  TownCityAccountsMeResult,
  TownCityUserEnvOverrides,
} from "../types/TownCityUser.js";
...
export class CityUserManager {
   ...
   * 解析当前有效 City user。
   */
  async resolveCurrentUser(input: ResolveTownCityUserInput = {}): Promise<ResolvedTownCityUser> {
    ...
    const env_overrides: TownCityUserEnvOverrides = {
    ...
    const resolved: ResolvedTownCityUser = {
      city_url,
      town_id,
      user_token,
   ...
   */
  async createUserClient(input: ResolveTownCityUserInput = {}): Promise<{
    /**
     * 当前有效身份。
     */
   ...
   */
  readAdminSecret(city_url: string, env: NodeJS.ProcessEnv | Record<string, string | undefined> = process.env): string | undefined {
