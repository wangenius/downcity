...
/**
 * 运行 `bay model create` 交互流程。
 *
 * 关键点（中文）
 * - 既供 `bay model create` 直接调用，也供裸 `bay model` manager 复用。
 */
...
async function runInteractiveModelCreate(
  ...
  if (providers.length === 0) {
    throw new Error(
      "No provider found. Please run `bay model create` and choose provider first.",
    );
  }
