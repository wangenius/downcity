...
`agent.create_session()` and `agent.get_session()` return `AgentSession`.

If the host needs lower-level runtime assembly, it can also use `agent.getRuntime()`, `agent.getContext()`, and `agent.get_sessionCollection()`.
