...
| `/v1/ai/language-model/stream` | `POST` | CityModel 原生 LanguageModelV3 通路 |
...
### `POST /v1/ai/language-model/stream`

这是 `CityModel` 内部使用的版本化 LanguageModelV3 transport。它传输标准模型
prompt、function tools、reasoning、usage 和 finish reason。产品代码应通过模型目录
返回的 `CityModel` 使用该通路，不应手写 transport 请求。
