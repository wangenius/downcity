...
import { updateStudioCLI } from "./core/update.js";
...
export async function runTerminalApp(argv: string[] = []): Promise<void> {
  ...
  intro(`Downcity Studio v${readStudioVersion()} (Esc to go back, Ctrl+C to exit)`);
...
async function runSelfUpdate(): Promise<void> {
  try {
    show("Updating studio CLI...");
    const result = await updateStudioCLI();
    showSuccess(`studio updated via ${result.mode} mode -> v${result.version}`);
    show("Please run `studio` again to use the updated CLI.");
  } catch (error) {
