/**
 * AgentHistory：`studio agent history` 维护命令。
 *
 * 关键点（中文）
...
export async function agentHistoryCleanCommand(
  ...
  if (!sessionId) {
    throw new CliError({
      ...
      fix: "Example: studio agent history clean <path> --channel telegram --chat-id 8444574557 --hard",
    });
  }
