...
| 路由 | 方法 | 用途 |
| --- | --- | --- |
| `/v1/bays/list` | `GET` | Bay 列表 |
| `/v1/bays/create` | `POST` | 创建 Bay |
| `/v1/bays/pause` | `POST` | 暂停 Bay |
| `/v1/bays/activate` | `POST` | 启用 Bay |
| `/v1/bays/remove` | `POST` | 删除 Bay |
| `/v1/bays/tokens/apply` | `POST` | 签发 user_token |
...
```json
{ "bay_id": "bay_xxx", "name": "Chrome Extension", "status": "active", ... }
```

### `POST /v1/bays/tokens/apply`
...
```json
{ "user_token": "ub_xxx", "bay_id": "bay_xxx", "user_id": "user_123", ... }
```
