...
- Keep clear boundaries:
  ...
  - Global Channel Accounts (`ship.db`): all bot information (token/appId/appSecret/name/owner/creator)
...
## In multi-agent setups: what is shared and what is isolated

This boundary is important:

- Bot credentials behind `channelAccountId` are shared globally in `ship.db`
- `chat` authorization rules are isolated per agent, even when multiple agents reuse the same channel account

That means multiple agents can bind the same Telegram / Feishu / QQ bot account while still having different:
...
- Chat Authorization decides who is allowed to use this specific agent
...
2. `agent-A` assigns one user to `admin`
3. `agent-B` does not assign that role

Result:

- the user is allowed in `agent-A`
- the same user can still be blocked in `agent-B`

So reusing bot credentials means reusing the platform identity, not reusing authorization policy.
