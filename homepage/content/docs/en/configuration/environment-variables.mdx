...
So key precedence inside `shell` sessions is: `DC_CTX_*` > agent runtime env > global env > host process env.
...
3. `shell` subprocess injection (global env + current agent runtime env + `DC_CTX_*`).
...
2. Plugin config from project `downcity.json` (`plugins.*`, `assets.*`).
...
4. Plugin config (`city plugin action ...` or `city voice ...`): writes project `downcity.json`.
