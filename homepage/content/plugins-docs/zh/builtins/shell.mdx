...
只读目录必须是已存在的绝对目录，不能位于项目目录内，也不能允许 group/world 写入。macOS 将其编译为 Seatbelt `file-read*` 规则，Linux 将其映射为 Bubblewrap `--ro-bind`；两端都不会增加写权限。
...
权限收缩时，Shell 会关闭仍持有旧权限的活动 session。macOS 还会自动把当前 `xcode-select` 对应的 Xcode 运行目录加入系统只读路径，并显式设置 `DEVELOPER_DIR`，因此 `/usr/bin/git` 不需要额外配置或项目外写权限。
