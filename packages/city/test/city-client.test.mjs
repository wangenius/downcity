...
test("City user().profile() directly requests Federation Accounts", async () => {
  const requests = []
  const profile = {
    user_id: "user_1",
  ...
  assert.deepEqual(await city.user().profile(), profile)
  assert.equal(requests[0].url, "https://fed.example.com/v1/accounts/me")
