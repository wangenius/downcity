...
Each entry must be an existing absolute directory outside the project and must not be group/world writable. macOS compiles it to a Seatbelt `file-read*` rule, while Linux maps it with Bubblewrap `--ro-bind`. Neither backend grants write access.
...
When access is removed, Shell closes active sessions that could retain the previous permission. On macOS, the Xcode runtime selected by `xcode-select` is added automatically as a system read-only path and `DEVELOPER_DIR` is set explicitly, so `/usr/bin/git` needs neither host configuration nor project-external write access.
