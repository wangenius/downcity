 ...
 *
 * 关键点（中文）
 * - `bay` 只负责本机 Agent 宿主能力，不再混入 City 管理入口。
...
/**
 * 解析当前 Bay 安装所绑定的 agent runtime 版本号。
 *
 * 关键点（中文）
 * - 这里读取的是 Bay 当前安装依赖中的 `@downcity/agent` 版本，
 *   不是 workspace 源码目录里的 package.json。
 * - 这样 `bay agent -v` 才能反映“这份 Bay CLI 实际会驱动哪个 agent runtime”。
 */
function resolveInstalledAgentVersion(): string {
  const candidate_package_paths = [
    // 关键点（中文）：Bay 独立包与 downcity 聚合包都会把依赖放在 package root/node_modules。
...
/**
 * 处理 `bay agent -v/--version`。
 *
 * 关键点（中文）
 * - commander 根命令会优先消费全局 `-v`，导致 `bay agent -v` 默认只显示 Bay 版本。
...
if (
  ...
  console.log(`bay ${packageJson.version} (agent ${installedAgentVersion})`);
  process.exit(0);
}
