/**
 * OrchestratorComponent：单次 Session run 编排组件抽象。
 *
 * 关键点（中文）
 ...
 * Orchestrator 组件抽象类。
 */
export abstract class OrchestratorComponent extends SessionComponent {
  /**
   * 组件名（由具体实现声明）。
   */
   ...
   *
   * 关键点（中文）
   * - fallback 消息构造由 orchestrator 内部实现，SessionCore 不直接依赖 persistor。
   */
