...
当前主模型实例在 `packages/downcity/src/agent/context/manager/RuntimeState.ts` 启动阶段创建，并通过 `ServiceRuntime.session.model` 暴露给系统使用。
...
### 2. 主执行体是 `SessionAgent`

`packages/downcity/src/agent/context/context-agent/SessionAgent.ts` 里的 `SessionAgent`，本质上是当前 `contextId` 下的 Agent 装配器。

说明：

- 代码命名和文档语义现在已经统一为 `SessionAgent`
...
- 怎么查
- 怎么拿
- 这次该给 `SessionAgent` 什么 memory pack
...
当前 package 里，`ServiceRuntime.session.afterSessionUpdatedAsync` 与底层 `SessionManager.afterContextUpdatedAsync` 已经为 Memory 这样的异步后处理留出了入口。
...
- `SessionAgent` 和后台维护都通过这个宿主来读写它

## 一句话总结

```text
Memory Service 是 Downcity 里负责长期状态的宿主：在线时为 SessionAgent 提供记忆，离线时把零散经验慢慢整理成稳定状态。
```
