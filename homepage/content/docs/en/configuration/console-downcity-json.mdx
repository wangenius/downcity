...
1. Host process env: `process.env`
2. Agent-local env overlay: `<agent>/.env` (user-managed)
3. SDK explicit override: `new Agent({ env })`
...
1. Agent runtime env map is `process.env + <agent>/.env + optional new Agent({ env })` (`env` wins).
2. When the host is `city`, Console-global env can be injected into `process.env` before Agent startup.
