...
`AgentRuntime` 持有当前 agent 的长期运行状态：
...
- 创建 model
- 创建 `SessionExecutorRegistry` 与 `SessionFacade`
- 创建 plugin registry
...
- 从 `AgentRuntime` 派生统一执行上下文
- 为 service 和 plugin 提供统一能力面

当前 service / plugin 拿到的是统一的：
...
- 按 `sessionId` 缓存 `LocalSessionExecutor`
- 管理 runtime 与 history store 的创建关系

#### `LocalSessionExecutor`

负责：

- 把 `model / history store / compactor / prompter / tools` 装配成一个可运行 session
