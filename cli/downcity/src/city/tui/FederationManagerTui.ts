...
function read_federation_membership_state(): FederationMembershipState {
...
async function prompt_federation(): Promise<FederationProfile | null> {
...
function select_federation(server: FederationProfile): void {
...
function format_membership_detail(membership: FederationMembershipState): string {
  return t({
    zh: [
...
function format_federation_list_detail(servers: FederationProfile[]): string {
  return [
...
function format_login_detail(membership: FederationMembershipState): string {
  return t({
    zh: [
      "{bold}登录{/bold}",
