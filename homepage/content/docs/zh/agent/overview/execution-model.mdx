...
- 当前项目使用 city 全局模型池中的某个模型
- 项目本身只保存 `modelId`
- 模型的 provider、密钥和模型名由全局模型池管理
...
- SDK 本地 session 的模型由 `session.set({ model })` 显式传入

这也是本地 SDK 最常见的踩坑点之一：你以为 SDK 会自动从项目配置里帮你把运行中的模型实例准备好，但当前实现不是这样，SDK session 仍然需要显式 `set({ model })`。

## 继续阅读

- [模型配置](/zh/docs/agent/configuration/models/platform-model-pool)
- [SDK 总览](/zh/docs/agent/sdk/overview)
