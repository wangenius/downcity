...
### 2.3 群聊触发与权限（trigger + groupAccess）

在群聊/超级群中，**非空消息默认都可进入执行判定**。

- **显式触发**（`@bot` / 回复 bot）与 **follow-up window** 仍会被记录，用于增强连续对话体验
- **空的显式触发**不会直接执行，但会激活 follow-up window

此外还有一层 **可对话人限制（groupAccess）**：

- 默认：`"anyone"`，允许任何群成员对话
- 可配置为 `"initiator_or_admin"`：仅发起人或群管理员可继续对话

当 `groupAccess="initiator_or_admin"` 时，发起人信息会写入 `.ship/.cache/telegram/threadInitiators.json`，用 chatKey 作为 key。

### 2.4 附件处理：保存到本地缓存 + 注入 `@attach`

当 message 含附件（document/photo/voice/audio/video）时，ShipMyAgent 会：
...
只有显式启用 reply（例如 `sma chat send --reply`）时，runtime 才会尝试解析目标 `messageId` 并映射到 Telegram `reply_to_message_id`。
