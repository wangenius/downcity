...
| 变量 | 用途 |
| --- | --- |
| `DOWNCITY_FEDERATION_ADMIN_SECRET_KEY` | `Admin City` 管理 City |
| `DOWNCITY_FEDERATION_TOKEN_SIGNING_KEY` | City 内部签发和校验 `user_token` |
...
`DOWNCITY_FEDERATION_ADMIN_SECRET_KEY`、`DOWNCITY_FEDERATION_TOKEN_SIGNING_KEY` 和 `BETTER_AUTH_SECRET` 会在 City 首次启动时自动写入 City 数据库的 `env` 表。provider API key 也统一通过 Admin API / City CLI 写入同一张表。
