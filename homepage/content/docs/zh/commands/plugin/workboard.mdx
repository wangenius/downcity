...
> 这页保留为命令入口。更完整的 Plugin 结构说明见 [Plugin 总览](/zh/agent-sdk-docs/plugins/overview)、[当前有哪些内建 Plugin](/zh/agent-sdk-docs/plugins/built-in-plugins) 和 [Plugin HTTP 注入](/zh/agent-sdk-docs/plugins/plugin-http)。
...
该接口返回结构化 JSON，但只包含对外安全的概览状态，不会直接暴露内部 session、plugin runtime、task 或路径细节。
...
- agent 会作为带方向和状态符号的 sprite 沿 corridor、gate、station 巡游与停靠
