...
Downcity 给 creators、indie builders 和团队提供一套可复用的 Agent 运行基础设施，把 Agent、模型、工具、任务、记忆、插件、共享服务、权限、usage、计费和控制台收束到同一层。你不需要为每个新的 AI 产品重复搭一遍 Agent 后端，而是可以让多个 Agent、产品和工作流复用同一套 runtime。
...
- 共享服务能力：集中管理模型目录、运行时 env、service routing、accounts、balance、usage、payment 和 HTTP 访问。
...
| 包 / 目录 | 作用 |
| --- | --- |
| `downcity` | 官方 CLI 聚合包，安装后提供 `town` 本机 Agent 宿主命令与 `city` 共享服务管理命令。 |
| `@downcity/agent` | 单 Agent runtime 与 SDK，负责 session、tool loop、service、plugin、HTTP/RPC、sandbox 与宿主接入。 |
| `@downcity/city` | 共享服务 runtime 与访问 SDK，负责 service 注册、action、auth、env、town 访问边界与 HTTP 调用。 |
...
- City 连接：通过 `town city` 让本机 Agent 连接当前 City server；共享模型和服务资源由 `city` 管理。
- 共享后端服务：让多个 Agent 和产品复用 accounts、balance、usage、payment、env、auth 和 service routing。
- 内建 Agent 能力：`chat`、`task`、`memory`、`shell`、`contact`、`skill`、`web`、`asr`、`tts`、`workboard`。
- 产品表层：Console UI、Chrome Extension、Agent SDK、共享服务 SDK 和 UI SDK。
...
`town` 负责本机 Agent 宿主，`city` 负责共享服务管理。使用 `town update` 更新全局 CLI。
...
`city` 负责共享服务和模型管理。`town city` 负责把当前 City 连接导入本机 Agent runtime。
...
- 共享服务 SDK 文档：[downcity.ai/city-sdk-docs](https://downcity.ai/city-sdk-docs)
