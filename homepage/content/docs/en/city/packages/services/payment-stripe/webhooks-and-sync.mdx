...
If redirect URLs are not configured explicitly, the service uses `DOWNCITY_CITY_BASE_URL` first, then falls back to the current request origin, and exposes two built-in result pages:
...
This layer does not grant studio access and does not mirror every Stripe object.
...
Other Stripe events can be ignored until the studio explicitly needs a new payment flow.
