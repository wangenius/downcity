---
title: Admin Visa
description: 可信环境如何管理 bay、签发 user_token，并维护 Runtime env。
---

`Admin Visa` 只应该运行在可信环境里。
...
把 `Admin Visa` 看成“可信侧和 City 的管理桥”最准确。
...
如果 `User Visa` 负责“用户态怎么调用 City”，那 `Admin Visa` 负责“可信侧怎么把这个调用环境准备好”。
...
<div className="dc-doc-flow" aria-label="Admin Visa 的调用链">
  ...
  <div className="dc-doc-lane">
    <div className="dc-doc-node">
      ...
      <span>调用 `/v1/bays/*`、`/v1/bays/tokens/apply`、`/v1/env/*`、`/v1/ai/models`、`/v1/base/instruction`。</span>
    </div>
  </div>
  <div className="dc-doc-arrow" aria-hidden="true">→</div>
  <div className="dc-doc-lane">
    <div className="dc-doc-node">
      ...
      <span>校验 `admin_secret_key` 后，再执行 bay、token 和 env 管理动作。</span>
    </div>
  </div>
</div>
...
当前语义就是删除 bay 记录。调用前要先确认这是否符合你的业务预期。

## `tokens.apply()`

这是最常用的管理端能力：为某个用户在某个 bay 下申请 `user_token`。
...
3. 把 `bay_id + user_token` 返回给前端
4. 前端再用 `User Visa` 调 City

## 它和 accounts 服务是什么关系

`Admin Visa` 不会替代 accounts 服务，反过来也一样。
...
- 业务后端再用 `Admin Visa` 申请 `user_token`
- 前端拿到 `bay_id + user_token`

### 模式 B：accounts 服务做登录

- 前端先用 guest `User Visa` 调 `accounts.login/register`
- accounts service 直接返回 `user_token`
- 前端切换到正常 `User Visa`

所以：

- `Admin Visa` 负责可信侧管理动作
- accounts 服务负责最小账号能力
...
除了 `bays.*` 和 `env.*`，管理端还可以读取 City 当前暴露的能力目录。
...
## Admin Visa 也可以调 service 的管理侧 service

除了 `bays.*` 和 `env.*`，`Admin Visa` 也可以调用 service 暴露出来的管理侧 service。
...
也就是说，`Admin Visa` 不只是在调内建 bays/env 接口，它同样是可信侧调用统一 service 路由的入口。

## 错误处理

`Admin Visa` 收到非 2xx HTTP 响应时会抛出 `Error`。这个错误会带两个额外字段：
...
- `403`：目标 bay 已暂停，不能继续签发 token。
- `404`：目标 bay 不存在。
- `500`：City 缺少必要配置，或管理动作内部执行失败。

## 当前不归 `Admin Visa` 管的事情
...
也就是说，`Admin Visa` 负责维护 bay、token 和 env；模型定义和 service 挂载方式仍然属于 `City` 运行时层。
