...
- `read`: paginated text reads and image attachments
- `write`: file creation or explicit atomic full-file replacement
- `edit`: atomic exact-text edits within one file

File and search tools can only access the Agent project root. They have no unrestricted mode; operations outside the project still require the Shell unrestricted approval flow. `grep` and `find` respect ignore files, return at most 200 results by default, and accept `max_results` up to 2,000.

When `read` opens a PNG, JPEG, GIF, WebP, or BMP image, the image is attached to the next model step as a temporary user file part. Other binary files return metadata only.
...
A live UI reads the complete local-tool request from `mutation.part.approval.request` when `session.subscribe((mutation) => {})` receives an approval-required Tool Part, then calls `session.resolve_approval(...)` after the user decides.
...
`AgentContext` does not expose a generic path bundle. A plugin uses `context.rootPath` as the project boundary and owns its domain storage paths inside the plugin. Operations on Agent Sessions go through `context.sessions`, not the Agent's Session directories.
