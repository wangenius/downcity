...
This is common for subscription or plan-based products.

### Scenario 2: analytics now, billing later

Many products first need to know which models, products, and users consume the most before they need a full billing engine.
...
### Not all billing logic should live inside InfraRuntime

Many teams use InfraRuntime as the fact layer and access layer while keeping higher-level commercial logic in their own systems.

## Read next

- For the usage fact model, read [Usage Event Model](/en/docs/city-infra/packages/services/usage/event-model)
- For usage read surfaces, read [Querying and Summary](/en/docs/city-infra/packages/services/usage/querying-and-summary)
- For Stripe topup syncing, read [@downcity/services](/en/docs/city-infra/packages/services)
