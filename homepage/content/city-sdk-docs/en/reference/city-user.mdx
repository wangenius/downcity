...
The low-level contract is intentionally small: the client sends `prompt` / `messages` / `model` / `size` / `ratio` / `quality` / `count` / `provider_options`; the City-side Provider normalizes upstream image responses, such as OpenAI image APIs, into AI SDK `UIMessage` file parts.
