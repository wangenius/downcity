...
`User City` is the runtime client for town-facing calls.

It binds one user inside one town:

- `city_url`
- `town_id`, automatically injected into AI service and user action calls
...
So from the product side, `User City` is not only "the thing that calls models." It is the unified user-context entry into the City runtime.
...
After receiving `session.user_token`, create a User City with `town_id + user_token` for AI service calls.
...
Pass `city_url + town_id + user_token`. This is for:
...
This keeps raw model IDs from scattering across your town code.
...
`client.ai.image()` uses job polling internally: it creates an image job with `image_create()`, polls `image_result()`, and returns the same `UIMessage` once the job succeeds. This avoids holding a long HTTP connection while slow image providers finish generation.
...
If you want to control polling yourself, call the lower-level job API directly:

```ts
const job = await client.ai.image_create({ prompt: "A fox standing in the snow" });
const current = await client.ai.image_result({ job_id: job.job_id });
```

The low-level contract is intentionally small: the client sends `prompt` / `messages` / `model` / `size` / `ratio` / `quality` / `count` / `provider_options`; the City-side Provider normalizes upstream formats such as Luchi async jobs, OpenAI/302.ai images APIs, and Gemini `generateContent` into AI SDK `UIMessage` file parts.

The City-side Provider `image` / `video` actions should also return `UIMessage`.

## `ai.tts()` / `ai.asr()`

`tts()` and `asr()` keep open return types because audio input and output transport shapes vary more across towns:
...
If you want the town code to think in terms of "payment methods" instead of hand-writing `service + action`, use:
...
- the request `town_id` does not match the town bound to the token
...
- pausing or re-activating towns
