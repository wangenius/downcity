...
function buildAgentChatFailureText(error?: string): string {
  return (
    String(error || "").trim() ||
    "Agent daemon returned empty error (check config with `bay agent status`)"
  );
}
...
async function resolveChatTargetAgentId(inputId?: string): Promise<string | null> {
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    emitCliBlock({
      ...
      note: "Use `bay agent chat --to <id>` or run this command in an interactive terminal.",
    });
    return null;
  }
  ...
  if (runningAgents.length === 0) {
    emitCliBlock({
      ...
      note: "Run `bay agent start` first.",
    });
    return null;
  }
...
export async function chatCommand(options: AgentChatCliOptions): Promise<void> {
  ...
  if (options.json === true) {
    emitCliBlock({
      ...
      note: "Use `bay agent chat --message <text> --json` for one-shot structured output.",
    });
    return;
  }
