 ...
 * - 不处理进程控制；启动/utils/停止逻辑单独放到 AgentActions 中。
 */

import fs from "fs-extra";
import path from "node:path";
import { basename } from "node:path";
import { getDaemonLogPath, getDaemonMetaPath, isProcessAlive, readDaemonPid } from "@/city/runtime/daemon/Manager.js";
import {
  getProfileMdPath,
  getDowncityJsonPath,
...
import { isAgentProjectInitialized } from "@/city/runtime/project/AgentInitializer.js";
...
import type {
  ...
  ConsoleUiDaemonMeta,
  ConsoleUiShipJson,
} from "@/shared/types/ConsoleUiGateway.js";
...
function selectAgentId(
  ...
  // 关键点（中文）：没有运行中 agent 时不返回历史/utils/离线 id，避免 UI 持续请求并触发 503 噪音。
  return "";
}
