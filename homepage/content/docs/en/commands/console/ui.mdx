...
- For directories that are not initialized yet, Global Agents also supports initialize-on-start: it creates `PROFILE.md`, `SOUL.md`, `USER.md`, `ship.json`, and `.ship/` before starting the runtime.
...
- Global / Env provides manual env management for both Console-shared and agent-private entries, all stored in `~/.ship/ship.db` `env_entries`.
...
- Agent / Authorization is stored in console `~/.ship/ship.db`, not written back to project `ship.json` in plaintext.
