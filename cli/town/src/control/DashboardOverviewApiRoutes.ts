...
const DC_VERSION = (() => {
  ...
  for (const package_path of candidate_package_paths) {
    try {
      const pkg = fs.readJsonSync(package_path) as { version?: string };
 ...
 */
export function registerDashboardOverviewApiRoutes(
  params: DashboardOverviewApiRouteParams,
): void {
  const { app } = params;
  ...
  app.get("/api/dashboard/overview", async (c) => {
    try {
      ...
      const sessions = ensureConsoleSessionItem(
        session_page.items.map((item) => toOverviewSessionItem(item)),
      );
...
function toOverviewSessionItem(
