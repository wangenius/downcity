---
title: Provider 环境变量
description: 如何在 Federation 运行环境中配置 Provider API Key。
---

所有 Provider 需要的 API Key 都通过 Federation 自己的 env 表统一管理。
...
`ctx.env(key)` 从 Federation 当前运行时绑定的 env 表读取。AIService 的自动透传 handler 也通过 `ctx.env(envKey)` 读取 API Key。
