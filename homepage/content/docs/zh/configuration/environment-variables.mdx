...
description: 说明 env_entries、channel_accounts 与 <agent>/.env 的作用域和 agent env 合并规则
---
...
|---|---|---|---|---|
| Console 全局 env | `~/.downcity/downcity.db` `env_entries`（`scope=global`） | 是（加密） | 所有 agent | 全局共享 key、共享占位符 |
| Agent 私有 env（DB） | `~/.downcity/downcity.db` `env_entries`（`scope=agent` + `agent_id`） | 是（加密） | 单个 agent（`agent_id`） | 项目私有密钥 |
...
1. 持久化的 env 类数据统一在 `downcity.db` 加密存储。
2. `<agent>/.env` 是该 agent 的运行时叠加层。
3. `downcity.json` 只做绑定（如 `channelAccountId`），不保存明文凭据。
...
sequenceDiagram
  ...
  CLI->>ENV: 解析项目 .env
  CLI->>CLI: 生成 agent env = agent scope DB env + .env（.env 优先）
  CLI->>JSON: 读取并合并 downcity.json
...
3. 再叠加当前 agent env（`agent scope env + <agent>/.env`）。
4. 最后注入 `DC_CTX_*` 等请求上下文变量。

因此，同名键冲突时优先级是：`DC_CTX_*` > agent env > global env > 宿主进程基础 env。
...
2. agent 进程 env 注入（`env_entries` 中的 agent scope env + `.env`）。
3. `shell` 子进程注入（global env + 当前 agent env + `DC_CTX_*`）。
...
如果运行环境本身拿不到真实公网 IP（例如机器位于 NAT、SLB、反向代理之后），可以显式设置：

1. `DOWNCITY_PUBLIC_URL`：直接指定完整公网 URL，例如 `https://console.example.com`
2. `DOWNCITY_PUBLIC_HOST`：只指定公网 host，CLI 会自动补成 `http://<host>:5315`

`city start` 会在没有显式配置时自动探测公网 IP，并把结果写入全局 Console Env 的 `DOWNCITY_PUBLIC_HOST`。这个值也会被 agent 的 `contact link` 用来生成可转交的联系码。
