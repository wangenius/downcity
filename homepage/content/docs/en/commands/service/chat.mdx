...
- `--q <text>`: keyword filter (title/chatId/contextId/actor)

## `info` key options

- `--chat-key <chatKey>`: target conversation (fallback to `DC_CTX_CHAT_KEY`)
- `--context-id <contextId>`: explicit context id (higher priority)
...
- the command returns as soon as runtime accepts the request, while actual delivery is scheduled in runtime memory
- if runtime exits or restarts before delivery, pending delayed/scheduled sends are not restored automatically
- for durable and recoverable scheduling, prefer the `task` service
