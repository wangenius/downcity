...
> `downcity` 与 `studio` 都是同一个 CLI 可执行命令。
>
> `@downcity/studio-cli` 与 `downcity` 发布的是同一份 CLI 内容和版本线。
>
> 如果安装后仍然找不到 `studio`，请先看 [FAQ](/zh/docs/operations/faq)。
>
> 后续升级可以直接执行 `studio update`，它会更新你最初安装的那个包。
...
> `studio agent start` 依赖 console 先运行。
