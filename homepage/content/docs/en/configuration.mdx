...
- In `direct`, control parameters are read from Markdown frontmatter metadata
  (`chatKey`, `reply(message_id)` / `message_id` / `messageId`, `delay`, `sendAtMs/sendAt/time`, `react/reactions`).
...
- In `direct`, `delay/sendAtMs/sendAt/time` is scheduled asynchronously: run completion does not wait for delayed delivery.
