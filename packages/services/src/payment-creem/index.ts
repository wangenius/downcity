...
const creemPaymentEnv: EnvRequirement[] = [
  ...
  {
    key: "CREEM_SUCCESS_URL",
    description: "默认支付成功跳转地址；未在请求里传 success_url 时使用",
    required: false,
  },
  {
    key: "DOWNCITY_CITY_BASE_URL",
    description: "City 对外访问地址；未显式配置 success URL 时，会自动生成 Creem 默认跳转页地址",
    required: false,
  },
  {
    key: "CREEM_CURRENCY",
...
export function creemPaymentService(options: CreemPaymentServiceOptions): ServiceDefinition {
  ...
  return {
    ...
    instruction: [
      ...
      "当 options 未显式传入时，会回退读取 CREEM_API_KEY / CREEM_PRODUCT_ID / CREEM_WEBHOOK_SECRET / CREEM_SUCCESS_URL。",
      "如果 success URL 仍未配置，会继续尝试基于 DOWNCITY_CITY_BASE_URL 自动生成默认跳转页地址。",
      `currency=${normalized.currency}，success_url=${normalized.success_url || "read from CREEM_SUCCESS_URL or route input"}。`,
          ...
          const created = await createCreemCheckoutSession(
            ...
            resolveApiBaseURL(normalized, ctx),
            {
              ...
              success_url: resolveCheckoutRedirectURL({
                ...
                fallback: normalized.success_url ?? ctx.env("CREEM_SUCCESS_URL"),
                label: "success_url",
                redirectPath: "/v1/payment.creem/redirect/success",
                ctx,
...
function resolveCheckoutRedirectURL(input: {
  ...
  label: string;
  redirectPath: "/v1/payment.creem/redirect/success" | "/v1/payment.creem/redirect/cancel";
