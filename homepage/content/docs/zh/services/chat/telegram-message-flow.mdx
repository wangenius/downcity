...
> 代码参考：`packages/downcity/src/adapters/telegram/*`、`packages/downcity/src/adapters/base-chat-adapter.ts`、`packages/downcity/src/core/runtime/*`、`packages/downcity/src/core/runtime/agent.ts`
...
- Downcity 以 **轮询** 模式工作：定时调用 `getUpdates` 拉取 `TelegramUpdate[]`，并更新 `lastUpdateId`（持久化在 `.ship/.cache/telegram/lastUpdateId.json`）。
...
```text
<file type="document" caption="xxx.pdf">.ship/.cache/telegram/1738...-xxx.pdf</file>
