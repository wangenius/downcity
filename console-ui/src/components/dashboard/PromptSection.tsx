...
export function PromptSection(props: PromptSectionProps) {
  ...
  return (
    <section className="min-w-0 space-y-3">
      ...
      <div className="rounded-[16px] bg-secondary px-3 py-2 text-xs text-muted-foreground">
        ...
        {sections.map((section, sectionIndex) => {
          ...
          return (
            <details
              ...
              open={sectionIndex <= 1}
              className="rounded-[20px] bg-secondary px-3 py-3"
            >
              <summary className="cursor-pointer list-none py-1 text-xs font-semibold text-foreground/80">{`${title} · ${items.length}`}</summary>
              <div className="space-y-2 pt-1">
                {items.map((item, itemIndex) => (
                  <div key={itemIndex} className="space-y-1">
                    <div className="text-[11px] text-muted-foreground">#{String(item.index || "-")}</div>
                    <pre className="overflow-auto rounded-[16px] bg-card p-3 font-mono text-[11px] leading-relaxed text-foreground/85">
                      {String(item.content || "")}
                    </pre>
                  </div>
                ))}
