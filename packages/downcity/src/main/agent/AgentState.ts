 ...
 *
 * 关键点（中文）
 * - 这里是单 agent 运行态的主装配入口，负责把 config、model、session、service、plugin 串起来。
 * - `AgentState` 表示已经可以对外提供能力的完整运行态；启动早期的基础状态由 `RuntimeState.ts` 维护。
 * - 该模块同时负责热重载提示词系统与导出统一的 `getExecutionContext()` 入口。
 */
...
import {
  getExecutionContext,
} from "@/main/agent/ExecutionContext.js";
import {
  getAgentState,
  getAgentStateBase,
  setAgentState,
  setAgentStateBase,
  type AgentState,
} from "@/main/agent/RuntimeState.js";
...
export type { AgentStateBase, AgentState } from "@/main/agent/RuntimeState.js";
export {
  getAgentState,
  getAgentStateBase,
  setAgentState,
...
export { getExecutionContext } from "@/main/agent/ExecutionContext.js";
