...
When the first screen only loads a recent page, a live Mutation can still reference an older Message that is not in memory. Keep pending state by `message_id` or load adjacent history according to product policy; never overwrite the current page merely to handle one Mutation.
