...
| 包 / 目录 | 作用 |
| --- | --- |
| `downcity` | 官方 CLI 聚合包，安装后提供 `studio` 本机 Agent 宿主命令与 `city` 可部署 City 管理命令。 |
...
- Studio 控制面：通过 `studio start` 或 `studio console` 托管本机 Agent，并进入浏览器控制面。
...
`studio` 负责本机 Agent 宿主，`city` 负责可部署 City 管理。使用 `studio update` 更新全局 CLI。
