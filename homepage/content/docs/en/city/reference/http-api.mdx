...
## Town Routes (common user-side routes)
...
`/v1/accounts/oauth/callback` is the fixed return URL for GitHub, Google, and other third-party OAuth providers. Town clients usually do not call it directly.

## Admin Routes (same route space, admin credential required)

| Route | Method | Purpose |
| --- | --- | --- |
| `/v1/towns/list` | `GET` | List towns |
| `/v1/towns/create` | `POST` | Create town |
| `/v1/towns/pause` | `POST` | Pause town |
| `/v1/towns/activate` | `POST` | Activate town |
| `/v1/towns/remove` | `POST` | Remove town |
| `/v1/towns/tokens/apply` | `POST` | Issue user_token |
...
Request:
```json
{ "town_id": "town_xxx", "model": "deepseek-v4-flash", "prompt": "Hello" }
```
...
If the request body includes `town_id`, it must match the town bound to the `user_token`. Otherwise City returns `403`.
