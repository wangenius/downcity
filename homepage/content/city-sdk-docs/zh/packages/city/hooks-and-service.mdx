...
- `new AccountsService()` 给 City 增加 `accounts` service 容器
- `AccountsService` 会自动挂载 `/providers`、统一登录入口、`me`、管理侧读取、better-auth passthrough 与 OAuth callback 入口；具体登录方式只有在对应 provider 挂载且配置满足后才会开放
...
- 想看账号能力，读 [@downcity/services](/zh/payments)
- 想看 usage 记录，读 [@downcity/services](/zh/payments)
- 想看 Stripe 一次性充值，读 [@downcity/services](/zh/payments)
