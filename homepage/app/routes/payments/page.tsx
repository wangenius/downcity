...
/**
 * 递归按侧边栏顺序收集 Payments 文档 URL，用于目录页跳转。
 */
...
function findFirstChildDocUrl(lang: "en" | "zh", slugs: string[]) {
  ...
  const tree = paymentsSource.getPageTree(lang) as PageTreeNode | undefined;
  if (!tree) return undefined;

  const urls: string[] = [];
  collectPageUrls(tree, urls);

  const prefix = `/${lang}/payments/${slugs.join("/")}/`;
...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  const page = paymentsSource.getPage(cleanSlugs, lang);
  if (!page) {
    throw new Response("Not found", { status: 404 });
  }
  const alternate_lang = lang === "en" ? "zh" : "en";
  const alternate_page = paymentsSource.getPage(cleanSlugs, alternate_lang);
  ...
  return {
    ...
    title: page.data.title ?? "Payments",
...
export function meta({ loaderData }: Route.MetaArgs) {
  ...
  const title = `${loaderData.title} — Downcity Payments`;
  const description = loaderData.description || "Downcity Payments documentation";

  return create_page_meta({
    title,
    description,
    pathname: loaderData.url,
    keywords: "Downcity, Payments, accounts, balance, usage, stripe",
...
const clientLoader = browserCollections.payments.createClientLoader({
  id: "payments",
  component: ({
    default: Mdx,
    frontmatter,
