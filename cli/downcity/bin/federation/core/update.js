 ...
 * 执行 CLI 自更新。
 */
export async function updateCli(cwd = process.cwd()) {
...
export async function readInstalledCliVersion() {
    try {
        ...
        const package_json = JSON.parse(await readFile(path.join(global_paths.package_dir, "package.json"), "utf8"));
 ...
 */
async function installCliDeployGlobally(deploy_dir, cwd) {
 ...
 */
async function resolveGlobalCliPaths(cwd) {
 ...
 * 重新创建全局命令链接。
 */
async function recreateSymlink(link_path, target_path) {
    try {
 ...
 * 查找 downcity 仓库根目录。
 */
function findDowncityWorkspaceRoot(startDir) {
 ...
 */
function isDowncityWorkspaceRoot(dir) {
