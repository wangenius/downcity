...
两者都有 `prompt()`、`stop()`、`compact()`、`subscribe()`、`messages()`、`approvals()` 和 `fork()`。本地 Session 还提供 `syncshot()` 手动重新生成 system，以及 `snapshot()` 显式持久化完整 system；远程 Session 的 system 刷新与持久化策略由服务端宿主管理。
