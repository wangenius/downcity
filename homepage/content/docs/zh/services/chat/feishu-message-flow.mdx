...
- `message_type === post`：把富文本降级成“纯文本 + 可下载附件”
- `message_type === image | file | audio | media`：先下载到本地缓存，再注入 `<file ...>`
- 其他类型：直接回错误提示

所以当前版本里，飞书 `post`、图片、视频、文件、音频都会像 Telegram 一样被归一化后送入执行：

- `post` 内的文本 / 链接 / @ 会转成自然语言文本
- `post` 内的图片 / 媒体会转成附件占位并下载到本地
- 所有可下载附件最终都会以 `<file ...>` 形式注入执行输入
- 其中图片和 PDF 还会 best-effort 注入为模型 `file parts`
...
会先对用户那条消息自动补一个 reaction，表示“已经收到，开始处理”。

当前实现使用飞书消息 reaction API 的 `OK` 类型。
...
另外，当前版本支持在回复文本里写 `<file>` 标签并自动转换为飞书文件消息：
...
1. 先把整条消息解析成有序片段 `segments[]`
2. 文本片段按原顺序发送为飞书文本消息
3. 附件片段按原顺序上传并发送为对应附件消息
...
<file type="document" caption="资料">reports/a.pdf</file>
...
- `post` 里的复杂样式（加粗、斜体、分栏等）当前会退化成纯文本，不保留视觉样式
