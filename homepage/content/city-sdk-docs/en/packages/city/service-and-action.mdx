...
const list = await city.service("translate").get("list");
```
...
- `base.use(service)`
- `city.service(id).action(name).invoke(input)`
- `city.service(id).get(path, query?)`
...
From the product side, both still go through `city.service("service_id").action("action_id").invoke(...)`.
...
- For product-side calling, read [@downcity/city](/en/city-sdk-docs/packages/city/federation)
- For lower-level method details, read [City API](/en/city-sdk-docs/reference/federation)
