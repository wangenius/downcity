---
title: Many Products, One InfraRuntime
description: How one InfraRuntime can serve multiple products without mixing product differences with shared infrastructure.
---

When many products share one InfraRuntime, the goal is not “save one server.” The goal is to separate **shared infrastructure** from **product-specific differences**.
...
That lets hooks differentiate behavior with `ctx.product` and `ctx.user.metadata` instead of cloning InfraRuntime for each product.
