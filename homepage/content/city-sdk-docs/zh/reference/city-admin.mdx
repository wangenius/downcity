---
title: Admin City
description: 可信环境如何管理 town、签发 user_token，并维护 Runtime env。
---
...
<div className="dc-doc-flow" aria-label="Admin City 的调用链">
  ...
  <div className="dc-doc-lane">
    <div className="dc-doc-node">
      ...
      <span>调用 `/v1/towns/*`、`/v1/towns/tokens/apply`、`/v1/env/*`、`/v1/ai/models`、`/v1/base/instruction`。</span>
    </div>
  </div>
  <div className="dc-doc-arrow" aria-hidden="true">→</div>
  <div className="dc-doc-lane">
    <div className="dc-doc-node">
      ...
      <span>校验 `admin_secret_key` 后，再执行 town、token 和 env 管理动作。</span>
    </div>
  </div>
</div>
