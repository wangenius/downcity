...
|-------|------|----------|---------|-------------|
| `provider` | string | Yes | - | LLM provider: `anthropic`, `openai`, `deepseek`, `gemini`, `custom` |
...
> Note: `custom` now uses OpenAI Responses protocol by default (`/v1/responses`), which matches gateways like GMN that only support Responses.
> Minimal GMN example:
>
> ```json
> {
