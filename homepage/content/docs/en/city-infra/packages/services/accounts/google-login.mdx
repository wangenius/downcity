...
- the public InfraRuntime domain you plan to use
...
If you are only using the Downcity accounts service server-side OAuth flow, this redirect URI is the critical part. You usually do not need to register your product frontend as a JavaScript origin for this flow alone.

## The fields Google users most often get wrong

- choosing the wrong `Application type` instead of `Web application`
- putting the product frontend URL into `Authorized redirect URIs` instead of the InfraRuntime callback URL
...
After Google login succeeds, the accounts service still returns a normal InfraRuntime `user_token`.

So the product does not need a different post-login client flow just because the user came from Google:
...
- For the overall OAuth flow, continue with [OAuth and Session](/en/docs/city-infra/packages/services/accounts/oauth-and-session)
- If you also need WeChat Website App login, continue with [WeChat Login Setup](/en/docs/city-infra/packages/services/accounts/wechat-login)
- For the full integration path, read [Accounts Service](/en/docs/city-infra/guides/accounts-service)
- After deployment, write `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` into InfraRuntime env through City Terminal or the Admin API
