...
description: 使用 fed deploy 把 Federation 项目部署到 Workers、D1、Queue 和 R2。
---

这页是 Federation 项目部署到 Cloudflare Workers 的正式操作路径。

如果你只是想理解 Federation 怎么接入 Workers，先读 [Cloudflare Workers](/zh/city-sdk-docs/guides/cloudflare-workers)。这一页只讲上线步骤：`federation.json`、本地创建、本地部署、D1 / Queue / R2、admin key、env、OAuth callback 和健康检查。
...
`id` 是不依赖路径的 Fed 身份，`deployment.target` 表示部署目标。`entry` 和 `deployment.resources` 是可提交的部署意图；D1 database id、Cloudflare account、Worker URL 和 Wrangler 配置由 `fed deploy` 处理。
...
如果 `federation.json` 的 `deployment.target` 是 `cloudflare-workers`，`fed deploy` 会自动检查 Cloudflare 登录态。没有登录时会唤起 Wrangler 登录；account id 保存在系统级 Federation registry，后续部署自动复用。
...
如果没有显式配置 D1 / Queue，`fed deploy` 会根据 `federation.json.name` 使用默认名称 `${name}-db` 和 `${name}-queue`。Storage 只有在 `deployment.resources.storage` 存在时启用。
...
- 对 `cloudflare-workers` target 按 `deployment.resources.d1.name` 查找 D1；不存在时自动创建
- 按 `deployment.resources.queue.name` 查找 Queue；不存在时自动创建
...
- 从远程 D1 读取实际 admin key，并加密保存到本地 Federation registry
- 把 Worker URL、部署状态和 admin key 登记到本地 Federation registry，但不修改当前 active Federation
- 在传入 `--verify` 时请求 Worker `/health`

部署输出会按阶段汇总显示：`Project`、`Version`、`Build`、`Typecheck`、`D1 Database`、`Queue`、`Storage`、`Admin Key`、`Wrangler`、`Deployment` 和 `Federation Registry`。
...
`fed deploy --dry-run` 和 `fed deploy --verify-only` 不会修改项目版本号。
...
Federation 首次启动时会生成 admin key 并写入 D1 `env` 表。`fed deploy` 会等待 `/health` 初始化完成，再从远程 D1 读取实际 key，加密保存到本地 Federation registry；key 不会写入 `federation.json`。

用户在 downfed 中打开该 Federation 后，admin 工作区和 `fed query` 会自动使用本地保存的 key，不再需要手动执行 D1 查询。
