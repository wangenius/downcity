 ...
 *
 * 关键点（中文）
 * - 为 Town CLI 的输出排版提供统一的结构化输入。
 * - 所有命令层只描述“要表达什么”，具体排版交给 reporter 模块。
 * - 类型与渲染器放在同一子域，避免输出协议继续漂移到集中类型目录。
 */
/**
 * CLI 输出区块的语气类型。
 */
export type CliReportTone = "accent" | "success" | "info" | "warning" | "error";
/**
 * CLI 输出中的单条键值详情。
 */
...
//# sourceMappingURL=CliReporterTypes.d.ts.map