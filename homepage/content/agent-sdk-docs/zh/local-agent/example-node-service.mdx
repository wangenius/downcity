...
description: 一个典型的本地 Agent 嵌入模式示例，并说明什么时候要接 Federation AIService
---
...
- 或通过 `downcity agent start` 由 Downcity 暴露 HTTP gateway
...
也就是说，Node 宿主嵌入 `Agent` 和通过 Downcity 连接 `Federation AIService`，是两种不同的使用路径。
