...
In this setup, `AccountsService` owns the full accounts HTTP surface. Mounting it once also exposes registration, login, OAuth start/result, the better-auth passthrough route, and the OAuth callback route, so edge workers do not need to call `router()` for accounts internals.
...
Services add capability into the Federation. The product-side calling surface is still [@downcity/city](/en/city-sdk-docs/packages/city/federation).

## Read next

- For unified product-side service calls, read [@downcity/city](/en/city-sdk-docs/packages/city/federation)
