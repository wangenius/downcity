...
实时 UI 在 `session.subscribe((mutation) => {})` 收到 `approval-required` Tool Part 时，从 `mutation.part.approval.request` 读取完整本地工具请求，并在用户决定后调用 `session.resolve_approval(...)`。
...
`AgentContext` 不提供通用路径集合。Plugin 通过 `context.rootPath` 确定项目边界，并在自己的模块内管理领域存储路径；需要操作 Agent Session 时使用 `context.sessions`，不要读取 Agent 的 Session 目录。
