...
当前 `downcity` CLI 不再启动或重新绑定 Console UI。Console 公网访问应该由 Console 产品部署、反向代理、隧道或托管层提供。

这些值通常由本地 runtime host 探测时写入，或由部署环境注入，不是 `contact link` 要求用户手动填写的参数：

1. `DOWNCITY_PUBLIC_URL`：完整公网 URL，例如 `https://console.example.com`
2. `DOWNCITY_PUBLIC_HOST`：公网 host

本地 runtime host 会在可能时自动探测公网 host，并把结果写入 City 侧 Env 的 `DOWNCITY_PUBLIC_HOST`。这个值也会被 agent 的 `contact link` 用来生成可转交的联系码。
