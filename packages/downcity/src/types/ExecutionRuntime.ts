/**
 * ExecutionRuntime 类型定义。
 *
 * 关键点（中文）
 * - 这是 services / plugins / prompt system 共用的一套统一执行端口。
 * - 这里表达的是“执行时可用能力”，而不是宿主层 AgentRuntime 本体。
 * - 统一后不再区分 service runtime / plugin runtime 两套概念。
 */
 ...
 * 统一执行运行时。
 */
export interface ExecutionRuntime extends ExecutionContext {
  /**
   * Session 能力入口。
   *
   * 关键点（中文）
   ...
   *
   * 关键点（中文）
   * - 统一支持 `runtime.services.invoke(...)` 写法。
   * - 与 `invoke` 指向同一实现。
   */
