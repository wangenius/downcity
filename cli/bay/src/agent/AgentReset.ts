/**
 * `bay agent reset`：重新配置 Agent 的执行绑定。
 *
 * 关键点（中文）
...
export async function agentResetCommand(cwd: string = "."): Promise<void> {
  ...
  if (!controlPlaneRunning) {
    emitCliBlock({
      ...
      facts: [
        { label: "step 1", value: "bay start" },
        { label: "step 2", value: `bay agent restart ${projectRoot}` },
      ],
    });
    return;
  }
  ...
  emitCliBlock({
    ...
    note: `run \`bay agent restart ${projectRoot}\` to apply the new model`,
  });
}
