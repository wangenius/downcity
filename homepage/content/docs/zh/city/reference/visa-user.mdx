...
拿到 `session.user_token` 后，再创建带 `bay_id + user_token` 的 User Visa 调 AI service。

## guest 调用和用户态调用的区别

可以把 `User Visa` 分成两个阶段理解：
...
对 `User Visa` 来说，这两类 service 的调用方式完全一样。
...
`User Visa` 收到非 2xx HTTP 响应时会抛出 `Error`。这个错误会带两个额外字段：
...
这些都属于可信环境动作，应该交给 [Admin Visa](/docs/city/reference/visa-admin) 或你自己的后端。
