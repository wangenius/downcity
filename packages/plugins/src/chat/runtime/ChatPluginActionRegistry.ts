...
const CHAT_SEND_HELP_TEXT = [
  "",
  "消息协议：",
  "  frontmatter metadata 字段语义与 `studio chat send` 参数一致。",
  "  附件使用 `<file type=\"...\">path</file>`，支持 `document/photo/voice/audio/video`。",
  "  正文与 `<file>` 可以交错出现，运行时会按原顺序发送。",
  "",
  "常用示例：",
  "  studio chat send --text 'done'",
  "  studio chat send --chat-key <chatKey> --text 'done'",
  "  cat <<'EOF' | studio chat send --stdin --chat-key <chatKey>",
  "  第一行",
  "  第二行",
  "  EOF",
  "  studio chat send --text-file ./result.md --chat-key <chatKey>",
  "",
  "说明：",
...
const CHAT_REACT_HELP_TEXT = [
  "",
  "常用示例：",
  "  studio chat react --emoji '👍'",
  "  studio chat react --emoji '✅' --message-id <messageId>",
  "  studio chat react --chat-key <chatKey> --message-id <messageId> --emoji '🔥'",
  "",
  "说明：",
