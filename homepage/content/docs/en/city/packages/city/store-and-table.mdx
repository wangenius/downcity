...
You should still prefer services, and `Admin Gate` for bay-facing or managed entry points.

### Frontends should not treat tables as their API

Bay-side callers should continue to use `User Gate` and `Admin Gate`, not raw table access.
