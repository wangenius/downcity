...
`image_create()` returns after the task is submitted. A backend worker or admin process should call `/v1/ai/image/persist`; AIService stores the final result in its built-in `async_jobs` table, and `image_result()` reads that saved state.
...
The low-level contract is intentionally small: the client sends `prompt` / `messages` / `model` / `size` / `ratio` / `quality` / `count` / `provider_options`; the City-side Provider creates, persists, and reads image jobs through its own storage.
