...
description: "The InfraRuntime data layer, table capabilities, and why product, env, and usage facts eventually live in the store layer."
---
...
Many InfraRuntime capabilities are not just in-memory state. They must persist as data facts:
...
That is why `@downcity/infra` has not only routes and services, but also a store and table layer.

## When you need to care about this layer directly

- you are checking whether a service actually wrote its tables
- you want your own business data to interact with InfraRuntime data
...
`base.table(name)` is InfraRuntime's controlled exposure of table capability inside one runtime and lifecycle.

## Minimal example: reading and writing tables

```ts
const products = await base.table("products");
...
### InfraRuntime is not only an API proxy

Many management and product capabilities depend on persistent data facts behind the route layer.

### `table()` is not a reason to skip good service design

You should still prefer services, and `AdminClient` for product-facing or managed entry points.

### Frontends should not treat tables as their API

Product-side callers should continue to use `UserClient` and `AdminClient`, not raw table access.

## Read next

- For how the database is wired in, read [InfraRuntime](/en/docs/city-infra/packages/infra/infra-runtime)
- For terminal inspection of these tables, read [@downcity/city](/en/docs/city-infra/packages/city-terminal)
