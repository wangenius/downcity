...
前端可以先根据 `methods.items` 判断当前 City 是否真的开放了 Stripe 支付入口，再决定是否展示充值按钮。
...
如果你部署在 Workers 里，`STRIPE_SECRET_KEY` 和 `STRIPE_WEBHOOK_SECRET` 都应该在部署后写入 City env。
...
也就是说，很多场景下你不必再单独配置 `STRIPE_SUCCESS_URL` / `STRIPE_CANCEL_URL`，甚至不必手动配置 `DOWNCITY_CITY_BASE_URL`。
