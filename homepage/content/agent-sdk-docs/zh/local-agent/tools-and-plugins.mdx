...
`image_result` 只读取一次当前任务状态。如果返回 `queued` 或 `running`，保留 `job_id`，稍后继续调用 `image_result`。如果返回 `succeeded`，`data` 会是包含图片 file parts 的 AI SDK `UIMessage`。

`ImagePlugin` 不直接绑定 City，也不关心 OpenAI、DeepSeek 等 provider 的具体协议。传入 `image_create` 和 `image_result` 任务函数后，插件只负责把两步式任务 action 暴露给 Agent；具体轮询节奏由产品、Agent 或调用方根据 `poll_after_ms` 决定。
