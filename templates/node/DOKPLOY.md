...
`/data` 已经配置为持久化 volume。SQLite 数据库、model、auth session、usage、payment 和 City env 都会保存在这里。

容器启动命令已经写在 Compose 的内联构建配置里：

```bash
pnpm -C templates/node start
```
...
默认监听 `127.0.0.1:43127`，并使用 `templates/node/data.sqlite`。如果要模拟容器路径，可以临时传入：

```bash
HOST=0.0.0.0 DOWNCITY_CITY_DATABASE_URL=file:/tmp/downcity.sqlite pnpm -C templates/node start
```
