...
/**
 * 递归按侧边栏顺序收集 City SDK 文档 URL，用于目录页跳转。
 */
...
function findFirstChildDocUrl(lang: "en" | "zh", slugs: string[]) {
  ...
  const tree = citySdkDocsSource.getPageTree(lang) as PageTreeNode | undefined;
  if (!tree) return undefined;

  const urls: string[] = [];
  collectPageUrls(tree, urls);

  const prefix = `/${lang}/city-sdk-docs/${slugs.join("/")}/`;
...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  const page = citySdkDocsSource.getPage(cleanSlugs, lang);
  if (!page) {
    throw new Response("Not found", { status: 404 });
  }
  const alternate_lang = lang === "en" ? "zh" : "en";
  const alternate_page = citySdkDocsSource.getPage(cleanSlugs, alternate_lang);
...
export function meta({ loaderData }: Route.MetaArgs) {
  ...
  return create_page_meta({
    ...
    keywords: "Downcity, City SDK, citybase, agent infrastructure, services",
...
const clientLoader = browserCollections.citySdkDocs.createClientLoader({
  id: "citySdkDocs",
  component: ({
    default: Mdx,
    frontmatter,
