...
> 面向用户的 Agent API。更多细粒度命令建议优先使用 CLI（`city ...`）。
...
- `sessionId` 应传 runtime 返回的目标会话 ID。聊天场景下该 ID 由 chat runtime plugin 维护映射（不再要求可由平台字段直接拼接）。
- 附件会先被 runtime 落盘，再以 `<file ...>` 标签自动注入到用户消息前部。
- 当 `sessionId` 映射到 chat 会话时，会复用 chat runtime plugin 的同一条执行队列（与平台入站消息一致），并在执行前写入 chat history。
