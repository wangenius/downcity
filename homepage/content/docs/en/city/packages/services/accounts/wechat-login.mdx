...
If the callback domain configured in WeChat does not match the real public City domain used by the bay, login usually fails immediately.

So two things must stay aligned:

- the real public City domain used by the bay
...
So the bay does not need a special post-login client flow just because the user came from WeChat:
...
- configuring one domain in WeChat while the bay actually uses another City domain
