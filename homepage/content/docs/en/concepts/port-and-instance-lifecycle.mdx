...
- `sma agent start/restart` no longer expose `--port`.
...
- Daemon startup args are persisted to project debug metadata (`.ship/.debug/shipmyagent.daemon.json`).
- This process hosts runtime HTTP APIs (`/api/services/*`, `/api/extensions/*`).

### service (capability domain)

- `sma service ...` and `sma <service> <action>` call agent runtime APIs.
- They resolve target project first, then host/port, then call `/api/services/*`.

### extension (extension domain)

- `sma extension ...` and `sma <extension> <action>` call `/api/extensions/*`.
...
- Daemon startup args are persisted in `.ship/.debug/shipmyagent.daemon.json`, and service/extension routing prefers this source.
...
1. `--agent <name>`
2. if path is default `.`, prefer `SMA_AGENT_NAME`
3. `--path` / current directory
4. if `.` and `SMA_AGENT_PATH` exists, prefer that path
...
- In multi-agent scenarios, prefer `sma service ... --agent <name>`.
- Do not keep manually exported `SMA_SERVER_PORT` in shell profile.
