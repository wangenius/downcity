...
- `agent`：单个项目的运行时
- `downcity` / Console：全局控制面

## 一句话区分

- `agent` 管“这个项目怎么运行”
- `downcity` 管“有哪些项目、本地共享 chat accounts 和入口”

如果你习惯把系统拆成“控制面”和“执行面”，也可以这样记：

- `downcity` / Console 是控制面
- `agent` 是执行面
...
2. 管理本地共享资源时，使用的是 Downcity 能力，例如 chat account、Console

## 常见例子

### 绑定模型

- 模型本身在 Federation AIService 目录里
...
- bot 凭据和 chat account 是 Downcity 级共享资源
...
- 项目 runtime 状态回答“这个 agent 是否运行”
- 受管 agent registry 回答“全局有哪些 agent”

## 一个高频误区

很多人会问：“既然 Downcity 是用户入口，那为什么还要强调 agent 是独立运行时？”
