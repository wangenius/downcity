...
| 包 / 目录 | 作用 |
| --- | --- |
| `downcity` | 官方 CLI 聚合包，安装后提供 `bay` 本机 Agent 宿主命令与 `city` 可部署 City 管理命令。 |
...
| `@downcity/type` | 跨 package 共享协议类型，包含 Gate 返回的 City 模型描述等核心类型。 |
...
- Bay 控制面：通过 `bay start` 或 `bay console` 托管本机 Agent，并进入浏览器控制面。
- Agent 生命周期管理：创建、启动、停止、重启、诊断、对话、查看历史。
- 全局模型池：通过 `bay model` 管理 provider 与 model，并把项目绑定到模型 ID。
...
`bay` 负责本机 Agent 宿主，`city` 负责可部署 City 管理。使用 `bay update` 更新全局 CLI。
...
`bay` 会把 `downcity.json.execution.modelId` 解析成运行时 `LanguageModel`，再注入到 Agent session 中。
...
- 不要把真实密钥提交到仓库；优先使用本地环境变量或 `bay env`。
