...
This page explains how a Telegram message is translated into Downcity instructions, how `AgentRuntime` executes them, and how replies are delivered via the `chat_send` tool.

> Code references: `packages/downcity/src/adapters/telegram/bot.ts`, `packages/downcity/src/adapters/base-chat-adapter.ts`, `packages/downcity/src/runtime/agent/runtime.ts`, `packages/downcity/src/runtime/chat/*`
...
```text
<file type="document" caption="report.pdf">.ship/.cache/telegram/1738...-report.pdf</file>
...
Runtime only tries `reply_to_message_id` when reply mode is explicitly enabled (for example `city chat send --reply`).
