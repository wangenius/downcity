...
3. When the host is `city`, Console-global env is read from `downcity.db` and passed explicitly into `Agent`.
...
So key precedence for runtime metadata is: `DC_CTX_*` / `DC_SESSION_ID` > project `.env` > host-injected `env`.
