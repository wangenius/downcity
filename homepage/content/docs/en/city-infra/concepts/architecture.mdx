...
```txt
Interfaces
  -> products enter InfraRuntime through client or city terminal
Composition
  -> server / worker assemble InfraRuntime, official services, and models
Kernel
  -> InfraRuntime handles routing, auth, context, and hook scheduling
Capabilities
...
product UI / app / internal tool
  -> UserClient / AdminClient / city terminal
  -> InfraRuntime
  -> Service / AIService
  -> Provider / Database
```

Many clients can share one self-deployed InfraRuntime. Product clients stay lightweight and focus on UX; InfraRuntime centralizes auth, model routing, runtime env, hooks, and reusable capabilities.

## 1. Interfaces: who calls InfraRuntime

Downcity has two main external entry points:

- Product-side entry: `UserClient` and `AdminClient` from `@downcity/conduit`
- Operator entry: `@downcity/city`

Neither one implements the InfraRuntime itself. They both send requests into the same InfraRuntime.

## 2. Composition: who assembles InfraRuntime

Running Downcity is not just `new InfraRuntime(...)`. There is also an assembly layer:

- `blocks/node` is the Node.js + SQLite server example
- `blocks/edge` is the Cloudflare Workers + D1 deployment example
- `blocks/shared` reuses the common InfraRuntime assembly logic between server and worker

This layer is responsible for assembling `InfraRuntime + AIService + official services + models` into a runnable instance.

## 3. Kernel: what InfraRuntime itself owns

When an HTTP request reaches InfraRuntime, InfraRuntime does the following:

1. Refresh the runtime env view
2. Validate the `user_token` or admin key
3. Resolve identity, `product_id`, and `user_id`
4. Find the target Service and Action
5. Build one shared `ctx`
6. Run hooks and the Action

If the token is invalid, expired, or the `product_id` does not match, InfraRuntime returns 401 or 403 directly.
...
InfraRuntime automatically creates a route for every Action:

```txt
POST /v1/translate/zh2en  ->  translate.action("zh2en")
```

InfraRuntime itself does not care what “translation” or “payment” means as business concepts. It only routes the request to the right Action.
...
Used by pi-agent, the OpenAI SDK, curl, and other third-party tools:
