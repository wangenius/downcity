...
- AI service calls such as `city.ai.text()`, `city.ai.stream()`, and `city.ai.image_create()` / `city.ai.image_result()`
...
- you can pass it explicitly into `city.ai.text({ model })`
- every AIService call must pass an explicit model id or `CityModel`
