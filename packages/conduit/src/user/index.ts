...
export class UserClient {
  ...
  constructor(options: UserClientOptions) {
    ...
    this.product_id = readOptional(options.product_id);
  ...
  private buildInput(input: UserServiceInput): Record<string, unknown> {
    return {
      ...
      product_id: this.requireProductId(),
    };
  }

  private requireProductId(): string {
    if (!this.product_id) throw new TypeError("product_id is required for AI calls");
