...
- `Authorized redirect URI` 填成了产品前端地址，而不是 InfraRuntime 的 `/v1/accounts/oauth/callback`
...
Google 登录完成后，accounts service 仍然会按统一方式返回 InfraRuntime `user_token`。
...
- 想看 OAuth 整体链路，继续读 [OAuth 与 Session](/docs/city-infra/packages/services/accounts/oauth-and-session)
- 如果你还要接微信网站登录，继续读 [微信登录配置](/docs/city-infra/packages/services/accounts/wechat-login)
- 想看完整接入流程，读 [Accounts 服务指南](/docs/city-infra/guides/accounts-service)
- 部署后记得通过 City Terminal / Admin API 把 `GOOGLE_CLIENT_ID` 和 `GOOGLE_CLIENT_SECRET` 写进 InfraRuntime env
