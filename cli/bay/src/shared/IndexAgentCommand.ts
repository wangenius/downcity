 ...
 *
 * 关键点（中文）
 * - 统一承载 `bay agent` 命令树，避免主入口继续混合 console 与 agent 两套语义。
...
export function registerAgentCommands(
  ...
  const agent = program
    ...
    .version(`bay ${context.version} (agent ${context.agentVersion})`, "-v, --version")
  ...
  agent
    .command("list")
    .description("列出已登记到 Bay 的 Agent 项目")
        ...
        if (options.fix !== true) {
          emitCliBlock({
            ...
            facts: [
              {
                label: "Command",
                value: "bay agent doctor <path> --fix",
              },
            ],
          });
          return;
