...
export function format_header(state) {
    return [
...
export function format_city_item_label(item) {
    if (is_disabled_item(item)) {
        return `── ${item.title} ──`;
    }
    return item.title;
}
export function format_city_detail(item) {
    if (!item) {
...
export function format_footer(item) {
    const base = t({
...
export function build_city_subtitle(membership, balance) {
...
export function format_membership_detail(membership) {
    return t({
        zh: [
...
export function format_federation_list_detail(servers) {
    return [
...
export function format_login_detail(membership) {
    return t({
        zh: [
            "{bold}登录{/bold}",
...
export function format_balance_detail(account) {
    return [
...
export function format_current_user_detail(user) {
    return [
...
export function format_recharge_result(result) {
...
export function format_error_detail(title, message) {
    return [
        `{red-fg}{bold}${title}{/bold}{/red-fg}`,
        message || t({ zh: "未知错误", en: "Unknown error" }),
    ].join("\n");
}
export function loading_text(message) {
    return `{yellow-fg}${message}...{/yellow-fg}`;
}
export function format_locale_description(cli_locale) {
    if (cli_locale === "zh") {
        return t({
            zh: "当前默认语言：中文",
...
//# sourceMappingURL=FederationManagerFormat.js.map