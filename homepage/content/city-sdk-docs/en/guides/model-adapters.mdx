...
Use `stream_ai_sdk_model()` for an AI SDK model. For a private HTTP protocol, implement `stream()` directly and return `LanguageModelV3StreamResult`. AIService does not know the vendor request format.
