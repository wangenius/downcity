...
If you previously enabled persisted public mode with [`city public`](/en/docs/commands/public), plain `city start` also reuses that saved Console binding unless you override it explicitly.
...
- `city public on` makes that public Console behavior persist for future plain `city start` runs.
...
- `5314`: city runtime API port for `health`, managed plugin, plugin, and related endpoints.
