...
Federation generates an Ed25519 Key Ring on first startup and publishes verification keys through `/.well-known/jwks.json`. Product backends use `Bureau`, configured with a required `bureau_token`, to identify users online. Federation resolves `city_id` from the Bureau Token record instead of accepting it from the backend.
