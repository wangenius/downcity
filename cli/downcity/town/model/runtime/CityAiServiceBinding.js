...
function readSavedCityConfig() {
    if (!fs.existsSync(CONFIG_FILE_PATH))
        return {};
    try {
        const raw = JSON.parse(fs.readFileSync(CONFIG_FILE_PATH, "utf8"));
 ...
 */
export function readTownCityAiServiceConfig(env = process.env, options) {
    const saved = readSavedCityConfig();
    const city_url = normalizeBaseUrl(readString(env.DOWNCITY_CITY_URL)
    ...
    const admin_secret_key = readString(env.DOWNCITY_CITY_ADMIN_SECRET_KEY)
        ...
        || undefined;
    if (!city_url) {
        throw new Error("City URL is required. Set DOWNCITY_CITY_URL or configure an active server with `city` CLI.");
    }
    if (options?.requireUserToken !== false && !user_token) {
        throw new Error("City user_token is required. Set DOWNCITY_CITY_USER_TOKEN before starting Town agents.");
    }
    return {
        city_url,
        town_id,
 ...
 * 读取管理端模型目录。
 */
export async function listCityAiServiceModelsForAdmin(env = process.env) {
    const config = readTownCityAiServiceConfig(env, { requireUserToken: false });
    if (!config.admin_secret_key) {
        throw new Error("City admin_secret_key is required to list models. Set DOWNCITY_CITY_ADMIN_SECRET_KEY or configure the city CLI server.");
    }
    const city = new City({
        role: "admin",
 ...
 * 读取用户态可调用模型目录。
 */
export async function listCityAiServiceModelsForUser(env = process.env) {
 ...
 */
export function toCityAiModelChoices(models) {
 ...
 */
export async function listCityAiModelChoices(env = process.env) {
 ...
 */
export async function assertCityAiModelReady(modelId, env = process.env) {
...
//# sourceMappingURL=CityAiServiceBinding.js.map