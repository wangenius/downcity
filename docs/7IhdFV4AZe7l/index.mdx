---
title: Memory V2 重设计（完全替换版）
id: 7IhdFV4AZe7l
source: local_dir
rel_path: 7IhdFV4AZe7l
kind: dir
description: 
---
...
2. 改为两类触发：近 compaction 阈值触发 flush（优先保障不丢失）。可配置的轻量增量提取（只处理新片段，异步执行）。
...
1. search 失败返回：success: falseerrordisabled: trueaction（建议修复动作）
...
2. 抽取规则：稳定事实 -> .ship/memory/MEMORY.md时间性记录 -> .ship/memory/daily/YYYY-MM-DD.md
3. 迁移备份：.ship/memory/migration-backup/<contextId>-Primary.md
