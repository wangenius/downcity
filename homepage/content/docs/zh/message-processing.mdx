...
## 0.1) 默认 system（PROFILE.md + SOUL.md / USER.md（可选）+ DefaultPrompt）

从 v2 开始，AgentRuntime 的 system message 由两部分拼成：

- `PROFILE.md`：项目/仓库的长期指令（人类写给 agent 的规则与偏好）
- `SOUL.md`（可选）：稳定的执行原则（工作风格、风险边界、质量标准）
- `USER.md`（可选）：稳定的用户偏好（沟通方式、输出格式、优先级）
...
当前实现的“简化版运行逻辑（system = PROFILE.md + SOUL.md / USER.md（可选） + DefaultPrompt、无 simulation、按需加载历史）”说明见：`docs/runtime-simplified.md`。
