...
### 1. SessionRuntime 是前台执行者

SessionRuntime 负责一轮真实交互：
...
也就是说，SessionRuntime 不负责“拥有 memory”，但它负责“驱动 memory”。
...
sequenceDiagram
  ...
  User->>SessionRuntime: 新消息
  SessionRuntime->>History: append user message
  SessionRuntime->>Memory: search relevant memory
  Memory-->>SessionRuntime: memory snippets
  SessionRuntime->>LLM: build context and run
  LLM-->>SessionRuntime: assistant output
  SessionRuntime->>History: append assistant message
  SessionRuntime->>Memory: optional store / update
```
