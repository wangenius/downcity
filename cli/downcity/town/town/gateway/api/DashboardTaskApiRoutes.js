 ...
 */
export function registerDashboardTaskApiRoutes(params) {
    const { app } = params;
...
async function readTaskRunningState(params) {
    const task_id = resolveTaskIdFromTaskMdPath(params.task.taskMdPath) ||
        deriveTaskIdFromTitle(String(params.task.title || ""));
    const timestamp = String(params.task.lastRunTimestamp || "").trim();
    if (!task_id || !timestamp || !TASK_RUN_DIR_REGEX.test(timestamp))
        return false;
    const progress_path = path.join(getDowncityTasksDirPath(params.project_root), task_id, timestamp, "run-progress.json");
    const progress = (await fs.readJson(progress_path).catch(() => null));
...
async function deleteTaskRun(params) {
    ...
    if (!(await fs.pathExists(run_dir)))
        return false;
    const progress = (await fs.readJson(path.join(run_dir, "run-progress.json")).catch(() => null));
...
async function clearTaskRuns(params) {
    ...
    const timestamps = entries
        ...
        .map((item) => item.name)
        .sort();
    const deleted_timestamps = [];
    const skipped_running_timestamps = [];
...
function resolveTaskDir(project_root, title) {
...
//# sourceMappingURL=DashboardTaskApiRoutes.js.map