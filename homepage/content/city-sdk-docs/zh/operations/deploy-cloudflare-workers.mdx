...
如果你只是想理解 City 怎么接入 Workers 和 D1，先读 [Cloudflare Workers](/zh/city-sdk-docs/guides/cloudflare-workers)。这一页只讲上线步骤：`city.json`、本地部署、Git URL 部署、D1、admin key、env、OAuth callback 和健康检查。
...
如果 `city.json` 的 `target` 是 `cloudflare-workers`，`city deploy` 会自动检查 Cloudflare 登录态。没有登录时会唤起 Wrangler 登录；已经登录但 Cloudflare 无法自动返回 account 时，会提示你刷新登录，或输入一次 Cloudflare account id。输入的 account id 会保存到 `.city/deploy.json`，后续部署会自动复用。
...
```bash
city deploy https://github.com/example/my-city.git
```

Git URL 会被 clone 到临时目录，然后按同一套 `city.json` 流程部署。
...
DOWNCITY_WORKER_URL=https://my-city.<your-subdomain>.workers.dev
```
...
- 对 `cloudflare-workers` target 准备 D1，并在 `.city/deploy.json` 里没有 database id 时创建 D1
- 根据 `city.json` 和 `.city/deploy.json` 同步 `wrangler.toml`
- 执行 `wrangler deploy`
...
如果 `.env` 里配置了 `DOWNCITY_WORKER_URL`，可以只执行健康检查：
