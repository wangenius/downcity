...
When you expose Federation from your own local HTTP server, products still use normal HTTP. If `AccountsService` is configured with `local_login: true`, `/v1/accounts/providers` exposes the local login method, and `accounts/local/login` returns a normal `user_token`.
...
|---|---|
| Product code calls Fed AIService directly | `client.ai.text()` / `client.ai.stream()` |
| Downcity Agent uses a Fed model | get a `CityModel` from `client.ai.listModels()` and pass it to Agent |
...
The OpenAI-compatible provider still calls Federation. Federation reads
`body.model`, finds the provider bound to that model, and lets that provider
call the real upstream model. The frontend only carries `user_token`; `city_id`
is resolved from the token on the server.
