...
`TaskPlugin` can be used with no options for task management and script tasks. Agent tasks also require the host to provide a runtime model resolver. By default, cron uses the current machine timezone:
...
- `resolve_session_model` returns a runtime model instance; TaskPlugin never reads model IDs from AgentContext
