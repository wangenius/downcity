 ...
 * City 初始化后的内部状态。
 */
export interface CityInitState {
  /** 初始化后的 database */
  database: Database;
  /** 初始化后的底层 client */
 ...
 * 执行 City 初始化。
 */
export async function initialize_city(params: {
  /** runtime 能力 */
  runtime: Runtime;
  /** 已注册服务 */
  services: Service[];
  /** City ready 回调 */
  require_ready: () => Promise<{ studio: { get(id: string): Promise<{ studio_id: string; status: string } | undefined> } }>;
  ...
  table_map.set("studios", new TableApi(database, builtinTables.studios));
  ...
  if (!env_table) throw new Error("City env table is not initialized");
  const env_store = new EnvStore(env_table as CityTableApi<EnvEntry>);
  await env.attachStore(env_store);

  const studios_table = table_map.get("studios");
  if (!studios_table) throw new Error("City studios table is not initialized");
  const studio_store = new StudioStore(studios_table as CityTableApi<Studio>);
  ...
  for (const service of services) {
    ...
    service._env = env;
    service._studioStore = studio_store;
...
/**
 * 确保 City 系统级 env key 存在。
 *
 * 关键说明（中文）
 * - 系统密钥也统一走 City env 表托管
...
async function bootstrap_default_keys(
  ...
  const admin_key = env.get("DOWNCITY_CITY_ADMIN_SECRET_KEY") || `admin_${randomSecret()}`;
  if (!env.get("DOWNCITY_CITY_ADMIN_SECRET_KEY")) {
    await env.upsert({ key: "DOWNCITY_CITY_ADMIN_SECRET_KEY", value: admin_key });
  }

  const token_key = env.get("DOWNCITY_CITY_TOKEN_SIGNING_KEY") || `sign_${randomSecret()}`;
  if (!env.get("DOWNCITY_CITY_TOKEN_SIGNING_KEY")) {
    await env.upsert({ key: "DOWNCITY_CITY_TOKEN_SIGNING_KEY", value: token_key });
  }
