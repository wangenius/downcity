...
/**
 * 递归按侧边栏顺序收集 Products Docs 文档 URL，用于目录页跳转。
 */
...
function findFirstChildDocUrl(lang: "en" | "zh", slugs: string[]) {
  ...
  const tree = productsDocsSource.getPageTree(lang) as PageTreeNode | undefined;
  if (!tree) return undefined;

  const urls: string[] = [];
  collectPageUrls(tree, urls);

  const prefix = `/${lang}/products-docs/${slugs.join("/")}/`;
...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  const page = productsDocsSource.getPage(cleanSlugs, lang);
  if (!page) {
  ...
  return {
    path: page.path,
    title: page.data.title ?? "Products Docs",
...
export function meta({ loaderData }: Route.MetaArgs) {
  ...
  const title = `${loaderData.title} — Downcity Products Docs`;
  ...
  return [
    ...
    { name: "keywords", content: "Downcity, products, town cli, town console, chrome extension" },
...
const clientLoader = browserCollections.productsDocs.createClientLoader({
  id: "productsDocs",
  component: ({
    default: Mdx,
    frontmatter,
