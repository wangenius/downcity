 ...
 *
 * 关键点（中文）
 * - `bay plugin` 提供 console 侧静态 plugin catalog 入口。
 * - `list/status` 不依赖 agent，只展示内建 plugin 定义与 Bay 配置事实。
...
function listStaticCatalogEntries(): StaticCatalogEntry[] {
  ...
  const managedEntries = listPluginsWithLifecycle(plugins).map((plugin) => ({
    ...
    note: "Managed plugin. Use `bay plugin start/stop/restart/status` with an agent target for live state.",
  }));
...
async function promptPluginManagerAction(params: {
  ...
  const response = (await prompts({
    ...
    choices: [
      ...
      {
        title: "全局启用",
        description: availability.enabled ? "当前已启用" : "写入 Bay 级 lifecycle 配置",
        value: "enable",
      },
      {
        title: "全局关闭",
...
async function resolveInteractivePluginName(params: {
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    emitCliBlock({
      ...
      note: "Use `bay plugin info <pluginName>` or run this command in an interactive terminal.",
    });
    return null;
  }
