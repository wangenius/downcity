 ...
 *
 * 关键点（中文）
 * - 裸 `bay chat` 进入 chat plugin 管理，而不是只输出静态 help。
 * - chat channel account 属于 Bay 级配置，在这里通过“配置 channel”管理。
...
async function promptRootAction(): Promise<ChatManagerRootAction | null> {
  ...
  const response = (await prompts({
    ...
    choices: [
      ...
      {
        title: "配置 channel",
        description: `${accounts.items.length} 个 Bay 级 channel account`,
...
async function emitChannelAccountList(): Promise<void> {
  ...
  if (items.length === 0) {
    emitCliBlock({
      ...
      note: "在 `bay chat` 中选择“配置 channel”后新增 Telegram、Feishu 或 QQ account。",
    });
    return;
  }
