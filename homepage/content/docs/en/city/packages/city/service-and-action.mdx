...
- you want studio business actions, not raw model inference only
...
That is why the studio side can stay unified:
...
### Scenario 1: studio business action
...
From the studio side, both still go through `client.service("service_id").action("action_id").invoke(...)`.
...
- For studio-side calling, read [@downcity/city](/en/docs/city/packages/city/gate)
