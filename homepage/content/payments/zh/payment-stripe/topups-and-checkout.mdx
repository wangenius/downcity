...
同一个 `topup_id + method_id` 同时收到多个创建请求时，PaymentService 只会向 Provider 创建一次 Checkout。首个请求持有短租约；租约期间的并发请求返回 `409`，前端稍后重试即可。Checkout 已创建后，重复请求会返回同一笔 payment 和 `checkout_url`。

服务会把稳定的 `payment_id` 作为 Stripe 幂等键。即使创建请求在网络返回前中断，后续接管也不会生成第二笔 Stripe Checkout。
