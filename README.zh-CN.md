...
- 平台与执行内核解耦：`downcity` 负责平台与宿主管理，`@downcity/agent` 负责单 Agent 运行时。
...
| 包 / 目录 | 作用 |
| --- | --- |
| `downcity` | 平台层与 CLI，负责 city runtime、Console gateway、多 Agent 注册、模型池、全局配置与 daemon 管理。 |
| `@downcity/agent` | 单 Agent runtime 与 SDK，负责 session、tool loop、service、plugin、HTTP/RPC、sandbox 与宿主接入。 |
| `@downcity/city` | City 运行时，负责 service 注册、action、auth、env、studio 访问边界与 HTTP 路由。 |
...
| `cities/*` | 可部署 City 组合，用于把 `@downcity/city` 与 services 组装成 Node 或 Edge 运行形态。 |
...
- City runtime：通过 `@downcity/city`、`@downcity/services`、`@downcity/gate` 和可部署的 `cities/*`，让多个 studio 复用同一座服务城市。
...
`studio` 负责本机 Agent 宿主，`city` 负责 City 管理。使用 `city update` 更新全局 CLI。
...
- 包文档：[packages/agent/README.md](./packages/agent/README.md)、[packages/city/README.md](./packages/city/README.md)、[packages/services/README.md](./packages/services/README.md)、[packages/gate/README.md](./packages/gate/README.md)、[cli/city/README.md](./cli/city/README.md)、[cli/studio/README.md](./cli/studio/README.md)、[cli/downcity/README.md](./cli/downcity/README.md)、[packages/ui/README.md](./packages/ui/README.md)
