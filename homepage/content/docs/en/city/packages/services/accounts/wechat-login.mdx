...
If the callback domain configured in WeChat does not match the real public City domain used by the studio, login usually fails immediately.

So two things must stay aligned:

- the real public City domain used by the studio
...
So the studio does not need a special post-login client flow just because the user came from WeChat:
...
- configuring one domain in WeChat while the studio actually uses another City domain
...
- After deployment, write `WECHAT_CLIENT_ID` and `WECHAT_CLIENT_SECRET` into City env through Studio CLI or the Admin API
