...
| `tool` | `tool_call_id`, `tool_name`, `state`, `approval`, metadata | 工具输入、审批、运行和结果 |
...
每个 Assistant Part 有稳定的 `part_id`。`sequence` 表示当前 Message revision 中的线性顺序；如果最终 AI SDK 快照补充了流式阶段未出现的 Part，Assistant 完成前的最终完整快照可能校正 `sequence`。Message 完成后，canonical parts 顺序保持稳定。工具状态变化更新同一个 Part，不应在 UI 中创建“请求”和“结果”两个重复行。
...
AI SDK 原生审批保存 `approval_id / approved / reason`。Downcity 本地 Shell 审批还会把命令、工作目录和过期时间放在 `approval.request` 中。`approval-required -> running/completed` 与拒绝后的 `failed` 都更新同一个 Tool Part。

## 顺序、版本与可见性

每条 Message 都有稳定 `message_id`、创建后不变的 `sequence`，以及随完整快照更新递增的 `revision`。客户端不能用较低 revision 覆盖较高 revision。

Assistant 完成前，SDK 会用最终 UIMessage 原子收敛完整 parts。流式阶段缺失的工具会按最终相对位置补齐，不会被简单追加到正文之后。canonical part 写入失败会使 Turn 失败，不会发布一个历史不完整的 completed Turn。客户端收到更高 revision 的完整 `message` Mutation 时，应替换整条 Message，而不是保留旧 revision 的 parts 数组。
