...
import { collect_infra_env_catalog } from "./base-env-catalog.js";
...
/**
 * 构建 InfraRuntime 对外路由。
 */
export function build_infra_router(params: {
  /** runtime 能力 */
  runtime: Runtime;
  /** 已注册服务 */
  services: Service[];
  ...
  app.use("/v1/*", async (c, next) => {
    // 关键说明（中文）
    // InfraRuntime env 支持运行时动态更新，因此每次请求前先 refresh，
    ...
    if (result.product) c.set("product", result.product);
    await next();
  });
  ...
  app.get("/v1/env/catalog", async (c) => {
    try {
      ...
      return c.json({ items: collect_infra_env_catalog(services, runtime.env) });
    } catch (error) {
      return build_error_response(error);
    }
  });

  app.get("/v1/infra/instruction", async (c) => {
    try {
      authenticator.authorize({
        level: c.get("identity")?.kind ?? "guest",
        user: c.get("user"),
      ...
      return new Response(await build_infra_instruction(services), {
        status: 200,
        headers: {
  ...
  for (const service of services) {
    for (const def of service._listActionDefs()) {
      ...
      register(path, async (c) => {
        ...
        const db: Record<string, InfraTableApi> = {};
        if (service.tables) {
        ...
        try {
          ...
          ensure_product_identity_match(ctx);
...
/**
 * 校验 input.product_id 与 token 绑定产品一致。
 */
function ensure_product_identity_match(ctx: Context): void {
  if (ctx.identity?.kind !== "user") return;
  const request_product_id = typeof ctx.input.product_id === "string"
    ? ctx.input.product_id.trim()
    : "";
  if (!request_product_id) return;

  const token_product_id = ctx.product?.product_id ?? "";
  if (request_product_id !== token_product_id) {
    throw httpError(403, "product_id does not match the authenticated token");
  }
}
