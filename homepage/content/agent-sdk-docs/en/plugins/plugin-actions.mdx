...
When an action is called by a Session tool, its parameters also include the explicit `run_context` for that Session turn. It may be absent for direct CLI, HTTP, or scheduler calls. Use `run_context.agentEnv` and `run_context.agentSystems` for the effective step configuration; `context.env` and `context.systems` remain the Agent-level configured values.
