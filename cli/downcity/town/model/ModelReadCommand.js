...
import { parseBooleanOption, runStoreCommand, toSafeProviderView, } from "./ModelCommandShared.js";
/**
 * 注册 `list/get/discover/use` 命令。
 */
export function registerModelReadCommands(model) {
    registerListCommand(model);
    registerGetCommands(model);
    registerDiscoverCommand(model);
    registerUseCommand(model);
}
function registerListCommand(model) {
    model
        .command("list")
...
function registerGetCommands(model) {
    const get = model
        .command("get")
...
function registerDiscoverCommand(model) {
    model
...
function registerUseCommand(model) {
    model
        .command("use <modelId>")
        ...
        try {
            ...
            try {
                ...
                if (!exists)
                    throw new Error(`Model not found in platform pool: ${id}`);
            }
            finally {
                store.close();
            }
...
//# sourceMappingURL=ModelReadCommand.js.map