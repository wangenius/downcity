...
可以在 `studio chat` 的交互式菜单中选择“配置 channel → 配置 authorization”，也可以直接使用 CLI：
...
聊天用户被拒绝时，返回消息会包含类似 `studio chat auth set telegram:12345678` 的命令，用户可以直接转发给管理员。
...
- [Console](/zh/docs/commands/console/console)
