...
这种模式很常见，因为很多团队并不想把所有商业规则都写死在 InfraRuntime 内部。
...
### 不是所有计费逻辑都应该放在 InfraRuntime 里

很多团队会把 InfraRuntime 作为事实层和权限层，把更上层的财务 / 商业逻辑放在自己的系统里。

## 继续阅读

- 想理解 usage 记录了什么，读 [Usage 事实模型](/docs/city-infra/packages/services/usage/event-model)
- 想看读取入口，读 [查询与汇总](/docs/city-infra/packages/services/usage/querying-and-summary)
- 想看 Stripe 充值怎么同步到账，读 [@downcity/services](/docs/city-infra/packages/services)
