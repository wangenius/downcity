...
- 平台与执行内核解耦：`@downcity/city` 负责平台与宿主管理，`@downcity/agent` 负责单 Agent 运行时。
...
| 包 / 目录 | 作用 |
| --- | --- |
| `@downcity/city` | 平台层与 CLI，负责 city runtime、Console gateway、多 Agent 注册、模型池、全局配置与 daemon 管理。 |
| `@downcity/agent` | 单 Agent runtime 与 SDK，负责 session、tool loop、service、plugin、HTTP/RPC、sandbox 与宿主接入。 |
| `@downcity/infra` | 城市基础设施运行时，负责 service 注册、action、auth、env、product 访问边界与 HTTP 路由。 |
| `@downcity/services` | 城市公共服务，负责 accounts、balance、usage、payment 与 Stripe 支付闭环。 |
| `@downcity/conduit` | 面向用户端与管理端的 HTTP 调用 SDK。 |
| `@downcity/ui` | React + Tailwind UI SDK，提供 Console 与宿主应用可复用的界面组件。 |
| `blocks/*` | 可部署街区，用于把 infra 与 services 组装成 Node 或 Edge 运行形态。 |
...
- City 控制面：通过 `city start` 或 `city console` 管理运行时和多个 Agent。
- Agent 生命周期管理：创建、启动、停止、重启、诊断、对话、查看历史。
- 全局模型池：通过 `city model` 管理 provider 与 model，并把项目绑定到模型 ID。
- City Infra：通过 `@downcity/infra`、`@downcity/services`、`@downcity/conduit` 和可部署的 `blocks/*`，让多个产品复用同一套服务基建。
...
`@downcity/city` 与 `downcity` 发布的是同一份 CLI 内容。`city update` 会按你当前全局安装来源更新对应的包。
...
`city` 会把 `downcity.json.execution.modelId` 解析成运行时 `LanguageModel`，再注入到 Agent session 中。
...
- City Infra 文档：[downcity.ai/docs/city-infra](https://downcity.ai/docs/city-infra)
...
- 包文档：[packages/agent/README.md](./packages/agent/README.md)、[packages/infra/README.md](./packages/infra/README.md)、[packages/services/README.md](./packages/services/README.md)、[packages/conduit/README.md](./packages/conduit/README.md)、[packages/city/README.md](./packages/city/README.md)、[packages/ui/README.md](./packages/ui/README.md)
...
- 不要把真实密钥提交到仓库；优先使用本地环境变量或 `city env`。
