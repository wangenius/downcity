---
title: InfraRuntime
description: InfraRuntime 是运行时容器，管理 Service 生命周期、鉴权和 HTTP 路由。
---

`InfraRuntime` 是 Downcity 的核心实例。封装了 HTTP 路由、鉴权、环境变量和数据库基础设施。
...
InfraRuntime 自动为每个 Action 生成 HTTP 路由：
...
`InfraRuntime` 可以直接返回当前实例的说明文档字符串：
...
InfraRuntime 自动处理 `user_token` 和 `admin_secret_key` 的校验。Action 默认要求 `user_token`；传 `auth: ["admin"]` 时只允许管理端访问，传 `auth: []` 时表示免登录。

详见 [信任边界](/docs/city-infra/concepts/trust-boundary)。
