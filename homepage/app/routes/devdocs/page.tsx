...
function findFirstChildDocUrl(lang: "en" | "zh", slugs: string[]) {
  ...
  const tree = devSource.getPageTree(lang) as PageTreeNode | undefined;
  if (!tree) return undefined;

  const urls: string[] = [];
  collectPageUrls(tree, urls);

  const prefix = `/${lang}/devdocs/${slugs.join("/")}/`;
...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  const page = devSource.getPage(cleanSlugs, lang);
  if (!page) {
  ...
  return {
    path: page.path,
    title: page.data.title ?? "Developer Documentation",
...
export function meta({ loaderData }: Route.MetaArgs) {
  ...
  const title = `${loaderData.title} — Downcity Devdocs`;
  const description =
    loaderData.description || "Downcity developer documentation";
  ...
  return [
    ...
    {
      name: "keywords",
      content: "Downcity, devdocs, architecture, console-ui, chrome extension, runtime",
    },
    {
      property: "og:title",
      content: title,
...
const clientLoader = browserCollections.devdocs.createClientLoader({
  id: "devdocs",
  component: ({
    default: Mdx,
    frontmatter,
