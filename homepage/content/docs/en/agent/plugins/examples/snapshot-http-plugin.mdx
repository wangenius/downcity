...
- `plugin.http.runtime` is not the same kind of explicit call as `agent.plugins.runAction(...)`
- the current minimal SDK `agent.http.start()` only exposes session APIs and does not auto-mount plugin HTTP routes
