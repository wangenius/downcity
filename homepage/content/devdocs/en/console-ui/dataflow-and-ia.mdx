...
The real value of `console-ui/` is not only how the screens look. It is how it separates console-level, agent-level, and context-level information.
...
That is why `useDashboardRefresh.ts` first loads `agents`, then `contexts`, then chooses the next `contextId`.

### Switching context

When only the context changes, the UI does not need to reload the whole world. It only needs to refresh:
