...
The current Studio runtime endpoint inherited by nested `studio` invocations.
...
Current Studio runtime uses the same resolved id value that is passed into `new Agent({ id })`.
