...
- file attachment delivery via `@attach document <path>`
- inbound image, video, file, and audio download with `@attach` injection into Agent execution
...
From this version on, Feishu ownership should no longer be modeled as only one `FEISHU_AUTH_ID`.

Meaning:

- `ownerIds`: who should be treated as `is_master=yes`
- `dmPolicy`: whether DMs are open, pairing-based, allowlist-based, or disabled
- `allowFrom`: DM allowlist
- `groupPolicy`: whether groups are open or restricted
- `groupAllowFrom`: allowed group ids

The same config is now stored in console `~/.ship/ship.db` and managed in Console UI under `Agent / Authorization`, instead of being written to project `ship.json` in plaintext.
...
If name resolution works, the Feishu route in `.ship/channel/meta.json` should contain:
...
- execution accepts **text + image + video + file + audio** inputs
- inbound image, video, file, and audio messages are downloaded into `.ship/.cache/feishu/` and injected as `@attach ...`
- outbound replies support `@attach document|file <path>` file conversion
