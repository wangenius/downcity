...
/**
 * 从 InfraRuntimeOptions 创建 runtime。
 */
export function create_runtime_from_db(options: InfraRuntimeOptions): Runtime {
...
/**
 * 推断 InfraRuntime 内置表定义。
 */
function builtin_tables_for(dialect: "pg" | "sqlite"): BuiltinTables {
  return dialect === "pg"
    ? { products: pgProducts, env: pgEnv }
    : { products: sqliteProducts, env: sqliteEnv };
}
...
function infer_dialect(db: unknown): "pg" | "sqlite" {
  ...
  throw new Error("Unable to infer Drizzle dialect. Pass { dialect: \"pg\" } or { dialect: \"sqlite\" } to InfraRuntime.");
}
...
function extract_db_client(db: { $client?: unknown }): DbClient {
  ...
  throw new Error("Drizzle db must expose $client so InfraRuntime can initialize tables.");
}
 ...
 *
 * 关键说明（中文）
 * - InfraRuntime 把所有系统与业务 env 统一托管到 env 表
