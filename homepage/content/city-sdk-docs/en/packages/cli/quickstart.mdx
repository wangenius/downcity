...
Federation URLs, active state, admin keys, local PIDs, ports, logs, and deployment status are stored in the system-level Federation registry and do not depend on the current directory. `fed query` requests the active Federation. It adds the saved `admin_secret_key` when available and otherwise sends a public request.
