...
两者都有 `prompt()`、`stop()`、`compact()`、`subscribe()`、`messages()`、`approvals()` 和 `fork()`。本地 Session 还提供 `snapshot()`，用于显式固化当前生效的自定义 instruction；远程 Session 的持久化策略由服务端宿主管理。

| 场景 | 配置模型 |
| --- | --- |
| 本地 `AgentSession` | `session.set({ model })`，接收 `AgentModel` 实例 |
| `RemoteAgentSession` | 不提供模型配置方法；远程客户端只执行 Session |
...
- instruction 持久化：[`session.snapshot()`](/zh/agent-sdk-docs/sessions/snapshot)
