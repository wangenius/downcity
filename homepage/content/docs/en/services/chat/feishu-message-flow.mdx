...
1. send plain text body (with `<file>` tags removed)
2. upload local file and send Feishu `file` message
3. if the tag includes `caption`, send caption as an extra text message
...
- outbound `post` currently merges text, links, and images only; files/audio/video are still sent as follow-up standalone messages
