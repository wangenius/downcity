...
description: Detailed explanation of the local Agent id, path, instruction, model, env, modelCatalog, configureSession, tools, mode, and plugins options
---
...
An optional host-provided model catalog capability.
...
- this is a read-only host port and does not create `LanguageModel` instances
- it is mainly used for initialization, startup validation, control-plane display, and model switching
- if omitted, normal session execution can still work, but related control-plane capabilities are downgraded

## `configureSession`

A host hook that runs after session initialization.
...
- this is a good place for the host to inject a default model or other runtime settings into each session
- if `model` is also provided, the SDK applies that default `model` first and then runs `configureSession`
...
This controls how built-in plugins are assembled.
...
- `preset` auto-registers the built-in plugin set
- `custom` only uses the explicit `plugins` you pass
