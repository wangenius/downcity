...
| `@downcity/city` | City 运行时与 Gate 访问 SDK，负责 service 注册、action、auth、env、bay 访问边界与 HTTP 调用。 |
...
- City runtime：通过 `@downcity/city`、`@downcity/services` 和可部署的 `cities/*`，让多个 bay 复用同一座服务城市。
