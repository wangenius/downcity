...
`Federation` is the server-side core instance (what the docs call the City runtime). It encapsulates HTTP routing, auth, env vars, and database infrastructure. The client-side entry is [`City`](/en/city-sdk-docs/reference/city-user); do not confuse the two.
