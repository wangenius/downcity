...
CITY_WORKER_URL=https://my-city.<your-subdomain>.workers.dev
DOWNCITY_WORKER_URL=https://my-city.<your-subdomain>.workers.dev
CITY_D1_DATABASE_ID=<created-by-city-deploy>
...
- 对 `cloudflare-workers` target 准备 D1，并在 `.env` 里没有 `CITY_D1_DATABASE_ID` 时创建 D1
- 临时生成 Wrangler 配置，不写入项目目录
- 执行 `wrangler deploy --config <generated-wrangler.toml>`
- 把 Worker URL、D1 database id 等部署结果写回项目 `.env`
...
如果 `.env` 里配置了 `CITY_WORKER_URL` 或 `DOWNCITY_WORKER_URL`，可以只执行健康检查：
