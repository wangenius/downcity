...
function printTokenList(tokens: AuthTokenSummary[], json = false): void {
  ...
  if (tokens.length === 0) {
    emitCliBlock({
      ...
      note: "当前还没有本机 token，可执行 `bay token create <name>`。",
    });
    return;
  }
...
async function promptTokenCreateInput(params?: {
  ...
  if (!isInteractiveTerminal()) {
    emitCliBlock({
      ...
      note: "Use `bay token create <name>` or run this command in an interactive terminal.",
    });
    return null;
  }
...
async function promptTokenIdForDelete(): Promise<string | null> {
  if (!isInteractiveTerminal()) {
    emitCliBlock({
      ...
      note: "Use `bay token delete <tokenId>` or run this command in an interactive terminal.",
    });
    return null;
  }
...
async function runInteractiveTokenCommand(): Promise<void> {
  if (!isInteractiveTerminal()) {
    emitCliBlock({
      tone: "info",
      title: "Token command",
      note: "Use `bay token --help` for scriptable usage, or run `bay token` in an interactive terminal.",
    });
    return;
  }
