...
const DC_VERSION = (() => {
  const candidate_package_paths = [
    // 关键点（中文）：downcity 聚合包复制后的 bay runtime 位于 `<pkg>/bay/control`。
    path.join(__dirname, "../package.json"),
    // 关键点（中文）：workspace 内部包构建后位于 `cli/bay/bin/control`。
    path.join(__dirname, "../../package.json"),
  ];
...
export class ControlGateway {
  ...
  constructor() {
    // 关键点（中文）：Console UI 构建产物随 Bay CLI 一起托管，聚合包会复制到对应 public 目录。
    ...
    this.app.use(
      ...
      cors({
        ...
        allowHeaders: ["Content-Type", "Authorization", "X-Bay-Agent"],
      }),
    );
    this.app.use(
      "*",
