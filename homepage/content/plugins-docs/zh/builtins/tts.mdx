...
普通 CLI 使用直接走 `town tts` 快捷命令：

```bash
town tts status
town tts models
town tts install qwen3-tts-0.6b --active-model qwen3-tts-0.6b --format wav
town tts on qwen3-tts-0.6b --active-model qwen3-tts-0.6b
town tts use qwen3-tts-0.6b
town tts synthesize "Hello, welcome to Downcity" --language en --format wav
town tts doctor
town tts off
```

`configure` 存在于 SDK/plugin action 层，但没有暴露成 `town tts configure` 快捷命令。普通 CLI 调整使用 `town tts on`、`town tts use`，以及 `synthesize` 的单次参数。
...
| `configure` | 更新默认配置 | SDK 或高级 plugin action 调用 |
