...
Both expose `prompt()`, `stop()`, `subscribe()`, `messages()`, `approvals()`, and `fork()`. Model configuration is the important difference:
...
- Control execution: `session.stop()`, `session.set()`, and `session.fork()`
