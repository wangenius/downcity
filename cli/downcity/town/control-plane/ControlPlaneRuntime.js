 ...
 */
export async function readControlPlanePid() {
    try {
        const raw = await fs.readFile(getControlPlanePidPath(), "utf-8");
        const pid = Number.parseInt(String(raw || "").trim(), 10);
        if (!Number.isFinite(pid) || Number.isNaN(pid) || pid < 1)
            return null;
        return pid;
    }
    catch {
 ...
 */
async function readControlPlaneMeta() {
    try {
        const raw = (await fs.readJson(getControlPlaneMetaPath()));
 ...
 */
async function cleanupControlPlaneStateFiles() {
    try {
 ...
 */
export function isControlPlaneBindingMatch(actualHost, expectedHost) {
 ...
 */
function formatControlPlaneRestartHint(options, port) {
    const parts = ["town", "console", "restart"];
    if (options?.public === true)
        parts.push("-p");
    const explicitHost = String(options?.host || "").trim();
    if (explicitHost)
        parts.push("--host", explicitHost);
    if (port !== DEFAULT_CONTROL_PLANE_PORT)
 ...
 */
export function resolveControlPlaneHostForBinding(options) {
    const explicitHost = String(options?.host || "").trim();
    if (explicitHost)
        return explicitHost;
    if (options?.public === true)
        return PUBLIC_CONTROL_PLANE_HOST;
    return DEFAULT_CONTROL_PLANE_HOST;
}
/**
 * 解析 detached Console 命令行中的 host/port。
 */
export function parseControlPlaneProcessCommand(command) {
    const normalized = String(command || "").replace(/\s+/g, " ").trim();
    if (!/\bconsole run\b/.test(normalized))
        return null;
 ...
 */
export function findReusableControlPlaneProcess(processes, expected) {
    ...
    for (const item of processes) {
        ...
        if (parsed.port !== expectedPort)
            continue;
        if (expectedHost && !isControlPlaneBindingMatch(parsed.host, expectedHost))
            continue;
        return {
            pid: item.pid,
            host: parsed.host,
 ...
 */
async function recoverDetachedConsoleStatus(expected) {
    const host = String(expected?.host || "").trim()
        ? normalizeBindHost(String(expected?.host))
        : "";
    const port = typeof expected?.port === "number" && Number.isInteger(expected.port)
        ? expected.port
 ...
 * 获取 Console 当前运行状态。
 */
export async function getControlPlaneRuntimeStatus() {
    ...
    const port = meta && Number.isInteger(meta.port) ? meta.port : DEFAULT_CONTROL_PLANE_PORT;
    return {
        running: true,
        pid,
        host,
 ...
 */
export async function runControlPlaneRuntimeCommand(options) {
...
export async function startControlPlaneCommand(params) {
    ...
    const port = typeof effectiveOptions.port === "number" && Number.isInteger(effectiveOptions.port)
        ? effectiveOptions.port
        : DEFAULT_CONTROL_PLANE_PORT;
    const status = await getControlPlaneRuntimeStatus();
    if (status.running) {
        const statusUrl = String(status.url || "").trim();
        const currentBindHost = normalizeBindHost(status.bindHost || status.host || DEFAULT_CONTROL_PLANE_HOST);
        const currentPort = status.port || DEFAULT_CONTROL_PLANE_PORT;
        const sameEndpoint = isControlPlaneBindingMatch(currentBindHost, host) && currentPort === port;
    ...
    const child = spawn(process.execPath, [params.cliPath, "console", "run", "--host", host, "--port", String(port)], {
        cwd: process.cwd(),
        detached: true,
...
//# sourceMappingURL=ControlPlaneRuntime.js.map