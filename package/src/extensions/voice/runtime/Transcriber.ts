 ...
 *
 * 关键点（中文）
 * - 该函数是 voice extension 对 chat 等 service 暴露的核心能力。
...
export async function transcribeVoiceAudio(
  ...
  const strategy = normalizeStrategy(resolved.config.transcribe?.strategy);
  ...
  const tryRunner = async (
    ...
    try {
      let text = "";
      if (runner === "command") {
        const template = String(resolved.config.transcribe?.command || "").trim();
        if (!template) {
          throw new Error(
            "Voice transcribe strategy=command requires extensions.voice.transcribe.command",
          );
        }
  ...
  } else if (strategy === "funasr") {
    const result = await tryRunner("funasr");
    if (result) return result;
  } else if (strategy === "transformers-whisper") {
