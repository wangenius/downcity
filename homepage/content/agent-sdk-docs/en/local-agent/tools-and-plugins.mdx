...
This mounts `shell_exec` and `shell_session`. Use `shell_exec` for one-shot non-interactive commands, and `shell_session` for long-running or stdin-driven interactive PTY sessions.
