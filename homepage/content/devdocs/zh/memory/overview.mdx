...
1. 为什么一定要有 Memory
2. Memory 到底在系统里管什么
3. 它和 `ContextAgent`、`memory service`、`memoryAgent`、`LLM` 分别是什么关系
...
- Downcity 当前真正的主执行体在代码里叫 `ContextAgent`
- 但从语义上更接近 `SessionAgent`
...
| 角色 | 真正职责 | 不负责什么 |
| --- | --- | --- |
| `ContextAgent` | 执行当前这一步，回答问题，调用工具，推进任务；语义上更接近 `SessionAgent` | 不负责长期整理所有记忆 |
| `memory service` | 承载 Memory 的读写、检索、整理、投影 | 不是主对话 Agent |
| `memoryAgent` | `memory service` 内部的后台整理角色 | 不是和 `ContextAgent` 平级的第二主 Agent |
