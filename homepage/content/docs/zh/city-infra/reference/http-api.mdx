...
| 路由 | 方法 | 用途 |
| --- | --- | --- |
| `/v1/products/list` | `GET` | Product 列表 |
| `/v1/products/create` | `POST` | 创建 Product |
| `/v1/products/pause` | `POST` | 暂停 Product |
| `/v1/products/activate` | `POST` | 启用 Product |
| `/v1/products/remove` | `POST` | 删除 Product |
| `/v1/products/tokens/apply` | `POST` | 签发 user_token |
...
```json
{ "product_id": "prod_xxx", "name": "Chrome Extension", "status": "active", ... }
```

### `POST /v1/products/tokens/apply`
...
```json
{ "user_token": "ub_xxx", "product_id": "prod_xxx", "user_id": "user_123", ... }
```
...
| `403` | product_id 不匹配、product 已暂停 |
