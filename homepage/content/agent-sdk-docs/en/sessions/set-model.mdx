...
`session.set({ model })` is the only local SDK API for binding a runtime model instance to a Session.
...
- or you are implementing the host's `prepare_session` hook

## Host-managed injection

When several Sessions share a host model policy, use the model-agnostic Agent hook:
...
The SDK does not know which IDs, defaults, or persistence strategy `host_model_runtime` uses.
...
If a local Session has not received a model instance through `session.set({ model })` before execution, execution fails explicitly.

## What about remote sessions

Remote Sessions do not expose a model-setting API. Remote clients only send prompts; the server host selects and injects the model through `prepare_session`.
