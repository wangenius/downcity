...
If you are targeting a remote machine, also confirm that the Console deployment is externally reachable. Current `town` CLI no longer starts or rebinds Console UI, so public access should be handled by the Console product deployment, reverse proxy, tunnel, or hosting layer.
...
- that chat platform has no prior session history
- the current Agent has no connected chat platforms
