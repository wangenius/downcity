...
description: "The boundary between @downcity/studio-cli, AdminClient, and direct database inspection, and when the CLI is the better tool."
---

The CLI and `AdminClient` can both help you manage City, but they are better at different kinds of operational work.
...
move toward `AdminClient` or a trusted script.

## CLI, AdminClient, and direct DB inspection each fit different work
...
If you operate many studios, services, or model configurations, it remains a frequent operations entry.

### `AdminClient` is not just a more advanced CLI

They are different modes:

- `AdminClient`: trusted programmatic management
...
- For the basic agent entry, read [Studio CLI Quickstart](/en/docs/city/packages/studio-cli-terminal/quickstart)
- For model and table inspection, read [Models and Tables](/en/docs/city/packages/studio-cli-terminal/models-and-tables)
- For the data layer behind those facts, read [Store and Table](/en/docs/city/packages/city/store-and-table)
