...
description: "The City data layer, table capabilities, and why studio, env, and usage facts eventually live in the store layer."
---
...
```ts
const studios = await base.table("studios");
...
Many management and studio capabilities depend on persistent data facts behind the route layer.

### `table()` is not a reason to skip good service design

You should still prefer services, and `AdminClient` for studio-facing or managed entry points.

### Frontends should not treat tables as their API

Studio-side callers should continue to use `UserClient` and `AdminClient`, not raw table access.

## Read next

- For how the database is wired in, read [City](/en/docs/city/packages/city/city-runtime)
- For terminal inspection of these tables, read [@downcity/studio-cli](/en/docs/city/packages/studio-cli-terminal)
