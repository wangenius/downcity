...
`federation_url + "/v1/ai"`。同一段代码可以同时支持 HTTP 和本机 RPC
Federation URL。
...
HTTP Federation 下 transport 只返回 `baseURL`。`rpc://` Federation 下还会返回
`fetch`，它会把 OpenAI-compatible request 转成 Federation RPC，并保留 header、
query string 和 body。
