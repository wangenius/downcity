...
export async function manageEnv(a: CityPact, _baseUrl: string, runtime: admin_tui_runtime): Promise<void> {
  while (true) {
    const services = await fetchEnvScopes(a, runtime);

    const choices: admin_tui_select_option[] = [
      {
...
async function listAllEnv(a: CityPact, runtime: admin_tui_runtime): Promise<void> {
  try {
...
async function upsertEnv(a: CityPact, runtime: admin_tui_runtime): Promise<void> {
...
async function updateEnv(a: CityPact, runtime: admin_tui_runtime): Promise<void> {
  ...
  try {
    await runtime.with_loading(t({ zh: `更新 ${String(key)}`, en: `Update ${String(key)}` }), async () => await a.env.upsert({ key: key as string, value }));
...
async function removeEnv(a: CityPact, runtime: admin_tui_runtime): Promise<void> {
  const key = await runtime.text(t({ zh: "key", en: "key" }));
  if (!key) return;
  try {
...
async function refreshEnv(a: CityPact, runtime: admin_tui_runtime): Promise<void> {
  try {
...
async function fetchCurrentEnv(a: CityPact): Promise<{ key: string; value: string }[]> {
  try {
