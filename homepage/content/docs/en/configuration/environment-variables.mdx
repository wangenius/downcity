...
town plugin action asr configure --payload '{"modelId": "SenseVoiceSmall"}'
```

2. Manage shared env in City UI `Global / Env`, or pass them explicitly through `new Agent({ env })`.
3. Create channel accounts in City UI `Global / Channel Accounts`.
...
When you use `town start -p` or `town console start -p`, the CLI tries to print `Public URL` in startup output.
If you use `town public on`, future plain `town start` and `town console start` runs also reuse the saved public bind mode.

These values are normally injected by `town start` or the deployment environment. They are not endpoint parameters that users enter for `contact link`:
...
`town start` automatically detects the public IP and stores it in City-side Env as `DOWNCITY_PUBLIC_HOST`. Agent `contact link` generation also uses this value to create transferable contact codes.
