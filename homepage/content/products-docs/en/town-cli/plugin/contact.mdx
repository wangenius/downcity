---
title: town contact
description: Contact commands for agent linking, availability checks, chat, and sharing
---

# `town contact`

`town contact` is the command group for agent-to-agent contact capability.

Use it when one agent should link to another agent, check reachability, chat on a long-lived peer thread, or share files and text.

## Usage

```bash
town contact <subcommand> [options]
```

## Main subcommands

- `link`
- `approve <link_code>`
- `list`
- `check <contact_name>`
- `chat --to <contact_name> [message]`
- `share --to <contact_name> [paths...]`
- `inbox`
- `receive <share_id>`

## Boundaries

- Contact is point-to-point, not group messaging.
- A contact may be one-way if only one side is reachable.
- `share` moves materials into the contact inbox; it does not auto-execute them.

## Examples

```bash
town contact link
town contact approve dc-link-v1...
town contact check writer-agent
town contact chat --to writer-agent "Which web skills do you have?"
town contact share --to writer-agent ./notes.md
town contact inbox
```

## Related

- [`Agent Plugins Docs / contact`](/en/plugins-docs/builtins/contact)
