/**
 * Products Docs 文档布局路由模块。
 * 说明：
 * 1. `products-docs` 与 `docs`、各 SDK docs 平级存在，单独承载 Town CLI、Town Console 与 Chrome Extension 产品文档。
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  return {
    tree: productsDocsSource.pageTree[lang],
    lang,
  };
}
...
export default function Layout({ loaderData }: Route.ComponentProps) {
  ...
  useEffect(() => {
    ...
    if (location.pathname === "/products-docs" || location.pathname === "/products-docs/") {
      navigate("/en/products-docs", { replace: true });
    }
