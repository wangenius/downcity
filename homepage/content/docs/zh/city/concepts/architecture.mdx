...
```txt
Interfaces
  -> bays 通过 client / Bay terminal 接入 City
Composition
...
- 产品侧入口：`@downcity/city` 的 `User Visa` 与 `Admin Visa`
- 运维入口：`downcity`
...
3. 解析 `bay_id`、`user_id` 和身份级别
4. 找到目标 Service / Action
5. 组装统一 `ctx`
6. 执行 hook 和 action

如果 token 无效、过期或 `bay_id` 不匹配，City 会直接返回 401 或 403。
