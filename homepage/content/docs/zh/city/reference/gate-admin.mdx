...
把 `Admin Gate` 看成“可信侧和 City 的管理桥”最准确。
...
如果 `User Gate` 负责“用户态怎么调用 City”，那 `Admin Gate` 负责“可信侧怎么把这个调用环境准备好”。
...
<div className="dc-doc-flow" aria-label="Admin Gate 的调用链">
...
4. 前端再用 `User Gate` 调 City

## 它和 accounts 服务是什么关系

`Admin Gate` 不会替代 accounts 服务，反过来也一样。
...
- 业务后端再用 `Admin Gate` 申请 `user_token`
- 前端拿到 `bay_id + user_token`

### 模式 B：accounts 服务做登录

- 前端先用 guest `User Gate` 调 `accounts.login/register`
- accounts service 直接返回 `user_token`
- 前端切换到正常 `User Gate`

所以：

- `Admin Gate` 负责可信侧管理动作
- accounts 服务负责最小账号能力
...
## Admin Gate 也可以调 service 的管理侧 service

除了 `bays.*` 和 `env.*`，`Admin Gate` 也可以调用 service 暴露出来的管理侧 service。
...
也就是说，`Admin Gate` 不只是在调内建 bays/env 接口，它同样是可信侧调用统一 service 路由的入口。

## 错误处理

`Admin Gate` 收到非 2xx HTTP 响应时会抛出 `Error`。这个错误会带两个额外字段：
...
也就是说，`Admin Gate` 负责维护 bay、token 和 env；模型定义和 service 挂载方式仍然属于 `City` 运行时层。
