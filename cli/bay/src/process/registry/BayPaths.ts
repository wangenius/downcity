/**
 * BayPaths：Bay 全局运行态与 control plane 共享的路径规则。
 *
 * 关键点（中文）
 * - Downcity 的全局根目录固定在用户目录 `~/.downcity/`。
 * - `~/.downcity/downcity.db`：全局 SQLite 数据库，保存模型池与平台级配置。
 * - `~/.downcity/main/*`：bay 后台与 control plane 共享的运行文件目录。
...
/**
 * Bay 全局运行目录（pid/log/registry）。
 */
export function getBayRuntimeDirPath(): string {
...
export function getPlatformStoreKeyPath(): string {
  return path.join(getBayRuntimeDirPath(), "model-db.key");
}

/**
 * bay 后台 pid 文件路径。
 */
export function getBayPidPath(): string {
  return path.join(getBayRuntimeDirPath(), "bay.pid");
}

/**
 * bay 后台日志路径（stdout/stderr 合并）。
 */
export function getBayLogPath(): string {
  return path.join(getBayRuntimeDirPath(), "bay.log");
}
...
export function getControlPlanePidPath(): string {
  return path.join(getBayRuntimeDirPath(), "control-plane.pid");
}
...
export function getControlPlaneLogPath(): string {
  return path.join(getBayRuntimeDirPath(), "control-plane.log");
}
...
export function getControlPlaneMetaPath(): string {
  return path.join(getBayRuntimeDirPath(), "control-plane.json");
}

/**
 * 受管 agent registry 文件路径（Bay 维护的 agent 清单）。
 */
export function getManagedAgentRegistryPath(): string {
  return path.join(getBayRuntimeDirPath(), "agents.json");
}
