---
title: 官方服务
description: 用 package 服务给 InfraRuntime 增加 accounts、usage 和 payment 能力。
---

服务把多个 client 产品会复用的后端能力封装成 package。InfraRuntime 使用者不需要自己定义这些服务的数据库表。
...
服务会把自己的 action 和数据表注册进 InfraRuntime。官方产品示例里还会显式注册 `EnvService`、`ProductsService` 和 `AIService`。
...
UserClient 带上 `product_id + user_token` 后，调需要用户鉴权的服务入口：
...
- [Accounts 服务](/docs/city-infra/guides/accounts-service)
- [Usage 服务](/docs/city-infra/guides/usage-service)
- [Stripe 支付服务](/docs/city-infra/guides/payment-stripe-service)
