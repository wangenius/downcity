...
| Config scope | `ship.json.services.*` (per-project) | `~/.ship/ship.db` `extensions_config.*` (console-global) |
...
- Note:
  ...
  - extension switches/config are managed at console-global layer (`~/.ship/ship.db` `extensions_config.*`).
