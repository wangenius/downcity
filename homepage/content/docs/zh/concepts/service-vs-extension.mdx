...
| 配置作用域 | `ship.json.services.*`（每个 agent 项目独立） | `~/.ship/ship.db` 的 `console_config.extensions.*`（console 全局） |
...
- 补充：
  ...
  - extension 开关与配置在 console 全局层统一管理（`~/.ship/ship.db` 的 `console_config.extensions.*`）。
