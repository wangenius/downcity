...
Agent config no longer exposes a separate `permissions` block.
...
Chat user authorization is not written back into the Agent project directory.
...
Manage chat authorization from the Downcity chat product surface. Common management tasks include:
...
When a chat user is denied, the denial message includes the target user identity so an admin can assign the user to the right permission group.
...
- [Console](/en/docs/reference/cli)
- [Downcity chat docs](/en/docs/reference/cli)
