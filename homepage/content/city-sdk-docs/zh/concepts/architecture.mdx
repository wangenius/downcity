...
```txt
Interfaces
  -> cities 通过 client / City terminal 接入 City
Composition
...
它们都不实现服务端 runtime 本身，只负责把请求送进同一套 Federation。

## 2. Composition：谁来把 City 组装起来

Downcity 的运行时不是“new 一个 City 就结束”，中间还有一层面向部署环境的装配：

- `templates/node` 是 Node.js + SQLite 的开发者快捷示例
- `templates/edge` 是 Cloudflare Workers + D1 的开发者快捷示例
...
3. 解析 `city_id`、`user_id` 和身份级别
4. 找到目标 Service / Action
5. 组装统一 `ctx`
6. 执行 hook 和 action

如果 token 无效、过期或 `city_id` 不匹配，City 会直接返回 401 或 403。
...
Agent / client.ai.stream()
  -> CityModel.doStream()
  -> POST /v1/ai/stream
  -> Provider create_language_model(ctx)
  -> LanguageModelV3StreamPart
  -> Agent SessionMutation 或客户端 UIMessageChunk
```

### OpenAI 兼容通路

给 OpenAI SDK、curl 和其他兼容客户端调用。Downcity Agent 使用 Fed 模型时，
可以直接接收 `client.ai.listModels()` 返回的 `CityModel`：
