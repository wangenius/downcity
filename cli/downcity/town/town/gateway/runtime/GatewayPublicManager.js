 ...
 * - 只管理 Console / gateway 的公网暴露，不改 agent daemon 监听。
 * - 修改配置后，若 Console 正在运行，则自动重启使新绑定立即生效。
 */
import prompts from "prompts";
import { emitCliBlock, emitCliList } from "../../../shared/CliReporter.js";
import { getGatewayRuntimeStatus, restartGatewayRuntimeCommand, } from "./GatewayRuntime.js";
import { readGatewayPublicModeSetting, writeGatewayPublicModeSetting, } from "./GatewayPublicMode.js";
...
function toPublicModeSummary(setting) {
    if (setting.enabled !== true)
        return "local";
    return `public · ${String(setting.host || "0.0.0.0").trim() || "0.0.0.0"}`;
}
async function printPublicModeStatus() {
    const [setting, status] = await Promise.all([
        readGatewayPublicModeSetting(),
...
async function applyPublicModeSetting(setting, cliPath) {
    const nextSetting = await writeGatewayPublicModeSetting(setting);
    ...
    if (status.running !== true) {
        emitCliBlock({
            ...
            note: "配置已保存，将在下次 `town start` / `town console start` 时自动生效。",
        });
        return;
    }
    emitCliBlock({
...
async function promptInteractivePublicModeHost(initialValue) {
...
async function runInteractivePublicModeManager(cliPath) {
    ...
    while (true) {
        ...
        if (action === "on-custom") {
            const host = await promptInteractivePublicModeHost(String(setting.host || "0.0.0.0"));
            if (!host) {
                emitCliBlock({
                    tone: "info",
 ...
 * `town public` 命令入口。
 */
export async function gatewayPublicCommand(params) {
    ...
    emitCliList({
        ...
        items: [
            { title: "Use `town public` for interactive mode" },
            { title: "Use `town public status`" },
            { title: "Use `town public on [--host <host>]`" },
            { title: "Use `town public off`" },
        ],
    });
}
//# sourceMappingURL=GatewayPublicManager.js.map