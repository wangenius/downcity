/**
 * CLI bay/control-plane 命令装配。
 *
 * 关键点（中文）
 * - 这里的 `console` 更接近 Bay gateway / control plane 的运维入口，而不是单 agent API。
 * - 统一管理 top-level bay 生命周期命令与 control plane 模块命令。
...
/**
 * top-level bay/control-plane 命令注册参数。
 */
...
/**
 * 注册 top-level bay 生命周期命令与 `console` 模块命令。
 *
 * 语义说明（中文）
 * - `bay ...` / `bay console ...` 管的是本机宿主与平台控制面进程。
...
export function registerControlPlaneCommands(
  ...
  program
    .command("init")
    .description("初始化 Bay 全局配置（模型/插件等，写入 ~/.downcity/downcity.db）")
  ...
  program
    .command("start")
    .description("启动 bay runtime；使用 -a/--all、--console 或 -p/--public 可同时启动 Console")
  ...
  program
    .command("status")
    .description("查看 bay 后台、Console 与已托管 agent 运行状态")
    .helpOption("--help", "display help for command")
    .action(createVersionBanner(context.version, async () => {
