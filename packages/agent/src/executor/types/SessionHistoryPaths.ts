 ...
 * - 每个字段都为可选，未提供时回退到默认 `.downcity/agents/<encodedAgentId>/sessions/<encodedSessionId>/messages/*`。
 */
export type SessionHistoryPathOverrides = {
  /**
   * Session 根目录（通常是 `.downcity/agents/<encodedAgentId>/sessions/<encodedSessionId>`）。
   */
