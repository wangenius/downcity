...
`active.jsonl` 只保存最近一次 Compact 之后仍处于活动窗口中的真实 `SessionMessage`。它没有固定条数上限，也不按 API 页大小切分；长度由 token 阈值、`keepLastMessages` 和 `compactRatio` 共同决定。

Compact 发生时，SDK 选择 Active 的一个旧前缀，创建按 sequence 范围命名的不可变 Segment。Segment 每行先保存真实 Message，最后一行保存累计 Summary footer：
...
sequenceDiagram
    ...
    Session->>Recorder: token 超阈值，触发 Compact
    Recorder->>Model: 旧 Summary + Active 待压缩前缀
...
`meta.json` 保存列表和详情所需的轻量属性，包括 `sessionId`、`agentId`、标题、模型、时间戳、消息数、历史字节数和时区。Session 属性或消息状态变化时，SDK 会同步更新这些摘要字段。
