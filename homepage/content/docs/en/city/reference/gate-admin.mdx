...
If `User Gate` owns "how a user-context studio call reaches City," then `Admin Gate` owns "how the trusted side prepares that call environment."
...
Besides `studios.*` and `env.*`, `Admin Gate` can call admin-side services exposed by official packages.
...
In other words, `Admin Gate` is not only for built-in studios/env endpoints. It is also the trusted-side entry into the same unified service route space.
