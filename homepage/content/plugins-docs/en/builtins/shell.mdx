...
Local and remote Sessions expose the same API. A live UI can also call `reply.approval({ decision })` when `session.subscribe()` receives an approval-required Tool Part.
