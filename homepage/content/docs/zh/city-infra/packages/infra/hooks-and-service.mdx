---
title: Hook 与服务挂载
description: "hook 三层、service 挂载方式，以及它们为什么是 InfraRuntime 的共享扩展点。"
---

Downcity 很多共享能力不是靠“把代码复制到每个 service 里”完成的，而是通过 hook 和 service 进入 InfraRuntime。
...
- hook：给已有调用链插入规则
- service：给 InfraRuntime 增加一整组长期复用能力

所以你看到的 usage、账号、支付，并不是“额外开一套系统”，而是作为扩展点进入同一个 InfraRuntime。
...
- `accountsService()` 给 InfraRuntime 增加 `accounts` service
- `paymentService()` 给 InfraRuntime 增加统一支付入口，用来暴露当前可用的支付方式
- `usageService()` 给 InfraRuntime 增加 usage 记录能力和相关查询入口
- `stripePaymentService()` 给 InfraRuntime 增加 Stripe Checkout、webhook 和支付结果同步
...
- `base.use(service)`：把 service 挂进 InfraRuntime
...
它代表的是一整组进入 InfraRuntime 的能力边界，通常还伴随：
...
### service 不替代 conduit SDK

service 负责把能力挂进 InfraRuntime，真正的产品调用入口仍然是 [@downcity/conduit](/docs/city-infra/packages/conduit)。

## 继续阅读

- 想看 product 端怎么统一调用 service，读 [@downcity/conduit](/docs/city-infra/packages/conduit)
- 想看账号能力，读 [@downcity/services](/docs/city-infra/packages/services)
- 想看 usage 记录，读 [@downcity/services](/docs/city-infra/packages/services)
- 想看 entitlement 和支付，读 [@downcity/services](/docs/city-infra/packages/services)
