 ...
 */
function readInstalledPackageName(packageRoot) {
    try {
 ...
 * 检测当前 CLI 实际安装来源的包名。
 */
export function detectInstalledPackageName() {
    return readInstalledPackageName(resolveInstalledPackageRoot());
}
export function buildGlobalUpdateInvocation(manager, packageName) {
    if (manager === "pnpm") {
        return {
            command: "pnpm",
...
export function resolveUpdateManagerFromGlobalRoots(params) {
    ...
    if (pnpmRoot && isSubPath(packageRoot, pnpmRoot))
        return "pnpm";
    if (npmRoot && isSubPath(packageRoot, npmRoot))
        return "npm";
    return null;
}
function normalizeRealPath(target) {
...
function readGlobalRoot(manager) {
    try {
 ...
 * 自动判断当前全局安装使用的包管理器。
 */
export function detectInstalledUpdateManager() {
...
export async function updateCommand(options = {}) {
    ...
    const manager = options.manager && options.manager !== "auto"
        ? options.manager
    ...
    try {
        await runWithSpinner(() => runCommand(invocation.command, invocation.args), { text: `Installing ${packageName}@latest via ${manager}...` });
    }
    catch (error) {
        throw new CliError({
