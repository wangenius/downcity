...
description: Understand the bay agent command set by workflow rather than by flat command list
---

# Agent CLI Overview

`bay agent` is the command surface for the single-project Agent runtime.

If you think of the Agent as "the execution unit for one project," the `bay agent` command group is its operator console.
