...
`/data` 必须是持久化 volume。studio、model、用户、session、usage、payment 和 City env 都会保存在这个 SQLite 数据库里。
...
运行时 `ctx.env.MOONSHOT_API_KEY` 和 `ctx.env.DEEPSEEK_API_KEY` 会优先从 City 数据库读取。
...
产品前端和应用应该通过 `@downcity/gate` 调用 Node 街区：
...
可信后端应该用 `AdminClient` 携带 `DOWNCITY_CITY_ADMIN_SECRET_KEY`：
...
完整 SDK 能力继续看 [UserClient](/docs/city/reference/user-client) 和 [AdminClient](/docs/city/reference/admin-client)。
