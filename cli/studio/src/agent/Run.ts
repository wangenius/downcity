 ...
 *
 * 场景
 * - `studio agent start --foreground` 走这里（当前终端前台运行）
 * - daemon 子进程也复用这里作为真正运行入口
 *
 * 说明
 * - 后台常驻启动请使用 `studio agent start`，并用
 *   `studio agent restart` 管理。
 */
...
export async function runCommand(
  ...
  process.env.DC_Studio_PORT = String(port);
  process.env.DC_Studio_HOST = host;
  ...
  const shutdown = async (signal: string) => {
    ...
    agentLogger.info("👋 Downcity studio stopped");
    process.exit(0);
  };
