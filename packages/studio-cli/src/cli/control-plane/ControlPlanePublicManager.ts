...
async function applyPublicModeSetting(
  ...
  if (status.running !== true) {
    emitCliBlock({
      ...
      note: "配置已保存，将在下次 `studio start` / `studio console start` 时自动生效。",
    });
    return;
  }
...
export async function controlPlanePublicCommand(params: {
  ...
  emitCliList({
    ...
    items: [
      { title: "Use `studio public` for interactive mode" },
      { title: "Use `studio public status`" },
      { title: "Use `studio public on [--host <host>]`" },
      { title: "Use `studio public off`" },
    ],
  });
}
