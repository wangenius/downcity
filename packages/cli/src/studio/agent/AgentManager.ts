 ...
 *
 * 关键点（中文）
 * - 裸 `studio agent` 在交互式终端里进入这里，而不是只输出静态 help。
...
import { prepareForegroundAgent } from "../../city/control-plane/ControlPlaneCommand.js";
...
async function promptAgentProjectRoot(): Promise<AgentManagerAgentSummary | null> {
  ...
  if (agents.length === 0) {
    emitCliBlock({
      ...
      note: "运行 `studio agent create` 创建项目，或运行 `studio agent start <path>` 登记已有项目。",
    });
    return null;
  }
