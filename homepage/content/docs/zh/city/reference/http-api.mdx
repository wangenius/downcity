...
| 路由 | 方法 | 用途 |
| --- | --- | --- |
| `/v1/studios/list` | `GET` | Studio 列表 |
| `/v1/studios/create` | `POST` | 创建 Studio |
| `/v1/studios/pause` | `POST` | 暂停 Studio |
| `/v1/studios/activate` | `POST` | 启用 Studio |
| `/v1/studios/remove` | `POST` | 删除 Studio |
| `/v1/studios/tokens/apply` | `POST` | 签发 user_token |
...
```json
{ "studio_id": "studio_xxx", "name": "Chrome Extension", "status": "active", ... }
```

### `POST /v1/studios/tokens/apply`
...
```json
{ "user_token": "ub_xxx", "studio_id": "studio_xxx", "user_id": "user_123", ... }
```
