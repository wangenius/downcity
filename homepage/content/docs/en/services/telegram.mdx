...
- If a `messageId` is available in chat context, ShipMyAgent automatically uses Telegram `reply_to_message` for outbound sends.
- For lightweight acknowledgement/attitude, use `sma chat react --emoji '👍'` (supports `--big`).
