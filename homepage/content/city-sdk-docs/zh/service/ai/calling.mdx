...
`city.ai.catalog()` 在 `user_token` 身份下只返回当前已经满足运行时 `env` 的模型；同一条路由在 `admin_secret_key` 身份下会返回全部代码注册模型和它们的 `env_requirements`。
...
CityModel 请求 Federation 的 `/v1/ai/stream`。`city.ai.stream()` 也使用同一个
...
| 场景 | 推荐方式 |
|---|---|
| 产品代码直接请求 Fed 文本或流式生成 | `city.ai.text()` / `city.ai.stream()` |
...
| 图片、TTS、ASR、video | `city.ai.image_create()` / `city.ai.tts()` / `city.ai.asr()` 等 City SDK 方法 |
