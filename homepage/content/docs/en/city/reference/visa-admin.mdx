---
title: Admin Visa
description: How trusted environments manage bays, issue user tokens, and maintain runtime env.
---

`Admin Visa` should only run in trusted environments.

Typical cases:

- your own bay backend
- local admin scripts
- internal tools
...
The most accurate mental model is: `Admin Visa` is the trusted-side bridge into City management.
...
If `User Visa` owns "how a user-context bay call reaches City," then `Admin Visa` owns "how the trusted side prepares that call environment."
...
<div className="dc-doc-flow" aria-label="Admin Visa call flow">
  ...
  <div className="dc-doc-lane">
    <div className="dc-doc-node">
      <strong>Admin Visa</strong>
      <span>Calls `/v1/bays/*`, `/v1/bays/tokens/apply`, `/v1/env/*`, `/v1/ai/models`, and `/v1/base/instruction`.</span>
    </div>
  </div>
  <div className="dc-doc-arrow" aria-hidden="true">→</div>
  <div className="dc-doc-lane">
    <div className="dc-doc-node">
      ...
      <span>Verifies `admin_secret_key`, then executes bay, token, and env management actions.</span>
    </div>
  </div>
</div>
...
```ts
const items = await admin.bays.list();
```
...
```ts
await admin.bays.pause(bay.bay_id);

await admin.bays.activate(bay.bay_id);
```

Useful when you need to:

- stop all user calls for one bay
- temporarily take a bay offline
- reopen calls for a bay later

### `remove()`

```ts
await admin.bays.remove(bay.bay_id);
```

The current behavior is direct bay deletion. Confirm that this matches your business expectation before using it.

## `tokens.apply()`

This is the most common trusted-side action: issue a `user_token` for one user under one bay.
...
3. it returns `bay_id + user_token` to the client
4. the client calls City through `User Visa`

## Relationship with the accounts service

`Admin Visa` does not replace the accounts service, and the service does not replace `Admin Visa`.
...
- your backend uses `Admin Visa` to request `user_token`
- the frontend receives `bay_id + user_token`

### Pattern B: the accounts service owns login

- the frontend uses a guest `User Visa` to call `accounts.login/register`
- the accounts service returns `user_token` directly
- the frontend switches to a normal `User Visa`

So:

- `Admin Visa` owns trusted-side management actions
...
Besides `bays.*` and `env.*`, the admin side can also read the current capability catalog exposed by City.
...
## Admin Visa can also call service-provided admin services

Besides `bays.*` and `env.*`, `Admin Visa` can call admin-side services exposed by official packages.
...
In other words, `Admin Visa` is not only for built-in bays/env endpoints. It is also the trusted-side entry into the same unified service route space.

## Error handling

When `Admin Visa` receives a non-2xx HTTP response, it throws an `Error` with two extra fields:
...
- `403`: the target bay is paused, so no token can be issued.
- `404`: the target bay does not exist.
- `500`: City is missing required config, or the admin action failed internally.

## What `Admin Visa` does not manage right now

These do not belong to `Admin Visa` yet:
...
In other words, `Admin Visa` owns env maintenance; runtime model definitions and mounting still belong to the `City` runtime layer.
