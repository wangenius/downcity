...
- Agent / Authorization is now available for every agent, with an openclaw-inspired authorization model: owner, DM policy, group policy, allowlists, and pairing approval in one place.
- Agent / Authorization is stored in console `~/.ship/ship.db`, not written back to project `ship.json` in plaintext.
- In Agent / Authorization, owner directly drives `is_master`. When `ownerIds` is configured, runtime no longer depends on a single `FEISHU_AUTH_ID / TELEGRAM_AUTH_ID / QQ_AUTH_ID`.
