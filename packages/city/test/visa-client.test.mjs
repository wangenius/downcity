...
test("User Visa delegates AI calls", async () => {
  const requests = []
  const msg = { id: "msg_1", role: "assistant", parts: [{ type: "text", text: "hello", state: "done" }] }
  const visa = new Visa({
    role: "user",
  ...
  const result = await visa.ai.text({ prompt: "hi" })
  assert.deepEqual(result, msg)
  assert.equal(requests[0].url, "https://api.example.com/base/v1/ai/text")
  assert.deepEqual(JSON.parse(requests[0].init.body), { prompt: "hi", bay_id: "bay_demo" })
})
...
test("User Visa listServices()", async () => {
  const client = new Visa({
    role: "user",
    city_url: "https://api.example.com/base/",
    bay_id: "p",
...
test("User Visa service() → ServiceInvoker", async () => {
  const requests = []
  const client = new Visa({
    role: "user",
...
test("User Visa payment.methods() reads the unified payment directory", async () => {
  const requests = []
  const client = new Visa({
    role: "user",
    city_url: "https://api.example.com/base/",
...
test("User Visa payment.method(id).invoke() dispatches to the concrete payment service", async () => {
  const requests = []
  const client = new Visa({
    role: "user",
    city_url: "https://api.example.com/base/",
...
test("User Visa payment.method(id).invoke() rejects disabled or user-required methods early", async () => {
  const disabledClient = new Visa({
    role: "user",
    city_url: "https://api.example.com/base/",
    bay_id: "p",
    fetch: async () => json({
...
test("Admin Visa service() uses the shared /v1 route prefix", async () => {
  const requests = []
...
test("Admin Visa env list / catalog / upsert / remove", async () => {
  const requests = []
  const admin = new Visa({
    role: "admin",
...
test("Admin Visa bays CRUD + tokens.apply", async () => {
...
test("Admin Visa listServices() / listModels() / instruction()", async () => {
  const requests = []
  const admin = new Visa({
    role: "admin",
