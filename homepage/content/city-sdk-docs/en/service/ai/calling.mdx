...
manually building `baseURL`. The same code works for HTTP and local RPC
Federation URLs.
...
For HTTP Federation the transport only returns `baseURL`. For `rpc://`
Federation it also returns a `fetch` implementation that forwards the
OpenAI-compatible request through Federation RPC while preserving headers,
