...
```ts title="index.ts"
hero.createShot(); // 创建一个 shot
hero.shot("id"); // 切换到对应的shot
hero.current(); // 当前状态： room， shot， skill
room.clear(); // 清理room中的会话
```
...
hero 学习了 codex（知识库）, 如果在需要的时候，可以调用 对应的 方法来获取 codex 中的内容。
