...
- whether City has `DOWNCITY_CITY_TOKEN_SIGNING_KEY` configured
- whether the token was issued by the current City
- whether `studio_id` matches the studio embedded in the token

## client returns 403

Check:

- whether the `studio_id` in the request matches the studio bound to the current `user_token`
- whether the studio bound to the token has been paused
- whether the target studio was `active` when the admin side issued the token
...
- whether the target studio existed when the admin side issued the token
...
- whether City has both `DOWNCITY_CITY_TOKEN_SIGNING_KEY` and `DOWNCITY_CITY_ADMIN_SECRET_KEY`
