...
5. enabling or disabling the plugin in the current project config
...
Use the `town web` shortcut for normal CLI work:

```bash
town web status
town web providers
town web install --provider web-access --scope user
town web on --provider web-access
town web use agent-browser --browser-command agent-browser
town web doctor
town web off
```

`configure` exists as an SDK/plugin action, but it is not exposed as a `town web configure` shortcut. Use `town web on`, `town web use`, and `town web off` for normal CLI changes.
...
| `configure` | write config | SDK or advanced plugin action calls |
| `install` | install the current provider dependency | first-time setup |
| `on` | enable the plugin | set `plugins.web.enabled` for the current project |
| `off` | disable the plugin | set `plugins.web.enabled` for the current project |
...
If you want to understand the split between “plugin connects capability” and “external tool executes capability,” `web` is one of the best examples.
...
### `on/off` writes project enablement

That means:

- `configure` changes how the plugin should be used
- `on/off` changes whether the current project considers the plugin enabled

Those are related project-level writes: `configure` updates provider options, while `on/off` updates the project enablement flag.
