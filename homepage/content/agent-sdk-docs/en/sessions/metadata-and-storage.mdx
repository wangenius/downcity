...
`active.jsonl` contains only real `SessionMessage` values still in the active window after the latest Compact. It has no fixed message-count or API page-size limit. Its length is controlled by the token threshold, `keepLastMessages`, and `compactRatio`.

During Compact, the SDK selects an older Active prefix and creates an immutable Segment named by its sequence range. The Segment stores real Messages first and one cumulative Summary footer last:
...
sequenceDiagram
    ...
    Session->>Recorder: token threshold triggers Compact
    Recorder->>Model: old Summary + Active prefix
