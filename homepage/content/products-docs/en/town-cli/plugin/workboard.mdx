...
> This page remains as a command entry point. For the fuller plugin structure, see [Plugin Overview](/en/agent-sdk-docs/plugins/overview), [Built-in Plugins Today](/en/agent-sdk-docs/plugins/built-in-plugins), and [Plugin HTTP Injection](/en/agent-sdk-docs/plugins/plugin-http).
...
- `workboard.snapshot` plugin action, which Town Console calls through Agent RPC by default

These entry points return structured JSON, but only as a public-safe high-level projection. They do not expose internal session, plugin runtime, task, path, or model details.
