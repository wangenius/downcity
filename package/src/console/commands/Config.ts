...
import {
  ModelManager,
  type ModelPreset,
} from "@console/model/ModelManager.js";
...
function readConsoleShipConfig(): { shipJsonPath: string; config: ShipConfig } {
  return readShipConfigByPath(getConsoleShipJsonPath(), "console");
}
...
export function registerConfigCommand(program: Command): void {
  ...
  applyCommonOptions(
    ...
    if (keyPath) {
      ...
      if (isConsoleScopedConfigPath(pathTokens)) {
        const asJson = options.json !== false;
        try {
          const { shipJsonPath, config: shipConfig } = readConsoleShipConfig();
          const got = getByPath(
    ...
    (
      ...
      if (isConsoleScopedConfigPath(pathTokens)) {
        ...
        try {
          ...
          writeShipConfig(shipJsonPath, shipConfig);
          printResult({
            asJson,
            success: true,
  ...
  applyCommonOptions(
    model
      ...
      .option("--preset <presetId>", "模型预设 ID（来自 console/model）")
    ...
    (
      ...
      runStoreCommand(options, async (store) => {
        ...
        const modelName = String(options.name || "").trim() || preset?.id || "";
        if (!modelName) {
    ...
    (
      ...
      runStoreCommand(options, async (store) => {
        ...
        if (preset) {
          ...
          if (targetProvider.type !== preset.providerType) {
            throw new Error(
              `Preset "${preset.id}" expects provider type "${preset.providerType}", but provider "${targetProviderId}" is "${targetProvider.type}".`,
            );
          }
          // 关键点（中文）：指定预设时，默认把模型名对齐为预设 ID；显式 --name 可覆盖。
          nextModel.name = preset.id;
        }
