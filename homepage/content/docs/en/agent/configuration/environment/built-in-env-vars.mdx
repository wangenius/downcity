...
The current city runtime endpoint inherited by nested `city` invocations.

They are mainly used when an agent shell or tool launches another `city` command and that child process needs to route back to the current runtime.
