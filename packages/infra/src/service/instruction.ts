 ...
 * 模块说明文档协议。
 *
 * 负责定义 InfraRuntime / Service / InstallableService 可共享的 instruction 能力，
 * 以及 InfraRuntime 聚合输出时使用的格式化工具。
 */
...
export function formatInstructionDocument(input: {
  ...
  lines.push("# Downcity Infra Instruction");
  lines.push("");
  lines.push("## InfraRuntime");
  lines.push("InfraRuntime 是 Downcity 的基础设施运行容器，负责挂载 Service、初始化内置 env/products 能力、统一处理 /v1/* HTTP 路由，以及校验 user_token 与 admin_secret_key。");
  lines.push("");
  lines.push("基础使用：");
  lines.push("1. 创建 InfraRuntime 实例。");
