...
`image_create()` returns after the task is submitted. The server-side Provider/worker is responsible for persisting the final result in the background; `image_result()` only reads that saved state.
