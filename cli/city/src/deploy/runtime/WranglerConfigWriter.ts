 ...
 *
 * 关键点（中文）
 * - `city.json` 是简单的 City 项目声明，`wrangler.toml` 是 CLI 生成的 Cloudflare 配置。
 * - Cloudflare 默认值由 CLI 管理，用户不需要在 `city.json` 里写 worker_name 等细节。
 * - D1 database id 从 `.city/deploy.json` 读取，不污染用户手写配置。
 */
...
/**
 * 根据 City 项目配置和部署状态写入 wrangler.toml。
 */
export function writeWranglerConfig(
  ...
  const config_path = join(config_file.project_dir, "wrangler.toml");

  const lines = [
    `name = ${tomlString(config.name)}`,
    `main = ${tomlString(config.entry)}`,
  ...
  if (config.database) {
    lines.push(
      ...
      `database_id = ${tomlString(state_file.state.cloudflare?.database_id ?? "")}`,
    );
  }
