...
`DOWNCITY_FEDERATION_ADMIN_SECRET_KEY` is used by `Admin City` to manage City.

### token signing key

`DOWNCITY_FEDERATION_TOKEN_SIGNING_KEY` is used internally by City to issue and validate `user_token`.
