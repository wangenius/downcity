...
- 需要一条全新的会话：`agent.session_collection().create_session()`
- 需要继续一个已知 session：`agent.session_collection().get_session(sessionId)`
  - 需要做 session 切换器、历史侧边栏或第三方 session 浏览器：`agent.session_collection().list_sessions()`
  - 需要归档单个不再活跃的会话：`agent.session_collection().archive_session({ id })`
  - 需要浏览已归档的会话列表：`agent.session_collection().archive_sessions(input?)`
  - 需要永久清空已归档会话：`agent.session_collection().clean_archive()`
