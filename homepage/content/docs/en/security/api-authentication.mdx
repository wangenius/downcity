...
the first startup automatically creates a local token and writes it into the local CLI auth state for this machine. The startup output looks like this:
...
```bash
city auth token create console-ui --activate
```

Typical names are:

- `console-ui` for Console UI
- `chrome-extension` for the Chrome extension
- custom names for scripts and local tooling
...
Create a new token and set it as the current CLI token immediately:
...
```bash
city auth token revoke <tokenId>
```
