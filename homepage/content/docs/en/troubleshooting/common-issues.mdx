...
3. **Use npx (alternative)**:
   ...
   npx shipmyagent agent on --daemon
   ```
...
**Problem**: Cannot create `PROFILE.md`, `ship.json`, or `.ship/` directory.
...
### "Agent stops immediately after a runtime error"

**Problem**: You want the Agent to continue execution after an error, instead of stopping right away.

**Current behavior**:

1. Runtime now hands the error back to the Agent as a follow-up message and asks it to continue.
2. To avoid infinite loops, error handover is capped (currently 2 rounds); after that, the run returns failure.
3. `context length` errors still use the dedicated compaction retry flow.
...
1. **Find and kill the process**:
   ...
   shipmyagent agent on --daemon --port 3001
   ```
...
**Problem**: Missing or misplaced `PROFILE.md` file.

**Solution**:
```bash
# Ensure PROFILE.md is in the project root
ls PROFILE.md
...
3. **Restart with debug mode**:
   ```bash
   shipmyagent agent on --daemon --verbose
   # Look for Telegram connection logs
   ```

4. **Ensure Telegram auth is configured**:
   ```json
   {
     "services": {
      "telegram": {
        "enabled": true,
         ...
         "auth_id": "12345678"  // Optional but recommended
       }
     }
   }
   ```
...
1. **Enable verbose logging**:
   ```bash
   shipmyagent agent on --daemon --verbose --log-level debug
   ```
