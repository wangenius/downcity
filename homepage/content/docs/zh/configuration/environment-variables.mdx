---
title: 环境变量配置逻辑（宿主 Env、项目 .env、SDK 显式覆盖）
description: 说明 process.env、项目 .env、SDK env 覆盖与运行时元信息的实际优先级
---

> 标准设计文档： [Env 与 downcity.db 数据库设计](/zh/docs/configuration/env-downcitydb-design)


# 环境变量配置逻辑（宿主 Env、项目 `.env`、SDK 显式覆盖）
...
|---|---|---|---|---|
| 宿主进程 env | `process.env` | 取决于宿主 | 当前进程 | 共享 API Key、部署变量、CI/CD 变量 |
| 项目 env 文件 | `<agent>/.env` | 用户文件 | 单个 agent | 项目本地运行时叠加值 |
| SDK 显式覆盖 | `new Agent({ env })` | 否 | 单个 agent 实例 | 测试场景、嵌入宿主覆盖 |
| 运行时元信息 | 进程内存（`DC_SESSION_ID`、`DC_CTX_*`、`DC_AGENT_*`） | 否 | 单次请求 / 单个进程 | session 与服务端元信息 |
...
1. `@downcity/agent` 会自动读取 `process.env` 和 `<agent>/.env`。
2. `new Agent({ env })` 是唯一显式暴露给 SDK 调用方的 env 覆盖入口。
3. 当宿主是 `city` 时，可以在创建 Agent 前先把 Console 全局 env 注入到 `process.env`。
...
sequenceDiagram
  ...
  CLI->>PROC: 把 Console 全局 env 注入 process.env
  RT->>PROC: 读取宿主 env 快照
  RT->>ENV: 解析项目 .env
  RT->>RT: 应用可选的 new Agent({ env }) 显式覆盖
  RT->>JSON: 读取并合并 downcity.json
  RT->>RT: 使用最终 agent env 解析 ${ENV_KEY}
...
1. 先读取宿主进程 `process.env`。
2. 再叠加 `<agent>/.env`。
3. 最后叠加 `new Agent({ env })`。

因此，Agent 级同名键覆盖顺序是：SDK 显式 `env` > 项目 `.env` > 宿主 `process.env`。

### 3.2 运行时元信息注入优先级

1. 运行时 helper 先从已经解析好的 agent env 快照出发。
2. `DC_SESSION_ID`、`DC_CTX_*` 等请求级元信息最后注入。

因此，带运行时元信息时的优先级是：`DC_CTX_*` / `DC_SESSION_ID` > SDK 显式 `env` > 项目 `.env` > 宿主 `process.env`。
...
1. 项目层 `downcity.json` 的 `${ENV_KEY}` 解析。
2. Agent 运行时 env 组装（`process.env` + `.env` + 可选 SDK 覆盖）。
3. 本地 plugin 命令上下文组装。
4. 在已解析 env 之上继续叠加请求元信息的运行时 helper。
...
2. 如果宿主希望这些值参与 Agent 运行时 env，应该在 `new Agent(...)` 之前先把它们注入 `process.env`。
...
flowchart TD
  ...
  B -->|无绑定| D["检查 process.env 与 <agent>/.env 回退键"]
...
2. 在 Console UI `Global / Env` 维护共享 env，或者由部署宿主直接注入 `process.env`。
...
6. 出现“像复用旧值”的情况时，优先检查 `channelAccountId`、宿主 `process.env` 与项目 `.env`。
