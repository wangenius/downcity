...
function buildEnvEntryFromRowSync(row) {
    ...
    if (String(row.scope || "").trim() !== "global")
        return null;
    if (!key || !encrypted)
        return null;
    return {
 ...
 * 同步列出 env 条目。
 */
export function listEnvEntriesSync(context) {
 ...
 * 异步列出 env 条目。
 */
export async function listEnvEntries(context) {
    const rows = context.sqlite.prepare(`
    SELECT scope, agent_id, key, description, value_encrypted, created_at, updated_at
    FROM env_entries
    WHERE scope = 'global'
 ...
 * 新增或更新 env 条目。
 */
export async function upsertEnvEntry(context, input) {
    const scope = "global";
    const agentId = "";
 ...
 * 删除单个 env 条目。
 */
export function removeEnvEntry(context, keyInput) {
 ...
 * 同步列出全局环境变量。
 */
export function listGlobalEnvEntriesSync(context) {
    return listEnvEntriesSync(context);
}
/**
 * 同步读取全局环境变量映射。
 */
export function getGlobalEnvMapSync(context) {
 ...
 * 异步列出全局环境变量。
 */
export async function listGlobalEnvEntries(context) {
    return listEnvEntries(context);
}
/**
 * 异步读取全局环境变量映射。
 */
export async function getGlobalEnvMap(context) {
 ...
 * 新增或更新全局环境变量。
 */
export async function upsertGlobalEnvEntry(context, input) {
 ...
 * 删除单个全局环境变量。
 */
export function removeGlobalEnvEntry(context, keyInput) {
    removeEnvEntry(context, keyInput);
}
/**
 * 清空全局环境变量。
 */
export function clearGlobalEnvEntries(context) {
    context.sqlite.prepare("DELETE FROM env_entries WHERE scope = 'global';").run();
}
//# sourceMappingURL=StoreEnvRepository.js.map