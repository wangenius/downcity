...
`instruction.md` is optional. Sessions do not persist instruction by default and use the Agent's current instruction when restored. The local `session.snapshot()` method atomically writes only the Session's effective custom instruction. Deleting the file restores the default behavior; SDK core, plugin, and Session context blocks are never written to it.
