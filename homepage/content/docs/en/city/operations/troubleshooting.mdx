...
- whether `bay_id` matches the bay embedded in the token

## client returns 403

Check:

- whether the `bay_id` in the request matches the bay bound to the current `user_token`
- whether the bay bound to the token has been paused
- whether the target bay was `active` when the admin side issued the token
...
- whether the target bay existed when the admin side issued the token
