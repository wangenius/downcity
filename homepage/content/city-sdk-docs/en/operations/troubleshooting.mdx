...
- whether `town_id` matches the town embedded in the token

## client returns 403

Check:

- whether the `town_id` in the request matches the town bound to the current `user_token`
- whether the town bound to the token has been paused
- whether the target town was `active` when the admin side issued the token
...
- whether the target town existed when the admin side issued the token
