 ...
 *
 * 关键点（中文）
 * - 统一承接 `bay agent list` 的 registry 展示逻辑。
 * - 统一承接 `bay agent start` 在省略路径时的目标选择逻辑。
  ...
  if (agents.length === 0) {
    emitCliBlock({
      ...
      note: options?.runningOnly === true
        ...
        : "Run `bay agent start <path>` once to register an agent with Bay.",
    });
    return;
  }
...
/**
 * 为 `bay agent start` 解析最终要启动的项目目录。
 */
