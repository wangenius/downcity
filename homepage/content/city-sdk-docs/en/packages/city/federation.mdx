...
Local RPC is a trusted transport: `Admin City` does not need `admin_secret_key`, and `User City` does not need `user_token`. User calls still need `city_id` because that is product context. HTTP endpoints are unchanged and still require bearer tokens.
