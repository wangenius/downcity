...
If `AccountsService` is configured with `local_login: true`, `/v1/accounts/providers` only returns the local login method. Call `accounts.login/start` with `provider: "local"` and `city_id` to receive a `login_id`, then read the normal `user_token` from `accounts.login/result` and recreate or update User City with `city_id + user_token`.
