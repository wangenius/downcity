...
export interface ServiceInstallContext {
  table<TRow extends Record<string, unknown> = Record<string, unknown>>(name: string): InfraTableApi<TRow>;
...
export interface ServiceRouteContext {
  ...
  user?: RuntimeUser;
  /** 当前 user_token 对应的 product；免登录或 admin 请求时可能为空 */
...
export abstract class InstallableService extends Service {
   ...
   * 服务级全局 hook。
   *
   * 通过 ServiceInstallContext.hook 注册，InfraRuntime 会在所有 Service Action
  ...
  async _onInit(): Promise<void> {
    ...
    const ctx: ServiceInstallContext = {
      table<TRow extends Record<string, unknown> = Record<string, unknown>>(name: string): InfraTableApi<TRow> {
        ...
        return new TableApi(self._db, table) as unknown as InfraTableApi<TRow>;
      },
...
export type ServiceDefinition = {
   ...
   * - InfraRuntime 会把这些 key 自动注册到自己的 env requirement 目录中
   */
  env?: EnvRequirement[];
