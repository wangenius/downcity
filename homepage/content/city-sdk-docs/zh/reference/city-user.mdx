...
`image_create()` 的底层约定很简单：client 只发送 `prompt` / `messages` / `model` / `size` / `ratio` / `quality` / `count` / `provider_options` 这些输入；City 侧 Provider 负责把 OpenAI 图片 API 等上游响应归一成 AI SDK `UIMessage` file parts。
