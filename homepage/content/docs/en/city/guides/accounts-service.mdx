...
Both patterns converge to the same end state: the frontend gets `studio_id + user_token`, then uses a normal `User Gate` to call City.
...
The response includes `user_token`, `user_id`, and `email`. The studio client can then create a normal `User Gate` and call AI services.
...
- `@downcity/city` is what studio-side and trusted-side callers use to call that service
