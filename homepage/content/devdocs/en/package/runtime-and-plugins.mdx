...
```mermaid
flowchart TD
  A["bay console start"] --> B["start console control plane"]
  B --> C["maintain registry / model store / gateway"]

  D["bay agent start"] --> E["validate console and project state"]
...
`bay console start` brings up the global control plane. It handles registry, shared configuration, model pool coordination, and UI-facing gateway behavior.

`bay agent start` then assembles one project into a running agent host. That process reads project config, prepares model binding, creates the session layer, registers plugins, and starts managed runtimes when needed.
