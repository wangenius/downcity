---
title: 快速开始
description: 用正确启动顺序在 10 分钟内跑通 Console + Agent
---

# 快速开始

## 1. 安装

```bash
npm install -g downcity
# 或
pnpm add -g downcity
bay --version
city --version
```

> `downcity` 安装后会提供 `bay` 和 `city` 两个命令。
>
> 如果安装后仍然找不到 `bay`，请先看 [FAQ](/zh/docs/operations/faq)。
>
> 后续升级可以直接执行 `bay update`，它会更新你最初安装的那个包。

## 2. 初始化 Bay（仅首次）

```bash
bay init
```

该命令会生成 Console 全局配置（位于 `~/.downcity/`）。

## 3. 启动 Bay

```bash
bay start
```

> `bay agent start` 依赖 console 先运行。

## 4. 初始化项目

```bash
cd /path/to/your-repo
bay agent create .
```

初始化后会生成：

- `PROFILE.md`（必需）
- `SOUL.md`（可选）
- `downcity.json`
- `.downcity/` 运行目录

## 5. 绑定模型

当前项目 agent 使用的是全局模型池绑定方式。

`downcity.json` 最小示例应为：

```json
{
  "name": "my-agent",
  "version": "1.0.0",
  "execution": {
    "type": "api",
    "modelId": "quality"
  }
}
```

说明：

- `modelId` 指向 Bay 全局模型池中的某个模型
- 项目本身不再直接保存旧的 `model.primary` 结构
- 如果你还没准备模型池，请先到全局模型管理中准备一个可用模型

## 6. 启动 Agent

启动（默认后台 daemon，适合长期运行）：

```bash
bay agent start
```

如果你当前不在已初始化的 agent 目录里，Downcity 会从已登记 agent 中提供交互式选择。

需要在当前终端前台观察日志时再用：

```bash
bay agent start --foreground
```

## 7. 验证 Agent 状态

```bash
bay agent list
bay agent status
bay agent list --running
```

说明：

- agent 端口由 console 自动分配，不建议在快速开始里固定写死端口。
- 多 agent 场景下，每个 agent 端口可能不同。

## 8. 可选：启动 Console UI

```bash
bay start --console
```

## 9. 可选：Chrome 扩展网页选区直发 Agent

如果你在使用仓库内的 `chrome-extension`：

1. 在网页中选中文本后，选区右下角会出现消息按钮，点击后输入框会在选区左下角展开。
2. 点击浏览器扩展图标会打开 Extension Popup；按 `Cmd/Ctrl + U` 可直接打开页内输入框（Inline Composer）。
3. Extension Popup 只保留极简输入框、发送按钮、本页发送历史，以及一个设置按钮。
4. Extension Popup 中 Agent 切换为极简模式；Channel Chat 会按当前 Agent 自动选择首个可用会话。
5. 点击历史记录可快速回填当前页面之前发送过的内容。
6. 输入需求后按 `Cmd/Ctrl + Enter` 或点击发送按钮提交（`Enter` 换行）。
7. 若当前没有选区，发送时会自动按页面全文模式投递。
8. 按 `Esc` 可关闭输入框。
9. 全页模式会优先抓取最像正文的 `main/article` 区块；若页面存在多个强相关主体区块，会合并正文并附带页面图片引用。

## 下一步

- [配置详解](/zh/docs/quickstart/configuration)
- [Agent 文档总览](/zh/docs/agent/overview/what-is-agent)
- [`bay skill`](/zh/docs/commands/plugin/skill)
- [`bay task`](/zh/docs/commands/plugin/task)
- [FAQ](/zh/docs/operations/faq)
