...
List runtime plugin states.

### `POST /api/plugins/runtime/control`
...
### `POST /api/plugins/runtime/command`
...
- `sessionId` should be the exact target session returned by agent. For chat sessions, this is an opaque ID managed by the chat runtime plugin (not a parseable platform key).
- Attachments are materialized by agent and injected as `<file ...>` tags before the user instruction.
- If `sessionId` maps to a chat conversation, execution is enqueued into the same chat runtime plugin queue (same behavior as platform inbound messages), and chat history is appended before execution.
