...
description: Call City from the bay with User Gate.
---

Bay clients use `User Gate`. The common paths are `client.ai.*` for AIService and `client.service(id).action(name).invoke()` for custom services.
