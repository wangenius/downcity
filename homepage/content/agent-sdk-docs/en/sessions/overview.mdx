...
Both expose `prompt()`, `stop()`, `compact()`, `subscribe()`, `messages()`, `approvals()`, and `fork()`. Local Sessions additionally provide `snapshot()` to explicitly persist their effective custom instruction; remote persistence policy belongs to the server host.
...
- Persist instruction: [`session.snapshot()`](/en/agent-sdk-docs/sessions/snapshot)
