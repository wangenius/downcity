...
import { ProductsInvoker } from "../invoker/products/index.js";
import {
  defaultFetch,
  normalizeBaseURL,
  requestJSON,
...
export class AdminClient {
  ...
  readonly products: ProductsInvoker;
  ...
  constructor(options: AdminClientOptions) {
    ...
    this.secret = requiredString(
      options.admin_secret_key ?? process.env.DOWNCITY_INFRA_ADMIN_SECRET_KEY,
      "admin_secret_key",
    );
    ...
    this.products = new ProductsInvoker({ requestJSON: req });
  }
  ...
  /** 列出当前 InfraRuntime 暴露的 service 摘要。 */
  ...
  /** 列出当前 InfraRuntime 注册的模型目录（admin 身份可见额外状态字段）。 */
  ...
  /** 读取 InfraRuntime 聚合后的说明文档。 */
  async instruction(): Promise<string> {
    return this.text("/v1/infra/instruction", { method: "GET" });
  }
