 ...
 *
 * 关键点（中文）
 * - 裸 `bay agent` 在交互式终端里进入这里，而不是只输出静态 help。
...
type DanglingChannelAccount = {
  ...
  /**
   * agent 配置中引用但 Bay 全局账号池不存在的 account id。
   */
  accountId: string;
};
...
async function emitAgentManagerList(): Promise<void> {
  ...
  if (agents.length === 0) {
    emitCliBlock({
      ...
      note: "Run `bay agent start <path>` once to register an agent with Bay.",
    });
    return;
  }
...
async function promptAgentProjectRoot(): Promise<AgentManagerAgentSummary | null> {
  ...
  if (agents.length === 0) {
    emitCliBlock({
      ...
      note: "运行 `bay agent create` 创建项目，或运行 `bay agent start <path>` 登记已有项目。",
    });
    return null;
  }
...
async function connectAgentChannels(
  ...
  if (dangling.length > 0) {
    ...
    if (allAccountsAfterCleanup.length === 0) {
      emitCliBlock({
        tone: "info",
        title: "No Bay channel accounts found",
        note: "已清理悬空关联。请先运行 `bay chat`，选择“配置 channel”来配置 Telegram、Feishu 或 QQ account；agent 这里只做 connect。",
      });
      return cleanedAgent;
    }
  ...
  if (allAccounts.length === 0) {
    emitCliBlock({
      tone: "info",
      title: "No Bay channel accounts found",
      note: "请先运行 `bay chat`，选择“配置 channel”来配置 Telegram、Feishu 或 QQ account；agent 这里只做 connect。",
    });
    return agent;
  }
