 ...
 * - agent daemon 启动成功后必须登记到 bay 后台（强约束）。
 */
...
/**
 * 确保 bay agent registry 文件存在。
 *
 * 关键点（中文）
 * - 空 Bay 运行态也应拥有显式的空 registry，避免 Console 将“尚未启动任何 agent”误判为异常。
...
/**
 * 列出 Bay registry 中记录的 agent（按 projectRoot 排序）。
 */
