...
3. When the host is `town`, Console-global env is read from `downcity.db` and passed explicitly into `Agent`.
...
sequenceDiagram
  ...
  CLI->>HOST: Pass Console-global env explicitly
...
4. Plugin config (`town plugin action ...`, `town asr ...`, or `town tts ...`): writes project `downcity.json`.
...
2. Manage shared env in Console UI `Global / Env`, or pass them explicitly through `new Agent({ env })`.
...
`town start` automatically detects the public IP and stores it in global Console Env as `DOWNCITY_PUBLIC_HOST`. Agent `contact link` generation also uses this value to create transferable contact codes.
