...
默认 `stream(ctx, call)` 使用 `createClient().chat()` 执行模型流。如果某个模型必须
使用 Responses、Claude Messages、Gemini 或其他原生协议，直接覆盖 `stream()`，
调用对应上游并返回标准 `LanguageModelV3StreamPart` 流即可。
...
ai.use(deepseek.model({
  ...
  price: [
    "输入：1 credit / 1K tokens",
    "输出：3 credits / 1K tokens",
  ],
}));
```
...
`price` 同样会通过模型目录返回，仅用于产品展示。它是 Provider 自定义的字符串列表，
不会参与实际扣费；实际扣费仍由 `bill(ctx, output)` 生成。
...
`text()` 是非流式 SDK action；`stream(ctx, call)` 是 CityModel 标准模型流协议。
Provider 可以分别覆盖。自定义 stream 不需要构造完整的 CityModel：
...
`@downcity/city` 不内置 OpenAI、DeepSeek 或其他具体图片 Provider 构造器。City 包提供 `Provider` 抽象、`AIService` 注册机制，以及 `city.ai.image_create()` / `city.ai.image_result()` 的统一任务协议。
...
调用方只使用 `city.ai.image_create()` 和 `city.ai.image_result()`。Provider 实现侧通常只需要两个方法：
...
流式 action 返回异步 `charge` 时，Worker 使用 `waitUntil` 完成结算；Node 等没有 ExecutionContext 的运行时会把结算绑定到 Response 流收尾，扣费完成后才关闭响应流。普通异步 charge 则会在请求结束前直接完成。
 ...
 | `stream` | 执行 CityModel 标准调用并返回 LanguageModelV3 stream result |
