...
import { InfraRuntime, AIService } from "../bin/index.js"
import { createSqliteDb } from "./sqlite-db.mjs"

test("InfraRuntime instruction aggregates built-in and service documentation", async () => {
  const cwd = process.cwd()
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "downcity-infra-instruction-"))
  ...
  try {
    ...
    assert.match(text, /# Downcity Infra Instruction/)
    assert.match(text, /## Env \(env\)/)
    assert.match(text, /## Products \(products\)/)
...
test("InfraRuntime instruction endpoint requires admin auth and returns text", async () => {
  const cwd = process.cwd()
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "downcity-infra-instruction-http-"))
  ...
  try {
    ...
    const guestResponse = await base.handleRequest(new Request("http://localhost/v1/infra/instruction", {
      method: "GET",
    }))
    ...
    const adminResponse = await base.handleRequest(new Request("http://localhost/v1/infra/instruction", {
      method: "GET",
      headers: {
    ...
    assert.match(await adminResponse.text(), /Downcity Infra Instruction/)
  } finally {
    process.chdir(cwd)
    await fs.rm(tempDir, { recursive: true, force: true })
  }
...
test("InfraRuntime bootstraps internal secrets into the env table", async () => {
  const cwd = process.cwd()
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "downcity-infra-env-bootstrap-"))
  ...
  try {
    ...
    assert.match(envProvider.get("DOWNCITY_INFRA_ADMIN_SECRET_KEY"), /^admin_/)
    assert.match(envProvider.get("DOWNCITY_INFRA_TOKEN_SIGNING_KEY"), /^sign_/)
...
test("InfraRuntime rejects mismatched product_id for authenticated user requests", async () => {
  const cwd = process.cwd()
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "downcity-infra-product-"))
  ...
  try {
    ...
    const productResponse = await base.handleRequest(new Request("http://localhost/v1/products/create", {
      method: "POST",
      headers: {
    ...
    assert.deepEqual(await response.json(), {
      error: {
        message: "product_id does not match the authenticated token",
        type: "server_error",
      },
    })
  } finally {
...
test("InfraRuntime exposes service env requirements and env catalog", async () => {
  const cwd = process.cwd()
...
test("InfraRuntime refreshes runtime env before each request so external env updates take effect immediately", async () => {
  const cwd = process.cwd()
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "downcity-infra-env-refresh-"))
