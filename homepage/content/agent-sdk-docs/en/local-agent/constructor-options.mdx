...
- restored Sessions use the Agent's current instruction by default; call `session.snapshot()` to persist the Session's effective instruction to `instruction.md`
