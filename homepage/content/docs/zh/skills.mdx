...
- `skills_load`：按 `id/name` 查找并返回对应 `SKILL.md` 全文

典型工作流：

1. 模型调用 `skills_list` 发现是否有合适的 skill
2. 调用 `skills_load` 读取 `SKILL.md` 原文
3. 按 `SKILL.md` 的步骤执行（必要时再调用其它工具；工具权限可能会被 `allowed-tools` 限制）

如果本地 `skills_list` 没有目标技能，推荐先走 CLI 学习流程：`find -> add -> load`。

> 说明：`skills_load` 为无状态读取，返回 `SKILL.md` 内容作为当次工具结果，不会写入会话 pin。
...
- `sma skill add <spec>`：下载/学习没学会的技能（会同步到 `~/.ship/skills`）
- `sma skill load <name>`：读取已学会技能的 `SKILL.md` 内容

推荐顺序：`find -> add -> load`。如果 `list` 已经能看到目标技能，直接 `load` 即可。
