...
如果 `AccountsService` 配置了 `local_login: true`，`/v1/accounts/providers` 会只返回本地登录方式。调用 `accounts/local/login` 并传入 `city_id` 后会得到正常 `user_token`，之后用 `city_id + user_token` 创建或更新 User City。
