...
import { AdminClient, UserClient } from "../bin/index.js"
...
test("AIInvoker.text() posts to /v1/ai/text", async () => {
  ...
  assert.deepEqual(JSON.parse(requests[0].init.body), { prompt: "hi", product_id: "prod_demo" })
})

test("AIInvoker.listModels() returns ModelCatalog", async () => {
  const client = new UserClient({
    base_url: "https://api.example.com/base/",
    product_id: "prod_demo", user_token: "ub_test",
...
test("AIInvoker.stream() returns parsed chunks", async () => {
  ...
  const client = new UserClient({
    base_url: "https://api.example.com/base/", product_id: "prod_demo", user_token: "ub_test",
...
test("UserClient.service() → ServiceInvoker", async () => {
  const requests = []
  const client = new UserClient({ base_url: "https://api.example.com/base/", product_id: "p", user_token: "t", fetch: async (url, init) => { requests.push({ url, init }); return json({ ok: true }) } })
  ...
  assert.deepEqual(JSON.parse(requests[0].init.body), { title: "hello", product_id: "p" })
})
...
test("AdminClient.products CRUD + tokens.apply", async () => {
  const requests = []; const p = { product_id: "p1", name: "Demo", status: "active", created_at: "t", updated_at: "t" }
  const admin = new AdminClient({ base_url: "http://localhost:3001/", admin_secret_key: "sk", fetch: async (url, init) => {
    requests.push({ url, init })
    if (url.endsWith("/v1/products/list")) return json({ items: [p] })
    if (url.endsWith("/v1/products/create")) return json(p)
    if (url.endsWith("/v1/products/tokens/apply")) return json({ user_token: "ub_test", product_id: "p1", user_id: "u1", expires_at: "2026-01-01T00:00:00.000Z" })
    return json({ success: true })
  }})
  assert.deepEqual(await admin.products.list(), [p])
  assert.equal((await admin.products.create({ name: "Demo" })).product_id, "p1")
  await admin.products.pause("p1")
  assert.equal(requests[2].url, "http://localhost:3001/v1/products/pause")
  assert.deepEqual(await admin.products.tokens.apply({ product_id: "p1", user_id: "u1" }), {
    user_token: "ub_test",
    product_id: "p1",
    user_id: "u1",
...
test("AdminClient.listServices() / listModels() / instruction()", async () => {
  ...
  const admin = new AdminClient({
    ...
    fetch: async (url, init) => {
      ...
      if (url.endsWith("/v1/infra/instruction")) {
        return text("InfraRuntime instruction document")
      }
      return json({ ok: true })
    },
  })
  ...
  assert.equal(await admin.instruction(), "InfraRuntime instruction document")
