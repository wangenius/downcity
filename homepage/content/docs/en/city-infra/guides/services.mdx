...
description: Add accounts, usage, and payment capabilities to InfraRuntime through package services.
---

Services package backend capabilities that many client products reuse. InfraRuntime users do not define database tables for these official services.
...
Services register their own actions and tables into InfraRuntime. The product example also registers `EnvService`, `ProductsService`, and `AIService` explicitly.
...
UserClient calls authenticated service entry points after it has `product_id + user_token`:
...
- [accounts service](/en/docs/city-infra/guides/accounts-service)
- [usage service](/en/docs/city-infra/guides/usage-service)
- [Stripe payment service](/en/docs/city-infra/guides/payment-stripe-service)
