...
import { startServer } from "@/city/modules/http/Server.js";
import { ensureAgentToken, rotateAgentTokenIfNeeded } from "@/city/runtime/auth/AgentTokenService.js";
import { applyInternalAgentAuthEnv } from "@/city/runtime/auth/AuthEnv.js";
import { startLocalRpcServer } from "@/city/modules/rpc/Server.js";
...
export async function runCommand(
  ...
  // 初始化加载（进程级单例状态：root/config/utils/logger/chat/utils/agents 等）
  await initAgentState(cwd);
  // 端口解析（中文）：允许 number/utils/string；空值返回 undefined 以便走配置回退链。
  const parsePort = (
