...
import { getTownRuntimeDirPath, getGatewayLogPath, getGatewayMetaPath, getGatewayPidPath, } from "../../../process/registry/TownPaths.js";
 ...
 * 安全读取 gateway 元数据。
 */
async function readGatewayMeta() {
    try {
        const raw = (await fs.readJson(getGatewayMetaPath()));
 ...
 * 清理 gateway 状态文件。
 */
async function cleanupGatewayStateFiles() {
    try {
 ...
 */
export function isGatewayBindingMatch(actualHost, expectedHost) {
 ...
 */
function formatGatewayRestartHint(options, port) {
 ...
 */
export function resolveGatewayHostForBinding(options) {
    const explicitHost = String(options?.host || "").trim();
    if (explicitHost)
        return explicitHost;
    if (options?.public === true)
        return PUBLIC_GATEWAY_HOST;
    return DEFAULT_GATEWAY_HOST;
}
/**
 * 解析 detached Console 命令行中的 host/port。
 */
export function parseGatewayProcessCommand(command) {
 ...
 */
export function findReusableGatewayProcess(processes, expected) {
    ...
    for (const item of processes) {
        ...
        if (parsed.port !== expectedPort)
            continue;
        if (expectedHost && !isGatewayBindingMatch(parsed.host, expectedHost))
            continue;
        return {
            pid: item.pid,
            host: parsed.host,
 ...
 * 获取 Console 当前运行状态。
 */
export async function getGatewayRuntimeStatus() {
    ...
    const port = meta && Number.isInteger(meta.port) ? meta.port : DEFAULT_GATEWAY_PORT;
    return {
        running: true,
        pid,
        host,
 ...
 */
export async function runGatewayRuntimeCommand(options) {
...
export async function startGatewayRuntimeCommand(params) {
    ...
    if (status.running) {
        ...
        const currentBindHost = normalizeBindHost(status.bindHost || status.host || DEFAULT_GATEWAY_HOST);
        const currentPort = status.port || DEFAULT_GATEWAY_PORT;
        const sameEndpoint = isGatewayBindingMatch(currentBindHost, host) && currentPort === port;
...
//# sourceMappingURL=GatewayRuntime.js.map