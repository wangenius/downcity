...
flowchart TD
  ...
  L --> M["start HTTP server and Local RPC"]
  M --> N["start services and schedule runtime"]
```
...
flowchart TD
  ...
  APPEND --> RUN["session.prompt"]
  RUN --> CORE["Executor"]
  CORE --> MODEL["model / tools / prompt / history composer / compaction"]
...
- `Session`: single-session instance shell
- `Executor`: single-session run pipeline
- `CoreEngine`: model and tool-loop execution kernel inside `Executor`
- `PluginRegistry`: plugin registration and dispatch
- `BasePlugin`: plugin instance base class

## In One Sentence

```text
The current system logic is: console manages many agents, each agent runtime is centered on AgentRuntime, and session plus plugin runtimes all hang off that runtime in clearly separated roles.
```
