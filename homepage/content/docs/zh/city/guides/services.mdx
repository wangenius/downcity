...
服务会把自己的 action 和数据表注册进 City。官方产品示例里还会显式注册 `EnvService`、`TownsService` 和 `AIService`。

## 客户端调用服务

User City 调免登录服务入口时，只需要 `city_url`：
...
User City 带上 `town_id + user_token` 后，调需要用户鉴权的服务入口：
