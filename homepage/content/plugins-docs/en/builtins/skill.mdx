...
Use the `downcity skill` shortcut for the common workflow:

```bash
downcity skill list
downcity skill find "web access"
downcity skill install web-access
downcity skill lookup web-access
```

`list` tells you what is already available locally. `find` is useful only when the skill is missing from `list`. After `install`, run `lookup` before expecting the agent to follow that skill's instructions.
...
By default, `SkillPlugin` only scans the current project's `.agents/skills`.

To opt into more sources, pass constructor options:
...
- `use` controls built-in scan sources: `project` and `home`
- `paths` adds extra scan directories, with relative paths resolved from the project root
- `ignore` hides selected skills by name, regex, or predicate function
