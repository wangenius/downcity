---
title: Chat 设计原理
description: 从当前 chat plugin runtime 的视角解释渠道消息如何进入系统、归到 session、执行并回发
---
...
- 当前 `chat` plugin runtime 到底怎么工作

先给结论：

- `chat` 不是 session runtime
- `chat` 是消息编排中心
- `chat` 左边接渠道输入，右边接 `SessionFacade`
- `chat` 自己负责入站整理、排队、调用 session、回发结果

## 一句话模型

```text
用户消息先进入 chat plugin runtime
chat plugin runtime 把消息整理并归到 session
session 内核负责真正执行
chat plugin runtime 再把结果发回聊天渠道
```

## Chat 的真实职责

`chat` plugin runtime 负责：
...
- agent 持有 `AgentRuntime`
- `AgentRuntime` 持有 `SessionFacade`
- `chat` plugin runtime 通过 `AgentContext.session` 使用 `SessionFacade`
...
flowchart LR
  ...
  subgraph CHAT["Chat Plugin Runtime"]
    C1["渠道适配"]
    C2["鉴权 / 角色解析"]
    C3["入站增强"]
    C4["sessionId 映射"]
...
- 调用 `AgentContext.session.prompt`

所以更准确的说法是：

- chat queue worker 围绕 `SessionFacade.run` 编排，而不是自己拥有执行内核
...
```text
chat service 当前的真实职责，是把渠道消息整理、鉴权、入队并桥接到 SessionFacade.run，再把执行结果回发到渠道。
```
