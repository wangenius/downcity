...
import { DEFAULT_FEDERATION_URL, DEFAULT_CITY_ID, list_federations, normalizeCityUrl, read_city_admin_secret_for_url, readCityString, read_current_city_session, readCityState, resolve_selected_federation_url, upsert_federation_profile, writeCityState, } from "../../city/shared/CityStateStore.js";
const cityUserManager = new CityUserManager();
function readString(value) {
    return readCityString(value);
}
export function read_city_admin_secret_for_federation(federation_url) {
    return read_city_admin_secret_for_url(federation_url);
}
function find_federation(input) {
    ...
    return servers.find((server) => server.name === query ||
        ...
        server.federation_url === query) ?? null;
}
...
export function emit_federation_status(options) {
...
export function emit_federation_list(options) {
    const servers = list_federations();
    if (options?.as_json === true) {
        printResult({
            asJson: true,
...
function save_federation_user_session(session) {
    const state = upsert_federation_profile(readCityState(), {
...
export function run_federation_logout_command(options) {
    const state = readCityState();
    const federation_url = resolve_selected_federation_url(state);
...
export function run_federation_leave_command(options) {
    const state = readCityState();
    const federation_url = resolve_selected_federation_url(state);
...
export async function run_interactive_federation_manager() {
    if (!process.stdin.isTTY || !process.stdout.isTTY)
        return;
    await open_city_manager_tui();
}
//# sourceMappingURL=FederationConnection.js.map