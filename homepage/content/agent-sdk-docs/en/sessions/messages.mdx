...
Every Assistant Part has a stable `part_id`. Its `sequence` is the linear position in the current Message revision. If the final AI SDK snapshot supplies a Part that was absent from the live stream, the final complete snapshot may correct provisional sequences before the Assistant completes. The canonical Part order is stable after completion. A tool state change updates the same Part; a UI should not create separate request and result rows.
...
Before an Assistant completes, the SDK atomically reconciles its Parts against the final UIMessage. A Tool missing from the live stream is inserted at its final relative position instead of being appended after the final text. A canonical Part write failure fails the Turn rather than publishing an incomplete completed history. When a client receives a complete `message` Mutation with a greater revision, it should replace the whole Message instead of retaining the older revision's Parts array.
