...
export class FileMentionProvider {
     ...
     */
    constructor(slash_commands, work_dir, fd_path = null) {
    ...
    async getSuggestions(lines, cursor_line, cursor_col, options) {
        ...
        if (should_suppress_slash_argument_completion(text_before_cursor, current_line.slice(cursor_col), options.force)) {
            return null;
        }
 ...
 */
function to_pi_slash_command(command, name) {
    return {
        name,
        description: command.description,
        getArgumentCompletions: command.get_argument_completions
            ? (argument_prefix) => command.get_argument_completions?.(argument_prefix) ?? null
            : undefined,
    };
}
/**
 ...
 */
function extract_at_prefix(text) {
    let token_start = 0;
 ...
 */
function get_fs_mention_suggestions(work_dir, at_prefix, signal) {
    if (signal.aborted) {
        return null;
    }
 ...
 */
function collect_fs_mention_candidates(work_dir, signal) {
    const result = [];
    const stack = [""];
    while (stack.length > 0 && result.length < MAX_FALLBACK_SCAN) {
        ...
        for (const entry of entries) {
            ...
            const relative_path = normalize_path(relative_dir.length === 0 ? entry.name : join(relative_dir, entry.name));
 ...
 * 按 query 对候选项打分并排序。
 */
function rank_fs_mention_candidates(candidates, query) {
 ...
 * 对单个候选项打分。
 */
function score_candidate(candidate, lower_query) {
 ...
 * 将候选项转换为自动完成项。
 */
function to_mention_item(candidate) {
 ...
 */
function should_suppress_leading_whitespace_slash_path(text_before_cursor, force) {
    if (force === true) {
        return false;
    }
 ...
 */
function should_suppress_slash_argument_completion(text_before_cursor, text_after_cursor, force) {
    if (force === true) {
        return false;
    }
    if (!text_before_cursor.startsWith("/")) {
 ...
 */
function score_tokens(tokens, text) {
    let score = 0;
 ...
 */
function format_slash_command_description(command) {
    return command.description || undefined;
}
//# sourceMappingURL=FileMentionProvider.js.map