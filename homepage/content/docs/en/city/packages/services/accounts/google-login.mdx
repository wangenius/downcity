...
If you are only using the Downcity accounts service server-side OAuth flow, this redirect URI is the critical part. You usually do not need to register your studio frontend as a JavaScript origin for this flow alone.

## The fields Google users most often get wrong

- choosing the wrong `Application type` instead of `Web application`
- putting the studio frontend URL into `Authorized redirect URIs` instead of the City callback URL
...
So the studio does not need a different post-login client flow just because the user came from Google:
...
- After deployment, write `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` into City env through Studio CLI or the Admin API
