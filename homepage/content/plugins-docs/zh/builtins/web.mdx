...
`configure` 存在于 SDK/plugin action 层，但没有暴露成 `town web configure` 快捷命令。普通 CLI 调整使用 `town web on`、`town web use`、`town web off`。
...
| `configure` | 写入配置 | SDK 或高级 plugin action 调用 |
| `install` | 安装当前 provider 对应能力 | 首次接入时 |
| `on` | 启用 plugin | 设置当前项目的 `plugins.web.enabled` |
| `off` | 关闭 plugin | 设置当前项目的 `plugins.web.enabled` |
...
- `configure` 改的是怎么用
- `on/off` 改的是当前项目是否启用这个 plugin

它们都是项目级写入：`configure` 更新 provider 选项，`on/off` 更新项目启用开关。
