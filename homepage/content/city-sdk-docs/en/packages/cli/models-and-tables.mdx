...
- inspect models exposed by City AIService
- bind the current project to a specific `execution.modelId`
- test whether a model can actually be called through City AIService
...
That is why this page fits naturally with [Store and Table](/en/city-sdk-docs/packages/city/store-and-table):
...
Models, service tables, usage events, and env data can all affect one town flow, so you often need to inspect them together.

## Read next

- For operational decision-making with the CLI, read [Operations and Debugging](/en/city-sdk-docs/packages/cli/operations-and-debug)
- For the data layer behind these tables, read [Store and Table](/en/city-sdk-docs/packages/city/store-and-table)
