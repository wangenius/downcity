...
`@downcity/city` does not ship concrete image provider factories for Luchi, 302.ai, OpenAI, Gemini, or other upstreams. The City package provides the `Provider` abstraction, `AIService` registration, and the unified `client.ai.image_create()` / `client.ai.image_result()` job contract.
