...
`plugin_call` is bound to the plugin registry of the Agent that owns the current session. In multi-Agent hosts that cache or switch Agents in one process, a session's `plugin_call` does not read another Agent's registry.
