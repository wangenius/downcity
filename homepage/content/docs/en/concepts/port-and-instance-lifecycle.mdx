...
- `bay agent start/restart` does not require users to manage ports manually.
...
- `bay plugin ...` calls plugin routes
...
- In multi-agent scenarios, prefer `bay plugin ... --agent <id>`. Plugin actions run locally against the selected project path.
- Do not keep stale `DC_CITY_PORT` in your shell profile.
- If runtime status is abnormal, run `bay agent doctor --fix` and inspect daemon logs.
