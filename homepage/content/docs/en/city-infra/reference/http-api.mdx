...
## Product Routes (common user-side routes)
...
| --- | --- | --- |
| `/v1/products/list` | `GET` | List products |
| `/v1/products/create` | `POST` | Create product |
| `/v1/products/pause` | `POST` | Pause product |
| `/v1/products/activate` | `POST` | Activate product |
| `/v1/products/remove` | `POST` | Remove product |
| `/v1/products/tokens/apply` | `POST` | Issue user_token |
...
Request:
```json
{ "product_id": "prod_xxx", "model": "deepseek-v4-flash", "prompt": "Hello" }
```
...
If the request body includes `product_id`, it must match the product bound to the `user_token`. Otherwise InfraRuntime returns `403`.
