 ...
 * - 按顺序检查，首个失败即抛 CliError（Bay running → PROFILE.md → downcity.json → binding）。
 *
...
export async function resolveProjectRootByAgentId(agentId: string): Promise<{
  ...
  if (matchedRoots.length === 0) {
    return {
      error: `Agent not found in managed agent registry: ${agentId}. Run "bay agent list" to inspect ids.`,
    };
  }
  ...
  if (!registered) {
    return {
      error:
        ...
        `Run "bay agent list" to inspect registered agents.`,
    };
  }
  return { projectRoot };
}
