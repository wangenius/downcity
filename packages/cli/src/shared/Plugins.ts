...
function listStaticCatalogEntries(): StaticCatalogEntry[] {
  ...
  const managedEntries = listPluginsWithLifecycle(plugins).map((plugin) => ({
    ...
    note: "Managed plugin. Use `studio plugin start/stop/restart/status` with an agent target for live state.",
  }));
...
async function resolveInteractivePluginName(params: {
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    emitCliBlock({
      ...
      note: "Use `studio plugin info <pluginName>` or run this command in an interactive terminal.",
    });
    return null;
  }
