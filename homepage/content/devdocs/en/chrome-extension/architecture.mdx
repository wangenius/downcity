...
`chrome-extension/` is the browser-side entry point for pushing live web context into an agent.
...
- `src/popup/`: popup UI for direct send flow
...
- `public/content-script.js`: in-page interaction layer for selected text flow
...
- Request dispatch prefers `sendBeacon` with a `keepalive fetch` fallback so the popup can close immediately after enqueue
