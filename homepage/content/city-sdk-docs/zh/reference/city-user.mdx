...
`city.ai.catalog()` 返回 `ModelCatalog`：
...
模型目录中的 `model.price` 是 AIChannel 提供的价格展示文案列表；它只用于展示，
不代表最终扣费结果。
...
- 其他字段由 `AIService` 命中的 AIChannel 标准模型流决定
- handler 返回值应该是一个 `UIMessage`

如果你要调用自定义 service，并且返回结构不是 `UIMessage`，用 `city.service(...).action(...).invoke<T>()`。
...
它不是 HTTP 原始字节流。SDK 使用 `CityModel` 请求 Federation 的
`LanguageModelV3` 模型流，再在客户端转换成 chunk 对象。
...
Federation 的 `/v1/ai/stream` 只返回标准模型事件，不负责生成 UIMessage。该转换由
`city.ai.stream()` 在客户端完成。
...
生成图片的 file part `url` 会按具体 AIChannel 返回的内容原样返回。Channel 可以返回 HTTPS URL、R2/resource URL，或 `data:image/...;base64,...` data URL。
...
图片任务的底层约定很简单：client 只发送 `prompt` / `messages` / `model` / `size` / `ratio` / `quality` / `count` / `provider_options` 这些输入；City 侧 AIChannel 创建上游任务并实现 `image_fetch()` 查询上游状态，AIService 负责 Queue 调度和把 Downcity 任务状态存入 `async_jobs`。

`video()` 仍然固定返回 AI SDK `UIMessage`。City 侧的 AIChannel `video` action 也应该返回 `UIMessage`。
...
`city.listServices()` 返回当前 City 注册的 service 摘要：
...
- `city.payment.methods()` 对应 `GET /v1/payment/methods`
- `city.payment.method("stripe").invoke(...)` 会先读取 method 定义，再自动转发到 `payment/checkout/create`，并带上 `method_id: "stripe"`
...
`city.ai.stream()` 有两个失败阶段：HTTP 状态不是 2xx 时和其他方法一样抛带
`status/body` 的错误；如果 HTTP 已经成功但响应体为空，或不是有效的 CityModel
LanguageModelV3 stream，模型流解析阶段会抛普通错误。
