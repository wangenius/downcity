...
User Gate calls guest-access service entry points with `city_url` only:
...
User Gate calls authenticated service entry points after it has `bay_id + user_token`:
