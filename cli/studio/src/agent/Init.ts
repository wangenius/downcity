/**
 * `studio agent create`：在目标目录生成最小可用的 Downcity 工程骨架与配置文件。
 *
 * 目标
...
export async function initCommand(
  ...
  if (primaryModelId) {
    ...
    nextSteps.push('Use "studio model ..." to manage global model pool');
  }
  ...
  nextSteps.push('Run "studio agent start" to start the agent');
