...
- start local RPC so another local process can call the agent through `RemoteAgent`
- start plugin lifecycles and the Agent's action scheduling loop for long-lived background work

A simple mental model is:

- no `start`: use the Agent as a local library
- `start`: turn the Agent into a long-lived runtime that can expose RPC and run background capabilities
