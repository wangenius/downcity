...
import { markManagedAgentStopped, upsertManagedAgentEntry, } from "../registry/TownRegistry.js";
 ...
 * 计算 daemon pid 文件路径。
 */
export const getDaemonPidPath = (projectRoot) => path.join(getDowncityDebugDirPath(projectRoot), DAEMON_PID_FILENAME);
/**
 * 计算 daemon 日志文件路径。
 */
export const getDaemonLogPath = (projectRoot) => path.join(getDowncityDebugDirPath(projectRoot), DAEMON_LOG_FILENAME);
/**
 * 计算 daemon 元数据文件路径。
 */
export const getDaemonMetaPath = (projectRoot) => path.join(getDowncityDebugDirPath(projectRoot), DAEMON_META_FILENAME);
/**
 * 读取 daemon pid。
 *
 * 关键点（中文）
 ...
 * - 该函数用于状态展示，不抛异常。
 */
export const readDaemonMeta = async (projectRoot) => {
    try {
 ...
 * 诊断 stale 原因。
 */
export const diagnoseDaemonStaleReasons = async (projectRoot, pid) => {
    const reasons = [];
    reasons.push({
...
async function waitForDaemonReady(params) {
    ...
    while (Date.now() - started_at < timeout_ms) {
        if (!isProcessAlive(params.pid)) {
            throw new Error(`Daemon process exited before RPC became ready (${rpc_host}:${rpc_port})`);
        }
    ...
    throw new Error(`Daemon RPC did not become ready at ${rpc_host}:${rpc_port} within ${timeout_ms}ms`);
}
/**
 * 回滚启动失败状态。
 */
