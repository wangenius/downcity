...
两者都有 `prompt()`、`stop()`、`compact()`、`subscribe()`、`messages()`、`approvals()` 和 `fork()`。模型配置边界如下：

| 场景 | 配置模型 |
| --- | --- |
| 本地 `AgentSession` | `session.set({ model })`，只接收运行时模型实例 |
| `RemoteAgentSession` | 不提供模型配置方法；远程客户端只执行 Session |
| Downcity Agent 项目 | City 保存 `execution.modelId` 和 Session 覆盖，并在每轮执行前注入模型实例 |
