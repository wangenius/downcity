...
import { assertProviderType, getSupportedProviderTypes, parseBooleanOption, parseNumberOption, parsePositiveIntegerOption, } from "./ModelCommandShared.js";
/**
 ...
 */
export async function runInteractiveModelCreateFlow(options) {
...
async function runInteractiveProviderCreate(store, asJson) {
    const providerResp = (await prompts([
        {
            type: "text",
            name: "providerId",
            message: "Provider ID",
            validate: (value) => String(value || "").trim().length > 0 ? true : "providerId 不能为空",
        },
        {
            type: "select",
            name: "providerType",
        ...
        },
        {
            ...
            name: "apiKey",
            message: "API Key",
            validate: (value) => String(value || "").trim().length > 0 ? true : "apiKey 不能为空",
        },
    ]));
...
async function runInteractiveModelCreate(store, asJson) {
    const providers = await store.listProviders();
    if (providers.length === 0) {
        throw new Error("No provider found. Please run `town model create` and choose provider first.");
    }
 ...
 * - 自动跳过已存在的模型 ID。
 */
async function runInteractiveModelDiscoverCreate(store, providers, asJson) {
    const providerResp = await prompts([
        {
            type: "select",
            name: "providerId",
    ...
    if (!discovery.ok || discovery.models.length === 0) {
        throw new Error(discovery.error || `Provider ${providerId} 未返回可用模型`);
    }
    const modelPick = (await prompts({
        type: "multiselect",
        name: "modelIds",
 ...
 */
async function runInteractiveModelManualCreate(store, providers, asJson) {
    const modelResp = (await prompts([
        {
            type: "text",
            name: "modelId",
            message: "Model ID（本地标识）",
            validate: (value) => String(value || "").trim().length > 0 ? true : "modelId 不能为空",
        },
        {
            type: "select",
            name: "providerId",
        ...
        },
        {
            ...
            validate: (value) => String(value || "").trim().length > 0 ? true : "modelName 不能为空",
        },
        {
            type: "text",
            name: "temperature",
 ...
 */
export function registerModelCreateCommand(model) {
    model
        .command("create")
