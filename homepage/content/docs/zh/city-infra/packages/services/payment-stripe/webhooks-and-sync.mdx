...
如果你没有显式配置跳转地址，服务会优先基于 `DOWNCITY_INFRA_BASE_URL`，再回退到当前请求 origin，并自动提供两张可直接访问的默认结果页：
...
这层不是同步 entitlement，也不是复制 Stripe 全量对象。
...
如果 Stripe 和 InfraRuntime 当前状态不一致，webhook 记录层和支付记录层会是非常重要的依据。
