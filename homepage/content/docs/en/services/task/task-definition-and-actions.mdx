...
Cron expressions are interpreted in the local timezone of the runtime process. If the runtime runs in `Asia/Shanghai`, `0 13 * * *` means 13:00 Beijing time every day. HTTP proxies do not change this timezone; they can only affect IP-based location detection.
