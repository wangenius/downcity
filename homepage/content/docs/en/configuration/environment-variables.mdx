...
```mermaid
sequenceDiagram
  participant CLI as Town runtime host
...
1. Prepare local Town state and verify the City connection once.
2. Manage shared env in City UI `Global / Env`, or pass them explicitly through `new Agent({ env })`.
3. Create chat accounts from the Town chat product surface.
...
Concrete workflows live in [Town CLI](/en/products-docs/town-cli), [Town City docs](/en/products-docs/town-cli/city), [Town config docs](/en/products-docs/town-cli/config), and [Town chat docs](/en/products-docs/town-cli/plugin/chat).
...
5. Validate chat routing through the Town chat product surface after changing bindings.
