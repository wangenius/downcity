...
`image_create()` 的底层约定很简单：client 只发送 `prompt` / `messages` / `model` / `size` / `ratio` / `quality` / `count` / `provider_options` 这些输入；City 侧 Provider 负责把 Luchi 异步 Job、OpenAI/302.ai images API、Gemini `generateContent` 等上游格式归一成 AI SDK `UIMessage` file parts。
