...
description: "The three hook layers, service mounting, and why they are the shared extension points of InfraRuntime."
---

A lot of shared capability in Downcity is not created by copying logic into every action. It enters InfraRuntime through hooks and services.
...
- services: add a full reusable capability set into InfraRuntime

That is why auth, usage, and payment do not become separate systems. They become extension points inside the same InfraRuntime.
...
### Services do not replace the conduit SDK

Services add capability into InfraRuntime. The product-side calling surface is still [@downcity/conduit](/en/docs/city-infra/packages/conduit).

## Read next

- For unified product-side service calls, read [@downcity/conduit](/en/docs/city-infra/packages/conduit)
- For auth capability, read [@downcity/services](/en/docs/city-infra/packages/services)
- For usage recording, read [@downcity/services](/en/docs/city-infra/packages/services)
- For entitlements and payment, read [@downcity/services](/en/docs/city-infra/packages/services)
