...
---
 ...
 POST /v1/ai/language-model/stream → CityModel 原生 LanguageModelV3 通路
...
`LanguageModelV3`。`text()`、`stream()` 和 CityModel 原生通路会复用同一个 factory，
避免不同入口选择不同上游协议。
