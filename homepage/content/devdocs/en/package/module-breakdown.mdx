...
- `Executor.ts`: execution engine for a single `sessionId`; it owns run scope, compaction retry, Composer orchestration, and step persistence.
- `core-engine/`: internal model/tool-loop core used by `Executor`.
- `composer/`: history, system, context, and compaction protocols; there is no shared runtime base class.
- `store/`: session history facts, metadata, archive, lock, and JSONL persistence.
...
Composer is not an executor or a state container here. It is an `interface`-level assembly protocol. `Executor.prepareExecuteInput()` calls `ContextComposer.compose()`, `SystemComposer.resolve()`, `CompactionComposer.run(historyStore)`, and `HistoryComposer.prepare()` to build one `SessionExecuteInput` from tools, system messages, compaction policy, and history messages. The actual model call happens inside `Executor.runCoreEngine()`. CoreEngine only maintains per-run memory state and returns the final assistant message; persisted history facts only live in `store/`.
...
- plugin author APIs: `BasePlugin`, built-in plugins, and plugin definition types.
- city runtime integration APIs: runtime startup/shutdown, server/RPC startup, plugin scheduling, project initialization, and model creation.
- cross-package protocol types: platform, daemon, RPC, auth, store, inline instant, and related control-plane contracts.

HTTP routers, sandbox runners, internal plugin runtime runners, and other implementation details are not exported from the root entrypoint.
...
6. `src/session/core-engine/CoreEngineMessageState.ts`
7. `src/executor/Executor.ts`
8. `src/plugin/core/Manager.ts`
9. `src/plugin/builtins/chat/ChatPlugin.ts`
