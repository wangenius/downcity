...
Agent / client.ai.stream()
  ...
  -> Agent SessionMutation or client-side UIMessageChunk
```

### OpenAI-compatible pathway

Used by the OpenAI SDK, curl, and other compatible clients. Downcity Agent can
use Fed models directly by receiving `CityModel` from `client.ai.listModels()`:
