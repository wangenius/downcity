...
- `base_url`
- `studio_id`，调用 AI service 或自定义 user action 时会自动注入
...
所以从产品视角看，`UserClient` 不是“只用来调模型”，而是“用户态调用 City 的统一入口”。
...
拿到 `session.user_token` 后，再创建带 `studio_id + user_token` 的 UserClient 调 AI service。
...
传 `base_url + studio_id + user_token`，适合：
...
- token 已过期
- token 签名无效
- 请求里的 `studio_id` 与 `user_token` 绑定的 studio 不一致
...
这些都属于可信环境动作，应该交给 [AdminClient](/docs/city/reference/admin-client) 或你自己的后端。
