...
Then bind it in project `downcity.json`:
...
- Downcity saves the file under `.downcity/.cache/telegram/` and passes it to the agent as a `<file ...>` block.
- For images and PDFs, runtime also best-effort injects model-readable `file parts`; the original `<file>` text protocol is still kept as fallback.

Example: send a PDF with caption: `Summarize this PDF and list action items.`

### Local STT model management

If you want to manage local voice-to-text models, use the dedicated `asr` plugin command group:

```bash
# enable and install model
city asr on SenseVoiceSmall

# inspect current asr config
city asr status
```

When the `asr` plugin is enabled and its transcription config under `plugins.asr` is ready, incoming Telegram `voice/audio` attachments are transcribed in best-effort mode and appended into the user instruction.
...
- If a user sends a new message as a Telegram reply, the referenced message body and available quote snippet are also injected into the Agent input.
- While the agent is running, Telegram shows a periodic **typing** indicator (`sendChatAction`) until the result is sent.
- If the agent sends a mixed text + attachment reply via `chat send`, Telegram delivers each piece in the actual order it appears in the source message.
...
- **No response at all**: confirm the process is running and check `.downcity/logs/<YYYY-MM-DD>.jsonl` (search for `TELEGRAM`, `telegram`, or errors).
