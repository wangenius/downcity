...
A live UI reads the complete request from `mutation.part.approval` when `session.subscribe((mutation) => {})` receives an approval-required Tool Part, then calls `session.resolve_approval(...)` after the user decides.
