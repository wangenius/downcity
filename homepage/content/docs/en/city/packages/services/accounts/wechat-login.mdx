...
If the callback domain configured in WeChat does not match the real public City domain used by the town, login usually fails immediately.

So two things must stay aligned:

- the real public City domain used by the town
...
So the town does not need a special post-login client flow just because the user came from WeChat:
...
- configuring one domain in WeChat while the town actually uses another City domain
...
- After deployment, write `WECHAT_CLIENT_ID` and `WECHAT_CLIENT_SECRET` into City env through Town CLI or the Admin API
