...
3. 当宿主是 `town` 时，会先从 `downcity.db` 读取 Console 全局 env，再显式传给 `Agent`。
...
1. 共享模型目录（`model_providers`、`models`）只来自 `downcity.db`。
...
`town start` 会自动探测公网 IP，并把结果写入全局 Console Env 的 `DOWNCITY_PUBLIC_HOST`。这个值也会被 agent 的 `contact link` 用来生成可转交的联系码。
