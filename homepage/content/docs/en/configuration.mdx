...
Complete guide to configuring ShipMyAgent via `ship.json`, `Agent.md`, and `Soul.md`.

## Overview

ShipMyAgent uses three main configuration files:
...
In a running `shipmyagent run` / `shipmyagent start` process, saving `Agent.md` triggers hot reload automatically (no restart required).
If `Soul.md` exists at project root, it is injected as an additional static system prompt and also hot-reloads on save.
...
`Soul.md` is an optional static prompt file at project root. It is loaded together with `Agent.md` and can define stable principles such as execution style, risk boundaries, and quality bar.
