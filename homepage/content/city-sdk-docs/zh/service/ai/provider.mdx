...
图片任务存储由 `AIService` 负责：它内置声明 `async_jobs` 表，Federation 初始化 Service 时会自动建表。Provider 只负责创建或启动上游任务，以及把上游最终结果转换成固定协议返回。
...
后台 queue、cron 或 admin worker 在 create 后调用 admin persist 端点：
...
class ImageProvider extends Provider {
  ...
  async image_persist(ctx: Context): Promise<AIImageProviderPersistResult> {
    // 读取 ctx.locals.ai_image_job，调用/查询上游，并返回最终结果。
    return { job_id: String(ctx.input.job_id), status: "running", poll_after_ms: 2000 };
  }
}
```

`image_result` 是可选方法。默认情况下，AIService 会直接从内置 `async_jobs` 表读取 result。只有 Provider 确实需要自定义读取结果时，才覆盖 `image_result(ctx)`。

AIService 负责 job 存储和结果持久化。Provider 只负责上游调用、上游轮询、usage 解析，以及把结果转换成 AI SDK `UIMessage` file parts。
