...
If redirect URLs are not configured explicitly, the service uses `DOWNCITY_INFRA_BASE_URL` first, then falls back to the current request origin, and exposes two built-in result pages:
...
This layer does not grant product access and does not mirror every Stripe object.
...
Other Stripe events can be ignored until the product explicitly needs a new payment flow.
...
1. did the webhook reach InfraRuntime?
...
If Stripe and InfraRuntime look inconsistent, the webhook and payment-record layers are the first sources of truth to inspect.
