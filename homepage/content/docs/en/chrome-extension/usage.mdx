...
The inline composer reads the current selection or page body and supports two paths:

- `Agent Dispatch`: keeps the original Agent + Channel Chat delivery flow
- `Model Reply`: calls the default model from settings and renders the answer on the page

The footer keeps the original `Agent / Channel` route button, and adds a separate icon-only mode button for switching between the two modes. The extension remembers your last choice.
