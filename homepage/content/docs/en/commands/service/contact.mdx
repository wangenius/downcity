...
A also stores B. If B has `DOWNCITY_PUBLIC_URL` or `DOWNCITY_PUBLIC_HOST` in global env, B sends its endpoint to A during approval and both sides create a bidirectional contact. Otherwise, A treats B as an inbound contact: A can authenticate requests from B, but it does not assume it can call B directly.
