...
```txt
Interfaces
  -> products enter the City runtime through client or operator tools
Composition
...
Many clients can share one self-deployed runtime. Product clients stay lightweight and focus on UX; the City runtime centralizes auth, model routing, runtime env, hooks, and reusable capabilities.
...
Neither one implements the server runtime itself. They both send requests into the same City runtime.

## 2. Composition: who assembles City

Running Downcity is not just `new City(...)`. There is also an assembly layer:

- `cities/node` is the Node.js + SQLite developer starter
- `cities/edge` is the Cloudflare Workers + D1 developer starter
