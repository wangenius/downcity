...
async function chooseCityModelId(): Promise<string | null> {
...
async function showCityModels(): Promise<void> {
  const choices = await listCityAiModelChoices();
  emitCliList({
    tone: "info",
...
export async function runInteractiveModelManager(): Promise<void> {
  ...
  while (true) {
    ...
    try {
      ...
      if (modelId) await bindModelToProject(modelId);
    } catch (error) {
      emitCliBlock({
        tone: "error",
