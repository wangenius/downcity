...
并带上当前 InfraRuntime 的 `redirect_uri`。

如果微信后台配置的回调域名和你当前真实对外访问的 InfraRuntime 域名不一致，用户在登录时通常会直接失败。

所以这里要保证两件事同时成立：

- 产品真正访问的 InfraRuntime 公网域名是对的
...
当微信登录完成后，accounts service 仍然会按统一方式返回 InfraRuntime `user_token`。
...
- 把网站应用登录和公众号登录混在一起
- 把网站应用登录和小程序登录混在一起
- 微信后台配置了别的域名，但产品实际访问的是另一个 InfraRuntime 域名
...
- 部署后记得通过 City Terminal / Admin API 把 `WECHAT_CLIENT_ID` 和 `WECHAT_CLIENT_SECRET` 写进 InfraRuntime env
