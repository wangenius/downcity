/**
 * `town plugin` 运行态命令共享辅助 + Agent 预检。
 *
 * 关键点（中文）
 ...
 * - 按顺序检查，首个失败即抛 CliError（Town running → PROFILE.md → downcity.json → binding）。
 *
 * @throws {CliError} 任一校验失败时抛出。
 */
export async function checkAgentPreflight(projectRoot, options) {
 ...
 * 解析正整数参数。
 */
export function parsePositiveIntOption(value, fieldName) {
 ...
 * 归一化 schedule 状态过滤参数。
 */
export function normalizeScheduledJobStatus(value) {
    ...
    throw new Error(`Invalid schedule status: ${value}. Use pending|running|succeeded|failed|cancelled.`);
}
/**
 * 解析项目根目录。
 */
export function resolveProjectRoot(pathInput) {
    ...
    if (raw === ".") {
        ...
        if (envAgentPath)
            return path.resolve(envAgentPath);
    }
 ...
 */
export async function resolveProjectRootByAgentId(agentId) {
    ...
    if (matchedRoots.length === 0) {
        return {
            error: `Agent not found in managed agent registry: ${agentId}. Run "town agent list" to inspect ids.`,
        };
    }
...
export async function resolvePluginProjectRoot(options) {
    ...
    const entries = await listManagedAgentEntries();
    const registered = entries.some((entry) => isRegistryEntryRunning(entry) &&
        path.resolve(String(entry.projectRoot || "").trim() || ".") === projectRoot);
    if (!registered) {
        return {
            error: `Agent is not registered in managed agent registry: ${projectRoot}. ` +
                `Run "town agent list" to inspect registered agents.`,
        };
    }
    return { projectRoot };
}
 ...
 */
export async function resolvePluginScheduleProjectRoot(options) {
    const explicitAgent = String(options.agent || "").trim();
    if (explicitAgent) {
        return resolveProjectRootByAgentId(explicitAgent);
    }
 ...
 */
export function validateAgentProjectRoot(projectRoot) {
    const missing = [];
 ...
 * 注入 plugin 目标解析通用选项。
 */
export function addPluginTargetOptions(command) {
    return command
 ...
 */
export function addPluginScheduleOptions(command) {
    return command
        .option("--path <path>", "项目根目录（默认当前目录）", ".")
        .option("--agent <id>", "agent id（从 managed agent registry 解析）")
        .option("--json [enabled]", "以 JSON 输出", parseBoolean, true);
}
//# sourceMappingURL=PluginTargetSupport.js.map