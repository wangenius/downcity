...
Use the interactive `bay chat` menu and choose "configure channel → configure authorization", or use the CLI directly:

```bash
bay chat auth set telegram:12345678
```
...
When a chat user is denied, the denial message includes a command like `bay chat auth set telegram:12345678` that they can forward to an admin.
