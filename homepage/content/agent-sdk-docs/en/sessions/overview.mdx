...
Both expose `prompt()`, `stop()`, `compact()`, `subscribe()`, `messages()`, `approvals()`, and `fork()`. Local Sessions additionally provide `syncshot()` to regenerate the system and `snapshot()` to explicitly persist it; remote refresh and persistence policy belongs to the server host.
