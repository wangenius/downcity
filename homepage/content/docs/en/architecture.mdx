...
- Per request, the runtime builds static **system** prompts from `PROFILE.md` (and optional `SOUL.md` / `USER.md`) plus `DefaultPrompt`
...
shipmyagent agent on --daemon --project /path/repo1 --port 3000 &
shipmyagent agent on --daemon --project /path/repo2 --port 3001 &
shipmyagent agent on --daemon --project /path/repo3 --port 3002 &
```
