...
async function promptAuthMethod(city_url: string): Promise<TownCityAuthMethod | null> {
  ...
  })) as { method?: TownCityAuthMethod };
  return response.method ?? null;
}

async function emailLogin(input: TownCityLoginInput): Promise<TownCityUserSession | null> {
  const response = (await prompts([
    {
      type: "text",
      name: "email",
...
async function emailRegister(input: TownCityLoginInput): Promise<TownCityUserSession | null> {
  const response = (await prompts([
    {
      type: "text",
      name: "email",
...
function buildUserSession(input: TownCityLoginInput & {
  user_token: string;
  user_id?: string;
  user_label?: string;
}): TownCityUserSession {
...
async function buildVerifiedUserSession(input: TownCityLoginInput & {
  user_token: string;
  user_id?: string;
  user_label?: string;
}): Promise<TownCityUserSession> {
...
async function readUserSessionFromToken(input: TownCityLoginInput & {
  user_token: string;
}): Promise<{
  user_id?: string;
  user_label?: string;
