...
- no dependency on `cli/city/*` internal source paths
...
- `cli/downcity/src/city/shared/FederationConnection.ts`
- `packages/ui/src`
...
gateway returns agent list
  -> host app adapter maps response into snapshot
...
In the host app, keep interaction logic inside queries, mutations, and hooks instead of moving it into UI primitives.
...
- keep main-package interaction in the host app
