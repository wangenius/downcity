...
description: How Downcity keeps agent context explicit, persisted, and inspectable
---
...
3. `.downcity/agents/<agentId>/sessions/<sessionId>/messages/messages.jsonl` session history
4. `.downcity/memory/MEMORY.md` long-term memory
5. `.downcity/memory/daily/<YYYY-MM-DD>.md` daily memory increments
6. `.downcity/agents/<agentId>/sessions/<sessionId>/memory/working.md` session working memory (optional)
...
- Memory retrieval is budgeted via `memory.search -> memory.read`, instead of full-file injection.
