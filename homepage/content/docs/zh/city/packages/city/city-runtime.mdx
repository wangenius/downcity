---
title: City 运行时
description: "City 实例、runtime、router、serve 和统一入口在 @downcity/city 里的角色。"
---
...
- 服务挂载点
- HTTP 路由入口
- token、studio、env 等基础设施

所以你通常不是“调用一下 City”，而是“启动一个 City，然后让多个 client 长期连接它”。
...
- 你只想知道产品前端怎么调 City：优先读 [@downcity/gate](/docs/city/packages/gate)
- 你只想知道账号登录怎么跑通：优先读 [@downcity/services](/docs/city/packages/services)
- 你只想看某个 custom service 怎么写：优先读 [Service 与 Action](/docs/city/packages/city/service-and-action)
...
这就是为什么同一个 City 模型可以复用到不同宿主环境，而不是每换一个平台就重写一套后端。

## 常用 API / 入口

这一页最相关的 City API 是：

- `new City({ db })`：创建 City 实例
...
如果你是从“怎么写能力”进入，下一页更关键的是 [Service 与 Action](/docs/city/packages/city/service-and-action)。

## 常见误解

### City 不是一个 conduit SDK

City 是服务端长期运行容器。产品侧真正调用它用的是 [@downcity/gate](/docs/city/packages/gate)。
...
- 想知道 City 里到底挂的是什么，读 [Service 与 Action](/docs/city/packages/city/service-and-action)
- 想知道服务和扩展点怎么进去，读 [Hook 与服务挂载](/docs/city/packages/city/hooks-and-service)
- 想知道数据层怎么落地，读 [Store 与 Table](/docs/city/packages/city/store-and-table)
- 想看更底层 API，再读 [City API](/docs/city/reference/city-runtime)
