...
### Current execution input: text only

The current Feishu adapter only sends **text messages** into execution:

- `message_type === text`: execute
- any other type: send an error message back

So unlike Telegram, this adapter does **not** currently inject Feishu files/images/audio as executable inbound attachments.
