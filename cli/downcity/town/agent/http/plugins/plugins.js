...
import { controlPluginState, listPluginStates, } from "@downcity/agent/internal/plugin/core/PluginStateController.js";
...
export function createPluginsRouter(options) {
    ...
    router.post("/api/plugins/availability", async (c) => {
        ...
        const availability = await options.getAgentContext().plugins.availability(pluginName);
        return c.json({
            success: true,
            pluginName,
            availability,
