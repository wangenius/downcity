 ...
 */
export async function reloadAgentSummary(projectRoot, fallback) {
...
export function readAgentExecutionBinding(config) {
    return String(config?.execution?.type === "api" ? config.execution.modelId || "" : "").trim();
}
export function readAgentChannelSummaries(config) {
...
export function findDanglingChannelAccounts(config) {
    const accountsById = loadChannelAccountMap();
    const channels = config?.plugins?.chat?.channels || {};
    const dangling = [];
    for (const channel of CHAT_CHANNELS) {
...
export function loadChannelAccounts(channel) {
...
export function loadChannelAccountMap() {
...
export function startActionChoices(agent) {
...
export function stopAndRestartActionChoices(agent) {
    if (agent.status !== "running") {
        return [];
    }
    return [
...
export async function promptAgentConfigAction(agent) {
    const response = (await prompts({
        type: "select",
        name: "action",
        message: t({
...
export async function promptCreateProjectPath() {
    const response = (await prompts({
        type: "text",
        name: "projectPath",
...
export async function startAgentProject(projectRoot) {
    const options = {};
...
export async function configureAgentId(agent) {
    const response = (await prompts({
        type: "text",
        name: "id",
...
export function buildAccountTitle(account) {
...
export async function connectAgentChannels(agent) {
    const shipJsonPath = getDowncityJsonPath(agent.projectRoot);
    const raw = fs.readJsonSync(shipJsonPath);
