...
The current Bay runtime endpoint inherited by nested `bay` invocations.

They are mainly used when an agent shell or tool launches another `bay` command and that child process needs to route back to the current runtime.

## `DC_AGENT_ID`

The current agent runtime id.

Current Bay runtime uses the same resolved id value that is passed into `new Agent({ id })`.
