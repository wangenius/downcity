---
title: 怎么使用 Downcity
description: 从安装到日常使用，快速跑通 Town、Agent、Console、Plugin、SDK 与 City
---
...
最常用的是本机 Town 路线：安装 `downcity`，启动 `town`，在你的代码仓库里创建 agent，然后通过 CLI 或 Console 与它对话。需要嵌入到应用时，再使用 `@downcity/agent` SDK。需要多人、产品或线上服务复用模型和服务能力时，再部署 City。

## 你会用到什么

| 入口 | 什么时候用 |
| --- | --- |
| `town` | 本机启动和管理 Agent、Console、模型池、插件和任务。 |
...
日常本机使用主要靠 `town`。`city` 主要用于部署或管理共享服务城市。
...
SDK 模式下，模型由你的宿主应用创建并注入。`@downcity/agent` 负责 Agent runtime，不负责解析 Town 全局模型池里的 `modelId`。
...
2. 添加模型
3. 签发 user token
4. 在 client 侧通过 User City 调用 City
...
4. `town model test model <modelId>`：确认模型真实可调用
...
- [City 快速开始](/zh/city-sdk-docs/quickstart)
