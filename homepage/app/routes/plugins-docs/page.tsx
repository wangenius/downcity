...
function findFirstChildDocUrl(lang: "en" | "zh", slugs: string[]) {
  ...
  const tree = pluginsDocsSource.getPageTree(lang) as PageTreeNode | undefined;
  if (!tree) return undefined;

  const urls: string[] = [];
  collectPageUrls(tree, urls);

  const prefix = `/${lang}/plugins-docs/${slugs.join("/")}/`;
...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  const page = pluginsDocsSource.getPage(cleanSlugs, lang);
  if (!page) {
