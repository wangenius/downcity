        ...
        switch (this.entry.kind) {
            ...
            case "tool-approval-result": {
                ...
                const mark = is_approved
                    ...
                    : current_theme.fg("error", FAILURE_MARK);
                const decision_text = current_theme.bold_fg(is_approved ? "success" : "error", this.entry.decision);
 ...
 */
function extract_key_argument(tool_name, args) {
    ...
    for (const key of candidates) {
        ...
        if (typeof value === "string" && value.length > 0) {
            ...
            const display = lower_name.includes("shell") && value.includes("\n")
                ? `${first_line}…`
                : first_line;
 ...
 */
function truncate_arg_value(key, value) {
