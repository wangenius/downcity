...
Use the `town asr` shortcut for normal CLI work:

```bash
town asr status
town asr models
town asr install SenseVoiceSmall --active-model SenseVoiceSmall
town asr on SenseVoiceSmall --active-model SenseVoiceSmall
town asr use SenseVoiceSmall
town asr transcribe ./voice.wav --language zh
town asr doctor
town asr off
```

`configure` exists as an SDK/plugin action, but it is not exposed as a `town asr configure` shortcut. Use `town asr on`, `town asr use`, and `town asr off` for the normal CLI path.
...
| `configure` | update ASR config | SDK or advanced plugin action calls |
...
If your main concern is “capability that participates in the message flow,” a plugin is often the right boundary.
