...
> `downcity` exposes `bay` for local agents and `city` for City administration.
>
> If `bay` is not found after install, see [FAQ](/en/docs/operations/faq).
>
> To upgrade later, run `bay update`. It upgrades the package you originally installed.

## 2. Initialize Bay (first time only)
...
> `bay agent start` requires console to be running first.
...
- `modelId` points to a model in the Bay global model pool
