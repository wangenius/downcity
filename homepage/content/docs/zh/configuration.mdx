...
通过 `ship.json`、`Agent.md` 和 `Soul.md` 配置 ShipMyAgent 的完整指南。

## 概述

ShipMyAgent 使用三个主要配置文件：
...
在运行中的 `shipmyagent run` / `shipmyagent start` 进程里，保存 `Agent.md` 后会自动热重载，无需重启服务。
如果项目根目录存在 `Soul.md`，会作为额外静态 system prompt 一并注入，并在保存时自动热重载。
...
`Soul.md` 是项目根目录下的可选静态提示词文件。它会与 `Agent.md` 一起加载，用于定义更稳定的执行原则，例如工作风格、风险边界和质量标准。
