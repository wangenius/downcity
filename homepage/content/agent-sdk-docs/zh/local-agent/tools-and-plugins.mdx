...
`image_result` 默认会等待任务进入终态。如果想用单次调用，也可以继续使用 `generate`：
...
`ImagePlugin` 不直接绑定 City，也不关心 OpenAI、DeepSeek 等 provider 的具体协议。传入 `image_create` 和 `image_result` 任务函数后，插件 action 会自己轮询，直到任务返回 AI SDK `UIMessage`。
