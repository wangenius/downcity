...
1. `memoryAgent` 是从属于 `memory runtime` 的
...
```mermaid
flowchart LR
  DAY["白天<br/>Session 持续执行"] --> CAP["memory runtime 轻量 capture"]
  CAP --> NIGHT["夜里 / 空闲时"]
...
```text
memoryAgent 不是第二个主执行体，而是 memory runtime 内部负责把长期状态慢慢整理好的后台角色。
```
