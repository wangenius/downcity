---
title: "Creem Payment Service"
description: "Turn one-time Creem Checkout payments into global balance topups."
---

`creemPaymentService()` adds a hosted Creem Checkout flow that completes an existing `balance` topup after Creem confirms payment through webhook.

It is not a subscription or entitlement service. The service only owns Checkout creation, local payment records, webhook event records, and the final call to `balance.finishTopup()`.

## Enable it

```ts
import { balanceService } from "@downcity/services";
import {
  creemPaymentMethod,
  creemPaymentService,
  paymentService,
  type CreemPaymentServiceBalanceBridge,
} from "@downcity/services";

const balance = balanceService();
const creemBalanceBridge: CreemPaymentServiceBalanceBridge = {
  readTopup: async (topup_id) => await balance.readTopup(topup_id),
  finishTopup: async (topup_id, extra) => await balance.finishTopup(topup_id, extra),
};

base.use(balance);
base.use(paymentService({
  methods: [
    creemPaymentMethod(),
  ],
}));
base.use(creemPaymentService({
  balance: creemBalanceBridge,
}));
```

## Required env

- `CREEM_API_KEY`: API key used to create Checkout sessions
- `CREEM_PRODUCT_ID`: Creem product used for the hosted Checkout page
- `CREEM_WEBHOOK_SECRET`: optional webhook signing secret for `creem-signature`
- `CREEM_SUCCESS_URL`: optional default success redirect
- `CREEM_CURRENCY`: optional display currency for payment method discovery
- `CREEM_API_BASE_URL`: optional API base URL override for tests

If the success redirect URL is not configured directly, the service derives a default redirect page from `DOWNCITY_CITY_BASE_URL` first and then falls back to the current request origin.

## Frontend flow

Create a trusted topup first through `balance`, then create a Creem checkout for it:

```ts
const checkout = await user.service("payment.creem").action("checkout/create").invoke({
  topup_id: "topup_demo",
});

location.href = checkout.checkout_url;
```

`paymentService()` also exposes Creem through:

```ts
const methods = await guest.service("payment").get("methods");
```

The Creem method is enabled only when both `CREEM_API_KEY` and `CREEM_PRODUCT_ID` are configured.

## Webhook sync

Configure Creem to send webhook events to:

```txt
POST /v1/payment.creem/webhook
```

The service applies `checkout.completed` once per event id. Repeated events return the stored sync status and do not credit balance twice.

## Routes

- `GET /v1/payment/methods`
- `POST /v1/payment.creem/checkout/create`
- `GET /v1/payment.creem/payments/me`
- `GET /v1/payment.creem/payments`
- `GET /v1/payment.creem/events`
- `POST /v1/payment.creem/webhook`
- `GET /v1/payment.creem/redirect/success`
- `GET /v1/payment.creem/redirect/cancel`
