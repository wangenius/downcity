...
- `session/` owns the execution axis
...
- model creation
- `SessionExecutorRegistry` and `SessionFacade` creation
...
- the unified execution surface derived from `AgentRuntime`
...
- runtime to history store creation linkage
...
- `AgentRuntime`, `AgentContext`, `Service`, and `Plugin` depend on it
...
1. `src/main/index.ts`
2. `src/agent/AgentRuntime.ts`
3. `src/agent/AgentContext.ts`
4. `src/session/SessionFacade.ts`
5. `src/session/execution/SessionExecutorRegistry.ts`
6. `src/session/execution/local/SessionEngine.ts`
