...
事件处理采用带租约的 `processing` 状态。同一个 `event_id` 的并发请求只有一个会执行同步；处理失败会记录为 `failed`，Stripe 后续重发同一事件时会重新处理。进程在处理中退出时，租约过期后也可以由下一次重发接管，不需要手工删除事件记录。

如果 webhook 比本地 payment 回填更早到达，事件会保留为 `pending`，而不是写成永久 `ignored`。Stripe 重发同一 `event_id` 后会再次匹配并完成同步。
