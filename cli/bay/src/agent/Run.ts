 ...
 *
 * 场景
 * - `bay agent start --foreground` 走这里（当前终端前台运行）
 * - daemon 子进程也复用这里作为真正运行入口
 *
 * 说明
 * - 后台常驻启动请使用 `bay agent start`，并用
 *   `bay agent restart` 管理。
 */
 ...
 * - 启动 agent 本机 RPC 与 Bay 托管的 HTTP gateway（双端口）
...
export async function runCommand(
  ...
  const shutdown = async (signal: string) => {
    ...
    agentLogger.info("👋 Downcity bay stopped");
    process.exit(0);
  };
