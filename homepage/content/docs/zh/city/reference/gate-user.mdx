...
- `city_url`
- `bay_id`，调用 AI service 或自定义 user action 时会自动注入
...
拿到 `session.user_token` 后，再创建带 `bay_id + user_token` 的 User Gate 调 AI service。
...
传 `city_url + bay_id + user_token`，适合：
...
- token 已过期
- token 签名无效
- 请求里的 `bay_id` 与 `user_token` 绑定的 bay 不一致
