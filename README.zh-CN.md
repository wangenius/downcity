...
| `cities/*` | 面向开发者的 City 快捷示例，用于组装 Node 或 Edge 运行形态；官方私有部署实现不放在这个仓库。 |
| `products/chrome-extension` | Chrome Extension，用于 Agent Session 侧栏对话、页面上下文抓取和可选 IM 转发。 |
...
- 产品表层：Console UI、Chrome Extension、Agent SDK、City SDK 和 UI SDK。
...
`cities/*` 保留为开发者快捷示例，不代表官方私有生产部署实现。
