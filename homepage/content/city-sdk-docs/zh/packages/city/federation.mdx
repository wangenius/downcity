...
如果 Federation 只需要被同一台机器上的可信进程访问，可以用 `@downcity/server` 暴露本机 RPC：

```ts
import { Federation } from "@downcity/city";
import { FederationRPC } from "@downcity/server";

const base = new Federation({ db });

const rpc = new FederationRPC(base);
const binding = await rpc.listen({
  host: "127.0.0.1",
  port: 15315,
});

console.log(binding.url); // rpc://127.0.0.1:15315
```

然后 `City` 可以直接连接这个 RPC 地址：
...
本机 RPC 是管理侧可信 transport：`Admin City` 不需要 `admin_secret_key`。`User City` 和 HTTP 保持同一套产品侧模型：公开 action 可以作为 guest 调用，需要用户身份的 action 仍然必须携带正常的 `user_token`。

如果安装了 `AccountsService`，RPC 请求 `/v1/accounts/providers` 只会返回本地登录方式。调用 `accounts/local/login` 并传入 `city_id` 后会得到正常 `user_token`，之后用 `city_id + user_token` 创建或更新 User City。
...
- `new FederationRPC(base)`：把当前 Federation 暴露为本机可信 RPC
