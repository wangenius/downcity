 ...
 */
export function ensurePlatformStoreSchema(context) {
    context.sqlite.exec(`
 ...
 */
function ensureAuthSchema(context) {
    context.sqlite.exec(`
 ...
 * 补齐 models 表的增量列。
 */
function ensureModelsTableColumns(context) {
    ...
    if (!columns.has("is_paused")) {
        context.sqlite.exec("ALTER TABLE models ADD COLUMN is_paused INTEGER NOT NULL DEFAULT 0;");
    }
}
/**
 * 补齐 channel_accounts 表的增量列。
 */
function ensureChannelAccountsTableColumns(context) {
    const rows = context.sqlite
 ...
 */
function ensureEnvEntriesColumns(context) {
    const envEntryColumns = context.sqlite
        .prepare("PRAGMA table_info(env_entries)")
        .all();
    const envEntryColumnNames = new Set(envEntryColumns.map((row) => String(row.name || "").trim()).filter(Boolean));
