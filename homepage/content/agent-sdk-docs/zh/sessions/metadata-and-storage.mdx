...
`instruction.md` 是可选文件。默认情况下 Session 不持久化 instruction，恢复时使用 Agent 当前 instruction。只有调用本地 `session.snapshot()` 后，SDK 才会原子写入当前 Session 生效的自定义 instruction。删除该文件会恢复默认行为；SDK core、plugin 和 Session context 不会写入该文件。
