...
除了 `studios.*` 和 `env.*`，`Admin Gate` 也可以调用 service 暴露出来的管理侧 service。
...
也就是说，`Admin Gate` 不只是在调内建 studios/env 接口，它同样是可信侧调用统一 service 路由的入口。
...
也就是说，`Admin Gate` 负责维护 studio、token 和 env；模型定义和 service 挂载方式仍然属于 `City` 运行时层。
