...
> `town agent start` requires console to be running first.
...
The current project agent binds to a model ID exposed by the shared AI service.
...
- `modelId` points to a model available through `town model list`
- the project stores the selected model under `execution.modelId`
- if you have not connected shared models yet, do that before starting the agent
