/**
 * CreateRuntimeModel：Bay 宿主侧 LanguageModel 工厂。
 *
 * 关键点（中文）
 * - `@downcity/agent` 只消费 `LanguageModel`，不再负责模型池解析。
 * - Bay 负责把 `execution.modelId` 解析成平台模型池中的 provider/model 配置。
...
function createRuntimeModelLoggingFetch(args: {
  ...
  return async (input, init) => {
    ...
    try {
      ...
      if (args.enabled) {
        void logger.log("info", "[bay] llm.fetch", {
          kind: "llm_fetch",
          url,
          method,
      ...
      if (args.enabled) {
        void logger.log("error", "[bay] llm.fetch.error", {
 ...
 * 2) 从 Bay 平台模型池解析 provider/model。
