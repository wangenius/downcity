 ...
 *
 * 关键点（中文）
 * - `town` 是本机 Agent 宿主环境入口；
 * - 本机 runtime、Console、模型、配置、agent 与插件相关命令都应通过 town 执行。
 */
...
const QUICK_COMMAND_GROUPS: Array<{ label: string; items: QuickCommandItem[] }> = [
  {
    label: "Town Runtime",
    items: [
      { label: "Version", command: "town -v", description: "查看 Town CLI 版本" },
      { label: "Town Status", command: "status", description: "查看 town 后台、Console 与托管 agent 状态" },
      { label: "Town Start", command: "start", description: "启动本机 Town runtime" },
      { label: "Town Start All", command: "start --all", description: "启动 Town、Console 与公网模式" },
      { label: "Town Restart", command: "restart", description: "重启 Town 后台并恢复已运行 agent" },
      { label: "Town Stop", command: "stop", description: "停止 Console、Town 后台与受管 agent" },
    ],
  },
  {
    label: "Console",
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Model", command: "model", description: "打开 Town 全局模型池管理器" },
      { label: "Token", command: "token", description: "管理本机 Bearer Token" },
      { label: "Env", command: "env", description: "管理平台 Env key" },
      { label: "Town Init", command: "init", description: "初始化 Town 全局配置" },
      { label: "Town Update", command: "update", description: "更新全局 downcity CLI 聚合包" },
    ],
  },
  {
    label: "City Admin",
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Chat Send", command: "chat send --text \"hello from town\"", description: "向当前 chatKey 发送消息" },
    ],
  },
  {
    label: "Task & Memory",
  ...
  },
  {
    ...
    items: [
      ...
      { label: "TTS Synthesize", command: "tts synthesize \"hello from town\"", description: "生成可发送的语音文件" },
    ],
  },
]
...
export function AgentCommandSection(props: AgentCommandSectionProps) {
  ...
  /**
   * 解析最终执行命令（中文）
   * - 已显式输入 `city|town ...`：保持原样。
   * - 首 token 是 Town 已知命令/全局参数（`-v`）：自动补 `town`。
  ...
  const resolveExecutionCommand = React.useCallback((commandTextInput: string): string => {
    ...
    if (!raw) return ""
    if (/^(?:city|town)(?:\s|$)/i.test(raw)) return raw
    const first_token = String(raw.split(/\s+/, 1)[0] || "").trim().toLowerCase()
    if (!first_token) return ""
    if (first_token.startsWith("-") || BAY_COMMAND_ROOTS.has(first_token)) return `town ${raw}`
          ...
          <div className="relative flex h-9 flex-1 items-center rounded-[14px] bg-background px-3">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="输入命令后按回车执行（city / town 可自动补全）"
