...
description: How to configure provider API keys in the InfraRuntime environment.
---

All provider API keys are managed through InfraRuntime's own env table.
...
`ctx.env(key)` reads from InfraRuntime's active env table. AIService automatic passthrough also reads the API key through `ctx.env(envKey)`.

See also [HTTP API - admin env management](/en/docs/city-infra/reference/http-api#admin-request-format).
