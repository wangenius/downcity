 ...
 *
 * 关键点（中文）
 * - 负责把需要运行中 agent 承载的 plugin actions 挂到 commander（`bay <plugin> <action>`）。
 * - 仅处理 CLI 参数映射与远程调用，不承载 plugin 状态机逻辑。
 * - 命令注册表与调度时间解析统一复用 agent 包实现，避免 Bay 维护第二套事实源。
 */
...
const CHAT_PLUGIN_HELP_TEXT = [
  ...
  "  跨 chat 发送请使用 `bay chat send --chat-key <chatKey>`。",
  "  如果要发正文/附件/定时消息，先看 `bay chat send --help` 与 `bay chat react --help`。",
  "",
  "Common examples:",
  "  bay chat send --text 'done'",
  "  bay chat send --chat-key <chatKey> --text 'done'",
  "  bay chat react --message-id <messageId> --emoji '✅'",
  "  bay chat context",
  "  bay chat history --limit 30",
].join("\n");
