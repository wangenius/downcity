...
> get `town_id + user_token` into the town frontend, then enter normal `User City` calls.
...
It is simply a `User City` with `city_url` only:
...
At that point the town enters real user context:
