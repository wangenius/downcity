...
When the `voice` plugin is enabled and its transcription config under `plugins.voice` is ready, incoming Telegram `voice/audio` attachments are transcribed in best-effort mode and appended into the user instruction.
