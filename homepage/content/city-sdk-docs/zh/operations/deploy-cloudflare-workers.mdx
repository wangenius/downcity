...
`type` 表示这是一个 City 项目，`target` 表示部署目标。当前支持 `cloudflare-workers`，也就是部署到 Cloudflare Edge Worker。`entry` 和 `resources.d1` 是可提交的部署意图；D1 database id、Cloudflare account、Worker URL 和 wrangler 配置都由 `city deploy` 在部署时处理，不写进 `city.json`。
...
D1 名称写在 `federation.json` 的 `resources.d1.name`：
...
如果没有显式配置，`city deploy` 会根据 `city.json.name` 使用默认 D1 名称 `${name}-db`。部署时会复用同名 D1；没有同名 D1 且不是 `--dry-run` 时会自动创建。D1 database id 是 Cloudflare 部署状态，不会写回项目配置。
...
- 对 `cloudflare-workers` target 按 `resources.d1.name` 查找 D1；不存在时自动创建
...
部署输出会按阶段汇总显示：`Project`、`Version`、`Build`、`Typecheck`、`D1 Database`、`Wrangler`、`Deployment` 和 `Active Server`。

`city.json` 只保留可提交的项目声明。业务密钥、provider keys 和 service env 应该写入 City 自己的 env 表，不写入 `city.json`。
...
部署完成后，请求一次 `/health`。这会让 Worker 启动 City，并创建内置的 `env` / `cities` 表。
...
City 首次启动时会把 `DOWNCITY_FEDERATION_ADMIN_SECRET_KEY` 写入自己的 `env` 表。Workers + D1 部署下，可以从远程 D1 查询：
...
请把这个 key 存在可信位置。它是 `Admin City`、City CLI 或直接调用 admin HTTP 接口时使用的管理凭证。
