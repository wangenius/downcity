...
`city agent start` then assembles one project into a running agent host. That process reads project config, prepares model binding, creates the session layer, registers plugins, and starts managed runtimes when needed.
