...
When a request contains image input and the current model does not support it, AIService switches to `fallback.image` automatically for `text`, `stream`, and `chat/completions`.
If no fallback is configured, AIService keeps the original model and the provider decides how to handle the request.
