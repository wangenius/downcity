...
`configure` 存在于 SDK/plugin action 层，但没有暴露成 `town asr configure` 快捷命令。普通 CLI 路径使用 `town asr on`、`town asr use`、`town asr off`。
...
| `configure` | 更新 ASR 配置 | SDK 或高级 plugin action 调用 |
