...
async function applyPublicModeSetting(
  ...
  if (status.running !== true) {
    emitCliBlock({
      ...
      note: "配置已保存，将在下次 `city start` / `city console start` 时自动生效。",
    });
    return;
  }
...
export async function controlPlanePublicCommand(params: {
  ...
  emitCliList({
    ...
    items: [
      { title: "Use `city public` for interactive mode" },
      { title: "Use `city public status`" },
      { title: "Use `city public on [--host <host>]`" },
      { title: "Use `city public off`" },
    ],
  });
}
