...
`image_create()` 会在任务提交后立即返回。后台 worker 或 admin 进程应该调用 `/v1/ai/image/persist`；AIService 会把最终结果写入内置 `async_jobs` 表，`image_result()` 只读取已经保存的任务状态。
...
图片任务的底层约定很简单：client 只发送 `prompt` / `messages` / `model` / `size` / `ratio` / `quality` / `count` / `provider_options` 这些输入；City 侧 Provider 通过自己的存储完成创建、持久化和查询。
