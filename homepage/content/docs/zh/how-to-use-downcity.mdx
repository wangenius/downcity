...
最常用的是本机 Town 路线：安装 `downcity`，启动 `town`，在你的代码仓库里创建 agent，然后通过 CLI 或 Console 与它对话。需要嵌入到应用时，再使用 `@downcity/agent` SDK。需要多个产品或线上服务复用模型、账号、usage、支付和服务能力时，再接入共享服务 runtime。

## 你会用到什么

| 入口 | 什么时候用 |
| --- | --- |
| `town` | 本机启动和管理 Agent、Console、模型绑定、插件和任务。 |
| `city` | 管理已部署的共享服务 runtime、模型、token 和服务资源。 |
| `@downcity/agent` | 在 Node 应用里嵌入本地 Agent，或调用远程 Agent。 |
| `@downcity/city` | 构建可部署的共享服务 runtime，让多个产品或 Town 复用后端能力。 |
...
日常本机使用主要靠 `town`。`city` 主要用于部署或管理共享服务 runtime。
...
Agent 启动前需要绑定一个共享模型。最简单的方式是进入交互式模型选择器：
...
SDK 模式下，模型由你的宿主应用创建并注入。`@downcity/agent` 负责 Agent runtime，不负责解析 Town 共享模型目录里的 `modelId`。
...
只在本机使用 Agent 时，通常不需要先部署共享服务 runtime。

当你需要下面这些能力时，再考虑共享服务 SDK：

- 多个 Town 或多个产品复用同一套模型目录
- 给终端用户签发 `user_token`
- 统一处理模型调用、用量、余额、支付和账号
- 把 Downcity 能力接入正式产品后端
- 在 Node 或 Edge 环境里部署共享服务 runtime

共享服务 SDK 当前由 `@downcity/city` 提供，分成三侧：
...
### 10.2 服务端创建共享服务 runtime

下面是一个最小可运行的本地共享服务 runtime。它使用 SQLite，并注册一个 echo 模型，方便你先把调用链跑通：
...
如果你已经有 OpenAI SDK，也可以直接调用共享服务 runtime 的 OpenAI 兼容端点：
...
2. 添加模型
3. 签发 user token
4. 在 client 侧通过 User City 调用共享服务

对应文档：

- [创建共享服务 runtime](/zh/city-sdk-docs/quickstart/create-city)
...
3. `town model list`：确认绑定模型是否存在并启用
4. `town model test <modelId>`：确认模型真实可调用
...
- [共享服务 SDK 快速开始](/zh/city-sdk-docs/quickstart)
