...
## Scenario 3: Local RPC

When Federation only needs to be called by trusted processes on the same machine, expose it with `FederationRPC` from `@downcity/server`:
...
Then point `City` at that RPC URL:
...
Local RPC is a trusted transport for admin access: `Admin City` does not need `admin_secret_key`. `User City` still follows the same product-side model as HTTP: public actions can be called as guest, and authenticated actions require a normal `user_token`.

If `AccountsService` is installed, RPC requests to `/v1/accounts/providers` only return the local login method. Call `accounts/local/login` with `city_id` to receive a normal `user_token`, then recreate or update User City with `city_id + user_token`.
