...
> When the docs say "City runtime", they mean this `Federation` instance, not the client-side `City`.
...
- `User City`: runs in a product client; only holds `city_id + user_token` and calls `ai.text()`, `ai.stream()`, `ai.image()`, and Service actions.
