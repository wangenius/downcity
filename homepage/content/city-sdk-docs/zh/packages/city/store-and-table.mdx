---
title: Store 与 Table
description: "Federation 的数据层、表能力和为什么 city、env、usage 最终都会落到 store 层。"
---
...
很多 Federation 能力都不是纯内存状态，它们最终都必须落到数据事实：
...
- 你要把 Federation 数据和自己的业务表联动
- 你想从代码里读写某些受控表
- 你在做运维排查，不想只看 HTTP 返回

## `base.table()` 代表什么

`base.table(name)` 不是“随便暴露数据库”，而是 Federation 对底层数据库能力的受控暴露。

它给你的不是随意 SQL 字符串，而是围绕 Federation 生命周期统一管理的表接口。

## 最小示例：读写 Federation 表
...
如果你想看更底层方法签名，去读 [Runtime API](/zh/city-sdk-docs/reference/runtime-api) 和 [City API](/zh/city-sdk-docs/reference/federation)。

## 常见误解

### Federation 不只是 API 代理

它后面还有长期存在的数据层，很多管理能力都建立在这些表上。

### `table()` 不是让你跳过 Federation 设计
...
- 想知道数据库是怎么接进来的，读 [Federation](/zh/city-sdk-docs/packages/city/federation)
- 想在终端里查这些表，读 [CLI](/zh/city-sdk-docs/packages/cli)
