...
- `shell` plugin runtime 最终还是宿主机直接 `spawn(...)`
...
```text
shell plugin runtime 不再直接 spawn，而是调用 SandboxRunner。
```
...
```mermaid
flowchart LR
  U["CLI / Tool 调用"] --> S["shell plugin runtime"]
  S --> R["SandboxRunner"]
  R --> X["Sandbox Session"]
  X --> P["sandbox process"]
...
```text
shell plugin runtime -> 宿主直接 spawn
```

改成：

```text
shell plugin runtime -> SandboxRunner
```

## 与 shell plugin runtime 的接入关系

当前 `shell` plugin runtime 保持不变的部分：
