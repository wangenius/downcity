...
`list` 用来确认本地已经有哪些 skill。只有当目标 skill 不在 `list` 里时，才需要 `find`。`install` 之后还要执行 `lookup`，agent 才能在当前上下文里按这个 skill 的说明使用它。
...
默认情况下，`SkillPlugin` 只扫描当前项目的 `.agents/skills`。

如果你希望显式使用更多来源，可以在构造函数里传入配置：
...
- `use` 控制内置扫描来源，支持 `project` 与 `home`
- `paths` 添加额外扫描目录，相对路径基于当前项目解析
- `ignore` 忽略指定 skill，支持名称、正则或函数
