...
macOS 使用 Seatbelt，Linux 使用 Bubblewrap。原生 Windows 默认通过锁定版本的 Microsoft MXC runtime 使用 `windows-mxc-dev`；设置 `DC_WINDOWS_SANDBOX=srt` 后可显式启用基于独立 Windows 用户、ACL 与 WFP 的 `windows-srt-alpha`。SRT 需要先运行 `npx @downcity/sandbox-windows-srt setup`，且当前只允许一个活动 workspace 安全域。两个 Windows 后端都不会在不可用时降级成 unrestricted。
