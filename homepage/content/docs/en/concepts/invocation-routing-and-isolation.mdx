...
- Cross-environment routing usually happens only when `host/port` or `DC_SERVER_*` is intentionally or accidentally overridden.

## Isolation Boundary (Core Principle)

The runtime model is effectively **one process bound to one projectRoot**:

- One `city agent start` process initializes one runtime state.
...
For `city service ...` and `city extension ...`, the flow is:
...
2. env vars: `DC_SERVER_HOST/DC_SERVER_PORT` (then `DC_CTX_SERVER_*`)
...
- stale `DC_SERVER_*` can redirect calls away from the current project.
...
- subprocess env includes `DC_CTX_SERVER_HOST/DC_CTX_SERVER_PORT` to target current runtime server
...
1. leftover exported `DC_SERVER_*` in terminal
...
2. Check `DC_SERVER_*` / `DC_CTX_SERVER_*`
3. Confirm `--path` or current directory points to target project
4. Run `city console agents` and verify endpoint/port against your target runtime
