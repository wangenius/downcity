/**
 * AcpSessionExecutor：基于 ACP 子进程的 session executor。
 *
 * 关键点（中文）
 ...
 * ACP executor 默认实现。
 */
export class AcpSessionExecutor implements SessionExecutor {
  ...
  constructor(options: {
    ...
    if (!this.rootPath) throw new Error("AcpSessionExecutor requires rootPath");
    if (!this.sessionId) throw new Error("AcpSessionExecutor requires sessionId");
  }

  async run(input: SessionRunInput): Promise<SessionRunResult> {
    if (this.running) {
      throw new Error("AcpSessionExecutor.run does not support concurrent execution");
    }
    const currentCtx = getSessionRunScope();
    const nextContext = {
      ...(currentCtx || {}),
  ...
  async requestCancelCurrentTurn(): Promise<boolean> {
    ...
    if (progress && progress.activeToolCallIds.size > 0) {
      // 关键点（中文）：工具已经发出但结果未回齐时，不应立刻 cancel。
      // 否则会把 tool-call 截断在半路，导致上下文里只剩 call 没有 result。
      this.cancelRequestedAfterToolResults = true;
      return true;
    }

    this.sendCancelCurrentTurn(remoteSessionId);
    return true;
  }
  ...
  private async handleNotification(envelope: AcpJsonRpcEnvelope): Promise<void> {
    ...
    if (sessionUpdate === "tool_call" || sessionUpdate === "toolCall") {
      ...
      if (toolCallId && progress) {
        progress.activeToolCallIds.add(toolCallId);
    ...
    if (
      ...
      const progress = this.activeAssistantProgress;
      if (toolCallId && progress) {
        progress.activeToolCallIds.delete(toolCallId);
      ...
      if (
        progress &&
        this.cancelRequestedAfterToolResults &&
        progress.activeToolCallIds.size === 0
      ) {
        const remoteSessionId = String(this.remoteSessionId || "").trim();
        if (remoteSessionId) {
          this.sendCancelCurrentTurn(remoteSessionId);
   ...
   *
   * 关键点（中文）
   * - 只有在“当前没有未完成 tool call”时才应调用。
   * - 同时把挂起 permission request 统一回成 cancelled。
   */
  private sendCancelCurrentTurn(remoteSessionId: string): void {
   ...
   *
   * 关键点（中文）
   * - 取消/协议错误后，最稳妥的做法是直接丢弃子进程与 remote session。
   * - 下一轮 run 会重新 `spawn -> initialize -> session/new -> bootstrap`。
   */
  private async resetRemoteRuntime(): Promise<void> {
    try {
      await this.dispose();
    } catch {
