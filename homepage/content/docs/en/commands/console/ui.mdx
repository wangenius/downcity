...
- Global Agents now supports creating a new agent directly from the UI: choose a project directory with the system folder picker, then provide name and primary model to create and start it in one flow.
- For directories that are not initialized yet, Global Agents also supports initialize-on-start: it creates `PROFILE.md`, `SOUL.md`, `downcity.json`, and `.downcity/` before starting the runtime.
