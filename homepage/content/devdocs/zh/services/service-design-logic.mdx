...
```text
service 的工作不是自己拥有所有能力，而是把外部输入组织成稳定主流程，并在正确节点调用 AgentContext、session 能力和 plugin 能力。
```
...
2. 做权限或前置校验
3. 归一化成内部语义对象
4. 决定是否进入 `session.prompt`
5. 必要时写业务侧事实流
6. 调用 plugin 点
7. 产出结果并送到出口

## 什么情况下应该进入 `session.prompt`
...
flowchart TD
  ...
  S --> SES["session.prompt / append message"]
