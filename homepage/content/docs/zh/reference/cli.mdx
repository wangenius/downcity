...
| `shipmyagent config` | 读取/更新 `ship.json` 与 `llm` provider/model 配置 |
| `shipmyagent stop` | 停止后台 agent 服务器 |
| `shipmyagent restart` | 重启后台 agent 服务器 |
| `shipmyagent services` | 管理 service runtime |
...
说明：
- 默认输出 JSON（便于脚本消费），可通过 `--json false` 切换成人类可读输出
- `config set` 会优先按 JSON 字面量解析（如 `true`、`123`、`{...}`、`[...]`），解析失败再按普通字符串写入
- 当 provider 被任意 model 引用时，不允许删除该 provider
- 当前 activeModel 不允许删除，需先切换到其他 model

### `sma chat`

```bash
sma chat send --text "已完成" [--chat-key <chatKey>] [--delay <ms>] [--time <time>] [--reply]
...
- `--delay <ms>` 为可选非负整数，按毫秒延迟后再发送消息
- `--time <time>` 为可选定时发送时间，支持 Unix 时间戳（秒/毫秒）或 ISO 时间字符串
- `--delay` 与 `--time` 互斥，不能同时使用
- `--reply` 让 `sma chat send` 尝试按目标消息发送为 reply（默认不启用 reply）
- `sma chat react` 用于给消息贴表情（当前主要用于 Telegram），`--emoji` 默认 `👍`
- `sma chat react` 的 `--message-id` 可选；不传时会尝试使用该 `chatKey` 最近一次入站消息的 `messageId`
- 典型场景：Agent 通过 `exec_command` 执行 `sma chat send` 回发消息
- `sma chat history` 默认返回最近 `30` 条事件，并支持按方向/时间窗口过滤
...
说明：
- `sma skill` 依赖本地 runtime（`sma start` 或 `sma run`）。
- `sma skill lookup` 为无状态读取，直接返回 `SKILL.md` 内容。
- 使用 skill 前建议先执行一次 `sma skill lookup`，以当前返回内容为准。
...
说明：
- `sma services` 仅在本地 runtime 已启动时可用（`sma start` 或 `sma run`）
- `sma service` 是 `sma services` 的别名（例如：`sma service restart chat`）
- 内建 command bridge 支持 `status|start|stop|restart`
...
默认模板支持 `{{...}}` 变量；初始化时会渲染内置变量 `{{agent_name}}`（取交互输入的 Agent 名称，若为空则回退为当前目录名）。

**选项**：

| 选项 | 描述 |
|------|------|
| `--force` | 覆盖现有 `ship.json` |
...
- Model name（输入真实值）
- Base URL（可选，回车跳过）
- Chat channels（Telegram / Feishu / QQ，多选）
- 推荐 skills（可选）

初始化会把 `API Key` / `Model` / `Base URL` 写入 `.env`，并在 `ship.json` 中引用 `${LLM_API_KEY}` / `${LLM_MODEL}` / `${LLM_BASE_URL}`：

- 若键不存在：自动追加
- 若键已存在且你输入了新值：会当场询问是否覆盖
...
运行提示：执行 `sma .`、`sma start`、`sma stop`、`sma restart`（或对应 `shipmyagent` 命令）时，会先打印当前 CLI 版本，例如 `sma version: 1.0.211`。
...
| `--webui [enabled]` |  | 启动交互式 Web 界面（默认：false）|
| `--webport <port>` |  | 交互式 Web 界面端口（默认：3001）|
