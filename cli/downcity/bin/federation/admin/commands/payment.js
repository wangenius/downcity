...
export async function managePayment(a, baseUrl, runtime) {
    ...
    while (true) {
        ...
        try {
            ...
            if (act === "payments") {
                const result = await runtime.with_loading(t({ zh: "支付记录", en: "Payments" }), async () => await svc.get("payments"));
            ...
            const result = await runtime.with_loading(t({ zh: "Webhook 事件", en: "Webhook Events" }), async () => await svc.get("events"));
            await runtime.show_table({
