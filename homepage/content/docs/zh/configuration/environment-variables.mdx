...
3. 当宿主是 `town` 时，会先从 `downcity.db` 读取 City 侧 env，再显式传给 `Agent`。
...
1. City AIService 模型目录（`model_providers`、`models`）只来自 `downcity.db`。
...
1. 准备本地 Town 状态，并验证 City 连接。
2. 在 City UI `Global / Env` 维护共享 env，或由宿主在 `new Agent({ env })` 时显式传入。
3. 通过 Town chat 产品表层创建 chat account。
4. 在 agent `downcity.json` 里绑定 `channelAccountId`。
5. 仅在需要时使用 `<agent>/.env` 做项目级运行时覆盖。

具体工作流见 [Town CLI](/zh/products-docs/town-cli)、[Town City 文档](/zh/products-docs/town-cli/city)、[Town config 文档](/zh/products-docs/town-cli/config) 和 [Town chat 文档](/zh/products-docs/town-cli/plugin/chat)。
...
5. 渠道配置变更后，通过 Town chat 产品表层验收路由。
...
当前 `town` CLI 不再启动或重新绑定 Console UI。Console 公网访问应该由 Console 产品部署、反向代理、隧道或托管层提供。
