...
The default runtime created by `createClient()` uses `.chat()`. If a model must
use Responses, Claude Messages, or another native protocol, override
`create_language_model(ctx)` and return the correct `LanguageModelV3`.
`text()` and the CityModel stream pathway both reuse this factory.
...
`/v1/ai/chat/completions` is already OpenAI-compatible, so it does not use `providerOptions`; the validated `reasoning_effort` remains in the request body for the custom `openai` action or automatic passthrough.
 ...
 If the default OpenAI-compatible non-streaming implementation is not enough,
override `text`. Model streaming no longer uses a Provider `stream` action;
override `create_language_model(ctx)` to select another upstream protocol.
...
```
 ...
 | `create_language_model` | Select the LanguageModelV3 used by `/v1/ai/stream` |
