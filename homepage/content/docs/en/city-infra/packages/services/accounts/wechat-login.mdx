...
in the InfraRuntime environment.
...
with the current InfraRuntime `redirect_uri`.

If the callback domain configured in WeChat does not match the real public InfraRuntime domain used by the product, login usually fails immediately.

So two things must stay aligned:

- the real public InfraRuntime domain used by the product
...
After WeChat login succeeds, the accounts service still returns a normal InfraRuntime `user_token`.

So the product does not need a special post-login client flow just because the user came from WeChat:
...
- configuring one domain in WeChat while the product actually uses another InfraRuntime domain
...
- After deployment, write `WECHAT_CLIENT_ID` and `WECHAT_CLIENT_SECRET` into InfraRuntime env through City Terminal or the Admin API
