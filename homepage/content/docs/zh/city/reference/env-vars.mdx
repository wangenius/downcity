...
| 变量 | 用途 |
| --- | --- |
| `DOWNCITY_CITY_ADMIN_SECRET_KEY` | `AdminClient` 管理 City |
...
`DOWNCITY_CITY_ADMIN_SECRET_KEY`、`DOWNCITY_CITY_TOKEN_SIGNING_KEY` 和 `BETTER_AUTH_SECRET` 会在 City 首次启动时自动写入 City 数据库的 `env` 表。provider API key 也统一通过 Admin API / Studio CLI 写入同一张表。
