 ...
 * 从网卡列表中提取首个公网 IPv4。
 */
export function detectPublicIpv4FromInterfaces(interfaces = os.networkInterfaces()) {
    for (const entries of Object.values(interfaces)) {
        for (const rawItem of (entries || [])) {
            const item = rawItem;
            const family = typeof item.family === "string"
                ? item.family
                : item.family === 4
                    ? "IPv4"
                    : "unknown";
 ...
 */
export function resolveControlPlanePublicUrl(params) {
    const env = params.env || process.env;
    const explicitUrl = String(env.DOWNCITY_PUBLIC_URL || "").trim();
    if (explicitUrl)
        return normalizeUrl(explicitUrl, params.port);
    const explicitHost = String(env.DOWNCITY_PUBLIC_HOST || "").trim();
    if (explicitHost)
        return formatUrl(explicitHost, params.port);
    const bindHost = String(params.bindHost || "").trim();
    if (isDirectReachableHost(bindHost)) {
        return formatUrl(bindHost, params.port);
    }
    if (params.publicMode !== true)
        return null;
    const detectedPublicIp = String(params.detectedPublicIp || "").trim() || detectPublicIpv4FromInterfaces();
    if (!detectedPublicIp)
        return null;
...
function isDirectReachableHost(host) {
...
//# sourceMappingURL=PublicAccess.js.map