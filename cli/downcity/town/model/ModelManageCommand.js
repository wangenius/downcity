...
import { discoverProviderModels, } from "./ModelSupport.js";
import { assertProviderType, parseBooleanOption, parseNumberOption, parsePositiveIntegerOption, resolveModelPresetOrThrow, runStoreCommand, } from "./ModelCommandShared.js";
/**
 * 注册 `add/pause/remove/update/test` 命令。
 */
export function registerModelManageCommands(model) {
...
function registerAddCommands(model) {
    const add = model
        .command("add")
        ...
        await runStoreCommand(options, async (store) => {
            ...
            if (preset) {
                ...
                if (!preset.providerTypes.includes(provider.type)) {
                    throw new Error(`Preset "${preset.id}" expects provider type in "${preset.providerTypes.join(", ")}", but provider "${providerId}" is "${provider.type}".`);
                }
            }
            if (!modelName) {
...
function registerPauseCommand(model) {
    model
...
function registerRemoveCommands(model) {
    const remove = model
        .command("remove")
...
function registerUpdateCommands(model) {
    const update = model
        .command("update")
    ...
    update
        ...
        await runStoreCommand(options, async (store) => {
            ...
            const hasAnyChange = options.type !== undefined ||
    ...
    update
        ...
        await runStoreCommand(options, async (store) => {
            ...
            if (!current)
                throw new Error(`Model not found: ${id}`);
            const hasAnyChange = options.provider !== undefined ||
            ...
            if (options.frequencyPenalty !== undefined && options.clearFrequencyPenalty) {
                throw new Error("--frequency-penalty and --clear-frequency-penalty cannot be used together");
            }
            if (options.presencePenalty !== undefined && options.clearPresencePenalty) {
                throw new Error("--presence-penalty and --clear-presence-penalty cannot be used together");
            }
            if (options.anthropicVersion !== undefined && options.clearAnthropicVersion) {
                throw new Error("--anthropic-version and --clear-anthropic-version cannot be used together");
            }
            const preset = resolveModelPresetOrThrow(options.preset);
...
function registerTestCommands(model) {
    const test = model
        .command("test")
...
//# sourceMappingURL=ModelManageCommand.js.map