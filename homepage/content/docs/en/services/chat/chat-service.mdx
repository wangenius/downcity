...
- `chat` authorization rules are shared globally at the city level and stored in `~/.downcity/downcity.db`.
- When multiple agents bind the same channel account, the same platform user uses the same role binding.
