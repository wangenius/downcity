...
```txt
Interfaces
  -> studios 通过 client / studio terminal 接入 City
Composition
...
- 产品侧入口：`@downcity/gate` 的 `UserClient` 与 `AdminClient`
- 运维入口：`@downcity/studio-cli`

它们都不实现 City 本身，只负责把请求送进同一个 City。

## 2. Composition：谁来把 City 组装起来

Downcity 的运行时不是“new 一个 City 就结束”，中间还有一层装配：

- `cities/node` 负责 Node.js + SQLite 的本地 server 示例
- `cities/edge` 负责 Cloudflare Workers + D1 的部署示例
- `cities/shared` 负责复用 server / worker 之间共同的 City 组装逻辑
...
3. 解析 `studio_id`、`user_id` 和身份级别
4. 找到目标 Service / Action
5. 组装统一 `ctx`
6. 执行 hook 和 action

如果 token 无效、过期或 `studio_id` 不匹配，City 会直接返回 401 或 403。
