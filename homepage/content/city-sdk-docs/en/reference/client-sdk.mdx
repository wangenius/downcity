...
- `rpc://127.0.0.1:15315`: trusted local RPC entry, no `user_token` or `admin_secret_key` required.

RPC is for trusted processes on the same machine. HTTP authentication is unchanged and still requires bearer tokens.
