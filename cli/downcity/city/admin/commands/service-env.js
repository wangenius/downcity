...
export async function manageEnv(a) {
    while (true) {
        ...
        if (!svcId || isCancel(svcId) || svcId === "back")
            return;
        if (svcId === "__init__" && services) {
            await initAllEnv(a, services);
            continue;
        }
        if (svcId === "__list__") {
            await listAllEnv(a);
        ...
        if (services) {
            ...
            if (svc)
                await configureServiceEnv(a, svc);
        }
    }
}
// ============================================================
// Init 模式
// ============================================================
async function initAllEnv(a, services) {
...
// 直接管理模式
// ============================================================
async function listAllEnv(a) {
    try {
        const items = await a.env.list();
        if (items.length === 0) {
            show("No env variables configured.");
            return;
        }
        console.log(`\n${items.length} env:\n`);
...
async function updateEnv(a) {
    const items = await fetchCurrentEnv(a);
    if (items.length === 0) {
        show("No env variables to update.");
        return;
    }
    ...
    if (!key || isCancel(key) || key === "__cancel__")
        return;
    ...
    if (!value)
        return;
    try {
        await a.env.upsert({ key: key, value });
...
// 按 Service 查看
// ============================================================
async function configureServiceEnv(a, svc) {
    while (true) {
...
// 工具
// ============================================================
async function fetchEnvScopes(a) {
    try {
