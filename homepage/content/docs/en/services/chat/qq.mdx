...
When the `voice` plugin is enabled and the `voice.transcriber` asset is available, QQ inbound `voice/audio` attachments are transcribed in best-effort mode and the transcript is appended to the user instruction.
