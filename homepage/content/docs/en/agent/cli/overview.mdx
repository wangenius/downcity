...
description: Understand the studio agent command set by workflow rather than by flat command list
---

# Agent CLI Overview

`studio agent` is the command surface for the single-project Agent runtime.

If you think of the Agent as "the execution unit for one project," the `studio agent` command group is its operator console.
...
- `chat`
- `history clean`
- interactive `studio agent` manager
