...
### Option 2: start a standalone RPC or HTTP server
...
Remote sessions do not accept a local model instance:

```ts
await session.set({ model });
```

They can request a server-side model by stable ID:

```ts
await session.set({ modelId: "quality" });
```

The server owns the model instance and resolves this ID through `resolve_model`.
