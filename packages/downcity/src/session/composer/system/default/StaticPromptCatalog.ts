 ...
 * 加载当前静态系统提示列表。
 */
export function loadStaticSystemPrompts(rootPath: string): string[] {
...
export class StaticPromptCatalog {
  ...
  private readonly getCurrentSystems: StaticPromptCatalogOptions["getCurrentSystems"];
  private readonly applySystems: StaticPromptCatalogOptions["applySystems"];
  ...
  private reloadStaticSystemPrompts(reason: string, filename?: string): void {
  ...
  start(): void {
    ...
    attachWatcher(this.rootPath, {}, (_eventType, filename) => {
      ...
      schedule("static-prompts", () =>
        this.reloadStaticSystemPrompts(changedReason, basename),
      );
    });
