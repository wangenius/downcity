---
title: Admin City
description: How trusted environments manage cities, issue user tokens, and maintain runtime env.
---
...
<div className="dc-doc-flow" aria-label="Admin City call flow">
  ...
  <div className="dc-doc-lane">
    <div className="dc-doc-node">
      ...
      <span>Calls `/v1/cities/*`, `/v1/cities/tokens/apply`, `/v1/env/*`, `/v1/ai/models`, and `/v1/base/instruction`.</span>
    </div>
  </div>
  <div className="dc-doc-arrow" aria-hidden="true">→</div>
  <div className="dc-doc-lane">
...
- stop all user calls for one city
- temporarily take a city offline
- reopen calls for a city later
...
The current behavior is direct city deletion. Confirm that this matches your business expectation before using it.

## `tokens.apply()`

This is the most common trusted-side action: issue a `user_token` for one user under one city.
...
Besides `cities.*` and `env.*`, the admin side can also read the current capability catalog exposed by City.
...
Besides `cities.*` and `env.*`, `Admin City` can call admin-side services exposed by official packages.
...
In other words, `Admin City` is not only for built-in cities/env endpoints. It is also the trusted-side entry into the same unified service route space.
