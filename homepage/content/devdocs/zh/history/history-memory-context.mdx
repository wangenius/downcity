...
最明确的原始历史层，是 chat managed plugin runtime 的事件流。
...
当前 memory runtime 已经是一个独立长期状态层。
...
因为托管 plugin runtime 是能力层，不是执行主体。
...
- 可以承接 request scope
- 可以被多个托管 runtime 协作访问

所以在当前架构里，最稳的主句应该是：

```text
系统围绕 Session 运转，托管 runtime 围绕 Session 工作，Context 由 Session 在每次运行中临时投影出来。
```
