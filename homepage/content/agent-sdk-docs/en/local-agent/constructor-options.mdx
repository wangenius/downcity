...
description: Detailed explanation of the local Agent id, path, instruction, prepare_session, env, tools, and plugins options
---
...
A host hook called before every Session turn.
...
- the SDK does not select, persist, or restore model IDs
- the host creates the `LanguageModel` and injects it through this hook
- the hook runs before every turn, so host configuration can change for the next turn
- the hook may prepare other host-owned runtime dependencies too
