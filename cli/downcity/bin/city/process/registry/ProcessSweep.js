 ...
 */
export function buildDetachedProcessSignalTargets(pid) {
 ...
 */
export function signalDetachedProcess(pid, signal) {
...
function parseDowncityCliArgs(command) {
    const normalized = normalizeCommand(command);
    const match = normalized.match(/(?:^|\s)(\S*[\\/]Index\.js)(?:\s+(.+)|$)/);
    if (!match)
        return null;
    const entryPath = String(match[1] || "");
    const isKnownEntry = /[\\/]bin[\\/]cli[\\/]Index\.js$/.test(entryPath) ||
...
export function isDowncityCliCommand(command) {
...
export function shouldSweepDetachedBayCommand(command, params) {
    ...
    if (args === null)
        return false;
    if (params.include_city_runtime &&
        /^run(?:\s|$)/.test(args)) {
        return true;
    }
...
export async function findDetachedBayProcesses(params) {
    ...
    return listDetachedBayProcesses({
        include_city_runtime: params?.include_city_runtime,
...
//# sourceMappingURL=ProcessSweep.js.map