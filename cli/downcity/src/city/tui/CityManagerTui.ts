...
import type { CityConnectionState, CityServerProfile } from "../types/CityConnection.js";
...
interface city_manager_state {
  ...
  /** 当前 City 连接状态。 */
  connection: CityConnectionState;
...
function read_city_connection_state(): CityConnectionState {
...
function save_city_user_session(session: CityUserSession): void {
...
async function prompt_city_server(): Promise<CityServerProfile | null> {
...
function select_city_server(server: CityServerProfile): void {
...
function format_connection_detail(connection: CityConnectionState): string {
  return t({
    zh: [
...
function format_city_list_detail(servers: CityServerProfile[]): string {
  return [
...
function format_login_detail(connection: CityConnectionState): string {
  return t({
    zh: [
      "{bold}登录{/bold}",
...
function format_balance_detail(account: CityBalanceAccount): string {
  return [
...
function format_recharge_result(result: CityRechargeResult): string {
