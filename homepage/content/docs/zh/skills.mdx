...
在运行中的 `shipmyagent agent on` / `shipmyagent agent on --daemon` 进程里，skills 目录和 `SKILL.md` 变更会自动热重载；下一次请求会使用最新内容。
...
当前 `shipmyagent agent on --daemon` 与聊天适配器默认都会把 skills 摘要 section 拼进 system prompt（`PROFILE.md` + 可选 `SOUL.md` / `USER.md` 拼接结果），让模型在对话一开始就知道：
