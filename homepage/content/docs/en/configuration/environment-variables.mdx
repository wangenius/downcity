...
3. When the host is `bay`, Console-global env is read from `downcity.db` and passed explicitly into `Agent`.
...
```mermaid
sequenceDiagram
  participant CLI as bay agent start
...
4. Plugin config (`bay plugin action ...`, `bay asr ...`, or `bay tts ...`): writes project `downcity.json`.
...
bay init
bay model create
bay plugin action asr configure --payload '{"modelId": "SenseVoiceSmall"}'
```
...
When you use `bay start -p` or `bay console start -p`, the CLI tries to print `Public URL` in startup output.
If you use `bay public on`, future plain `bay start` and `bay console start` runs also reuse the saved public bind mode.

These values are normally injected by `bay start` or the deployment environment. They are not endpoint parameters that users enter for `contact link`:
...
`bay start` automatically detects the public IP and stores it in global Console Env as `DOWNCITY_PUBLIC_HOST`. Agent `contact link` generation also uses this value to create transferable contact codes.
