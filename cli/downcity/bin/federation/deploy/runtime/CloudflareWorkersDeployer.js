...
import { readFederationProjectDeployEnv, } from "../../../federation/deploy/config/FederationProjectEnvLoader.js";
...
export async function deployCloudflareWorkers(config_file, options) {
    ...
    const wrangler_result = writeWranglerConfig(config_file, env_file, d1_result.resolved_database_id);
    emitCliBlock({
        tone: "success",
 ...
 * 执行 Wrangler deploy。
 */
async function runWranglerDeploy(config_file, params) {
 ...
 * 验证 Worker 健康状态。
 */
async function verifyWorker(base_url) {
    if (!base_url?.trim()) {
        emitCliBlock({
            tone: "warning",
 ...
 */
function registerDeployedServer(name, worker_url) {
...
//# sourceMappingURL=CloudflareWorkersDeployer.js.map