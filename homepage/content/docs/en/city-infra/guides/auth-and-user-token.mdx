...
```ts
const user = await admin.products.tokens.apply({
  product_id: "prod_xxx",
...
For new pure-client products, keep token issuance near one shared InfraRuntime instead of building a separate service per product.

## What the token should not do

- Do not put provider keys into the token.
- Do not put private billing rules into the token.
- Do not let the client hold `DOWNCITY_INFRA_ADMIN_SECRET_KEY`.
