...
`client.ai.image()` 内部使用任务轮询：先调用 `image_create()` 创建图片任务，再调用 `image_result()` 查询结果，直到成功后返回原来的 `UIMessage`。这可以避免图片模型耗时较长时让浏览器、代理或 Worker 自定义域名一直维持长连接。
...
如果你需要自己控制轮询，可以直接使用底层任务 API：

```ts
const job = await client.ai.image_create({ prompt: "一只站在雪地里的狐狸" });
