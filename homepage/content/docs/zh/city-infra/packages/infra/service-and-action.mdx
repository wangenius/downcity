...
description: "Service、Action、统一路由和 AIService 在 @downcity/infra 里的关系。"
---

`Service` 和 `Action` 是 `@downcity/infra` 里最重要的业务组织单位。
...
你最终给产品暴露的，不是一个“随便写的 handler”，而是 InfraRuntime 里统一管理的一组 action。
...
所以理解 Service / Action，本质上就是理解 Downcity Infra 如何组织业务能力。
...
```ts
import { InfraRuntime, Service } from "@downcity/infra";
...
这也是为什么产品端最后统一都能通过 `@downcity/conduit` 调：
...
AIService 不是另一套协议，而是 InfraRuntime 里一类更偏 AI 的 service。
...
- 想知道 hook 和 service 怎么围绕 Action 工作，读 [Hook 与服务挂载](/docs/city-infra/packages/infra/hooks-and-service)
- 想知道产品端怎么调用这些能力，读 [@downcity/conduit](/docs/city-infra/packages/conduit)
- 想看更底层 API 选项，读 [InfraRuntime API](/docs/city-infra/reference/infra-runtime)
