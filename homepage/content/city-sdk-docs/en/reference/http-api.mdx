...
instead of constructing transport requests manually. `client.ai.stream()` uses
the same transport and converts model parts into `UIMessageChunk` objects on the
client.
