...
import { getSupportedProviderTypes, toSafeProviderView, } from "./ModelCommandShared.js";
import { discoverProviderModels, resolveProjectRoot, setProjectPrimaryModel, } from "./ModelSupport.js";
...
function parseOptionalPositiveIntegerInput(input) {
    const value = String(input || "").trim();
    if (!value)
        return undefined;
...
async function promptProviderSelection() {
    ...
    if (providers.length === 0) {
        emitCliBlock({
            ...
            note: "运行 `town model create` 或直接在这里选择创建 provider。",
        });
        return null;
    }
    const response = (await prompts({
...
async function promptProviderAction(providerId) {
    const response = (await prompts({
        type: "select",
        name: "action",
...
async function promptModelAction(modelId) {
    const response = (await prompts({
        type: "select",
        name: "action",
...
async function printProviderDetails(providerId) {
...
async function editProvider(providerId) {
    ...
    const response = (await prompts([
        ...
        {
            type: (prev) => (prev === "replace" ? "text" : null),
            name: "apiKey",
            message: "新的 API Key",
...
async function removeProviderFromManager(providerId) {
    const service = new ModelPoolService();
    const usage = await service.getProviderUsage(providerId);
...
async function discoverProviderAndImport(providerId) {
    const store = new PlatformStore();
    try {
...
async function printModelDetails(modelId) {
    const service = new ModelPoolService();
...
async function editModel(modelId) {
    ...
    try {
        ...
        const currentProviderIndex = Math.max(providers.findIndex((provider) => provider.id === current.providerId), 0);
        const response = (await prompts([
            {
                type: "select",
                name: "providerId",
            ...
            },
            {
                ...
                validate: (value) => String(value || "").trim().length > 0 ? true : "模型名不能为空",
            },
            {
                type: "text",
                name: "temperature",
...
async function removeModelFromManager(modelId) {
    const service = new ModelPoolService();
    const usage = await service.getModelUsage(modelId);
...
async function toggleModelPauseState(modelId) {
    const service = new ModelPoolService();
    const usage = await service.getModelUsage(modelId);
...
async function testModelCall(modelId) {
...
async function runModelManager() {
