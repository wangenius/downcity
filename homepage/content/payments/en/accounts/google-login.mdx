...
- putting the downcity frontend URL into `Authorized redirect URIs` instead of the City callback URL
...
Start login through the unified `login/start` entry:
...
When `start.status === "redirect_required"`, send the user to `start.url`. After the OAuth callback completes, use `login/result` to poll or confirm the result:

```ts
const result = await guest.service("accounts").get("login/result", {
...
- For the overall OAuth flow, continue with [OAuth and Session](/en/payments/accounts/oauth-and-session)
- If you also need WeChat Website App login, continue with [WeChat Login Setup](/en/payments/accounts/wechat-login)
- For the full integration path, read [Accounts Service](/en/payments/accounts)
- After deployment, write `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` into Federation env through the Admin API or the `fed` admin workspace
