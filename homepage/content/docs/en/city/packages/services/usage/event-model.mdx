...
## How town-side users read their own usage
...
- self-serve usage awareness inside the town

## Common scenarios

### Scenario 1: track first, price later

First persist the facts, then later decide which models are billable, which calls count toward quotas, and which towns or flows are exempt.

### Scenario 2: debugging and operations

If you need to know whether calls really happened, whether one town is consuming abnormally, or whether one model is being hit far more than expected, the usage fact layer is often the first place to look.
