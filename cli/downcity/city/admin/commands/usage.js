...
export async function manageUsage(a, _baseUrl, runtime) {
    ...
    while (true) {
        ...
        try {
            if (act === "events") {
                const b = await runtime.with_loading(t({ zh: "用量事件", en: "Usage Events" }), async () => await svc.get("events"));
                ...
                const b = await runtime.with_loading(t({ zh: "用量汇总", en: "Usage Summary" }), async () => await svc.get("summary"));
                await runtime.show_table({
