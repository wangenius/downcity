...
One-shot scheduling is supported via optional `time`:

- `time` must be ISO8601 **with explicit timezone** (for example `2026-03-05T20:00:00+08:00`).
- When `time` is set, use `cron: "@manual"` to avoid mixed scheduling semantics.
...
- `sma task create ... --activate` is equivalent to `--status enabled`.
- `sma task update <taskId> --activate` sets `status` to `enabled`.

### Scheduler

The cron scheduler runs when your runtime is running (for example, when you launch `shipmyagent agent on` or `shipmyagent agent on --daemon` and keep the process alive). Only tasks with:

`status: enabled` and `cron != "@manual"`

are registered.

When task definitions change (`create/update/delete/status/enable/disable`), ShipMyAgent now auto-reloads the task scheduler so new status/time settings take effect immediately.
...
Use `skills_list` / `skills_lookup` to find and load it.
