...
- `base_url`
- `product_id`, automatically injected into AI service and user action calls
...
The most important point is not the method list. `UserClient` unifies three kinds of product-side calls:

- AI service: `client.ai.*`
- custom service: services you registered in InfraRuntime yourself
- service: services registered into InfraRuntime by services, such as `accounts`, `usage`, or `payment`

So from the product side, `UserClient` is not only "the thing that calls models." It is the unified user-context entry into InfraRuntime.
...
After receiving `session.user_token`, create a UserClient with `product_id + user_token` for AI service calls.
...
Pass `base_url + product_id + user_token`. This is for:
...
It is not the raw HTTP byte stream. The SDK parses the AI SDK UIMessage SSE body returned by InfraRuntime into chunk objects.
...
The InfraRuntime-side stream handler should return the result of AI SDK `createUIMessageStreamResponse()` or `streamText().toUIMessageStreamResponse()`.
...
The InfraRuntime-side Provider `image` / `video` actions should also return `UIMessage`.
...
`client.listServices()` returns the registered service summaries for the current InfraRuntime:
...
This is a service you registered into InfraRuntime yourself:
...
This is a service added into InfraRuntime by an official package:
...
If you want the product code to think in terms of "payment methods" instead of hand-writing `service + action`, use:
...
- both end up in the unified `/v1/*` route space inside InfraRuntime
...
- the request `product_id` does not match the product bound to the token
...
- pausing or re-activating products

Those are trusted-side actions and belong in [AdminClient](/en/docs/city-infra/reference/admin-client) or your own backend.
