...
function printTokenList(tokens: AuthTokenSummary[], json = false): void {
  ...
  if (tokens.length === 0) {
    emitCliBlock({
      ...
      note: "当前还没有本机 token，可执行 `studio token create <name>`。",
    });
    return;
  }
...
async function promptTokenCreateInput(params?: {
  ...
  if (!isInteractiveTerminal()) {
    emitCliBlock({
      ...
      note: "Use `studio token create <name>` or run this command in an interactive terminal.",
    });
    return null;
  }
...
async function promptTokenIdForDelete(): Promise<string | null> {
  if (!isInteractiveTerminal()) {
    emitCliBlock({
      ...
      note: "Use `studio token delete <tokenId>` or run this command in an interactive terminal.",
    });
    return null;
  }
...
async function runInteractiveTokenCommand(): Promise<void> {
  if (!isInteractiveTerminal()) {
    emitCliBlock({
      tone: "info",
      title: "Token command",
      note: "Use `studio token --help` for scriptable usage, or run `studio token` in an interactive terminal.",
    });
    return;
  }
