...
---
 ...
 Extend `Provider` and override `createClient` to get the default `text` action and CityModel stream runtime:
 ...
 POST /v1/ai/stream            → CityModel LanguageModelV3 path
...
The default `stream(ctx, call)` implementation uses `createClient().chat()`. If
a model must use Responses, Claude Messages, Gemini, or another native protocol,
override `stream()`, call that upstream, and return a standard
`LanguageModelV3StreamPart` stream.

## Model context window

Use `context_window` to declare the model's total context capacity in tokens:
...
ai.use(deepseek.model({
  ...
  price: [
    "Input: 1 credit / 1K tokens",
    "Output: 3 credits / 1K tokens",
  ],
}));
```

AIService returns this value through `/v1/ai/models`. When `@downcity/agent` runs a City model, its default session compaction starts at 80% of the configured window, leaving room for system messages, tools, and the current output. Models without `context_window` keep the Agent's existing 128k compaction budget, while an explicit `maxInputTokensApprox` still takes precedence.

`price` is also returned in the model catalog for product display. It is a
Provider-defined string list and does not participate in charging; actual
charges still come from `bill(ctx, output)`.

## Model reasoning effort

A reasoning-capable model declares its available levels and optional default through `reasoning`:
...
These levels are returned by `/v1/ai/models`. A caller sends one field whose value must match an entry in `reasoning.efforts[].id`:
...
AIService resolves model fallback first, then validates `reasoning_effort` against the final model. When the request omits it, AIService uses `default_effort`; when the model has no default, the upstream provider keeps its own default. Unsupported levels return `422`.

The default OpenAI-compatible `text` action and CityModel stream runtime convert the validated level into AI SDK `providerOptions.reasoningEffort`. Providers with different option shapes can override `build_reasoning_provider_options(ctx, model)`: Anthropic can map to `anthropic.thinking` / `anthropic.effort`, while Gemini can map to `google.thinkingConfig`. Custom actions should read the validated result with `read_resolved_reasoning(ctx)` instead of parsing the raw request again.

`/v1/ai/chat/completions` is already OpenAI-compatible, so it does not use `providerOptions`; the validated `reasoning_effort` remains in the request body for the custom `openai` action or automatic passthrough.

 ## Overriding text / stream

`text()` is the non-streaming SDK action. `stream(ctx, call)` is the standard
CityModel stream protocol. Providers can override them independently, and a
custom stream does not need to construct a complete CityModel:

 ```ts
 import {
   Provider,
   type CityProviderStreamCall,
   type CityProviderStreamResult,
   type Context,
 } from "@downcity/city";
 ...
 class CustomProvider extends Provider {
   ...
   protected async stream(
     ...
     const response = await requestCustomUpstream(ctx, call);
     return {
       stream: convertToLanguageModelStream(response.body),
     };
   }
 }
 ```
...
`@downcity/city` does not ship concrete image provider factories. City provides the `Provider` abstraction, `AIService` registration, and the unified `city.ai.image_create()` / `city.ai.image_result()` job contract.
...
Passing `balance` into AIService also enables the default balance precheck for model-consuming actions. AIService blocks users whose wallet is already negative before calling the Provider, then charges the actual usage after the Provider succeeds.
...
Callers only use `city.ai.image_create()` and `city.ai.image_result()`. Provider implementations normally use two methods:
...
Use `bill(ctx, output)` when a provider or a specific model needs to turn a finished AI call into a charge draft. `bill` does not deduct balance by itself. AIService reads the returned charge line and calls `balance.charge()` through the configured `BalanceService` bridge. The wallet can go negative after this post-success charge; the next model-consuming AI request is blocked by AIService's balance precheck.

Background `image/fetch` atomically claims an image job so only one worker calls the Provider at a time. After the Provider succeeds, AIService completes billing with a stable job idempotency key before writing `status: "succeeded"` and `result_json`. A billing failure leaves the job retryable, and a repeated attempt does not debit twice. `image_create()` never bills, and user `image_result()` only reads the `async_jobs` cache. When Queue consumption has no current user context, AIService automatically uses the owner stored when the job was created; a charge line can still override it with an explicit `user_id`.

For an asynchronous stream `charge`, Workers use `waitUntil` for settlement. Node and other runtimes without an ExecutionContext bind settlement to the Response stream finalization and close the stream only after billing completes. A non-stream asynchronous charge completes before the request ends.
...
```
 ...
 | Method | Description |
 |---|---|
 | `createClient` | Override for text or CityModel streaming; returns an OpenAI-compatible client |
 | `stream` | Execute a CityModel call and return a LanguageModelV3 stream result |
