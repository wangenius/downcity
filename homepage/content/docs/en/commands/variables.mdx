...
| `city service ...` | Target agent is running; project is registered; project has `downcity.json` + `PROFILE.md` | `--agent`, `--path`, `--host`, `--port` | Uses local IPC by default; `--agent` overrides `--path` |
| `city plugin ...` | Target agent is running; project is registered; project has `downcity.json` + `PROFILE.md` | `--agent`, `--path`, `--host`, `--port` | Uses local IPC by default; `--agent` overrides `--path` |
...
| `city <plugin> <action>` (e.g. `city asr ...`) | Target agent is running | `--path`, `--host`, `--port` | Forwarded through local IPC by default; explicit remote mode uses `/api/plugins/action` |

Additional notes:

- Local `city service`, `city plugin`, `city task`, `city chat`, and similar commands now use local IPC by default instead of HTTP.
- A call only enters remote HTTP mode when you explicitly pass `--host` or `--port`.
- In remote HTTP mode, token resolution order is: `--token` > `DC_AUTH_TOKEN` > `DC_AGENT_TOKEN` > local CLI auth state stored in console secure storage.
...
- non-agent remote invocations can still fall back to local CLI auth state
...
- Only explicit remote HTTP calls resolve auth through `--token` / `DC_AUTH_TOKEN` / `DC_AGENT_TOKEN` / local CLI auth state
