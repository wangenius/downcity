 ...
 * 根据路径推断静态资源 MIME。
 */
export function resolveConsoleUiContentType(filePath: string): string {
...
/**
 * 返回 Console UI 前端文件或 SPA fallback。
 */
export async function serveConsoleUiFrontendPath(params: {
  context: Context;
  publicDir: string;
  requestPath: string;
}): Promise<Response> {
  ...
  if (await fs.pathExists(candidatePath)) {
    ...
    if (stat.isFile()) {
      ...
      return params.context.body(content, 200, {
        "Content-Type": resolveConsoleUiContentType(candidatePath),
  ...
  if (!(await fs.pathExists(indexPath))) {
    return params.context.text(
      "Console UI frontend not found. Build console-ui first.",
      503,
    );
  }
