 ...
 */
export const buildRunArgsFromOptions = async (projectRoot, options) => {
    // 关键点（中文）：daemon 子进程必须强制前台模式，避免再次进入 startCommand 形成递归拉起。
    const args = ["agent", "start", projectRoot, "--foreground", "true"];
    ...
    const rpc_port = typeof options.rpcPort === "number" && Number.isInteger(options.rpcPort)
        ? options.rpcPort
