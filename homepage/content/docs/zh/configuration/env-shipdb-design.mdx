...
description: 详细说明 global_env、agent_env、channel_accounts 的表结构、加密边界与运行时加载/写入逻辑
---
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `key` | TEXT PK | env key，例如 `OPENAI_API_KEY` |
...
用途：agent 私有环境变量（DB 版）。

| 字段 | 类型 | 说明 |
|---|---|---|
| `agent_id` | TEXT | agent 唯一标识（projectRoot 绝对路径） |
| `key` | TEXT | env key |
| `value_encrypted` | TEXT | AES-GCM 密文 |
| `created_at` | TEXT | ISO 时间 |
| `updated_at` | TEXT | ISO 时间 |
...
1. `global_env.value_encrypted`
2. `agent_env.value_encrypted`
...
sequenceDiagram
  ...
  RT->>DB: 读取 global_env
  RT->>DB: 按 agent_id 读取 agent_env
  RT->>DOT: 解析项目 .env
  RT->>RT: 生成 runtime env = agent_env + .env（.env 优先）
...
2. `agent_env` 也是 agent 私有层；最终 runtime env = `agent_env + .env`（`.env` 覆盖）。
3. console 共享层 env 只来自 `global_env`。
