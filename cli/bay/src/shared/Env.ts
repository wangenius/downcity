 ...
 *
 * 关键点（中文）
 * - agent shell 会注入 `DC_AGENT_PATH/DC_AGENT_ID`，用于嵌套 Bay CLI 定位当前 agent。
...
/**
 * 限制 `bay env copy` 只能由本机 CLI 执行。
 */
function assertEnvCopyAllowedFromLocalCli(): void {
  if (!isAgentShellExecution()) return;
  throw new Error("bay env copy can only be run from the local CLI, not from an agent shell.");
}
