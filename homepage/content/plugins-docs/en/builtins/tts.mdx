...
Use the `town tts` shortcut for normal CLI work:
...
`configure` exists as an SDK/plugin action, but it is not exposed as a `town tts configure` shortcut. Use `town tts on`, `town tts use`, and per-call `synthesize` options for normal CLI changes.
...
| `configure` | update default config | SDK or advanced plugin action calls |
