 ...
 *
 * 关键点（中文）
 * - Bay runtime是强依赖：`bay start` + `bay agent start` 都会使用这里的默认配置。
...
export async function controlPlaneInitCommand(): Promise<void> {
    ...
    emitCliBlock({
      ...
      note:
        existingModelsCount > 0
          ? "检测到已有模型池。`bay init` 不会修改模型配置。"
          : "当前还没有模型池。你可以稍后执行 `bay model create` 配置模型。",
    });
  } finally {
    modelStore.close();
  }
  emitCliBlock({
    ...
    summary: "base-only",
    note: "基础设施已初始化。下一步可执行 `bay model create` 配置模型。",
  });
  emitCliList({
    tone: "accent",
    title: "Created",
