...
当 `start.status === "redirect_required"` 时，把用户带到返回的 `start.url`。OAuth callback 完成后，通过 `login/result` 轮询或确认结果：
...
- 想看 OAuth 整体链路，继续读 [OAuth 与 Session](/zh/payments/accounts/oauth-and-session)
- 如果你还要接微信网站登录，继续读 [微信登录配置](/zh/payments/accounts/wechat-login)
- 想看完整接入流程，读 [Accounts 服务](/zh/payments/accounts)
- 部署后记得通过 Admin API 或 `fed` 的 admin workspace 把 `GOOGLE_CLIENT_ID` 和 `GOOGLE_CLIENT_SECRET` 写进 Federation env
