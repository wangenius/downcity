...
function truncate_inline_text(input, max_length) {
...
function stringify_json_preview(value, max_length) {
    try {
...
function pick_first_text_field(value, field_names) {
...
function summarize_tool_args(args) {
    ...
    if (is_json_object(args)) {
        const prioritized_field = pick_first_text_field(args, ["cmd", "command", "query", "path", "url", "text"]) ||
...
function summarize_tool_result(result) {
    ...
    if (is_json_object(result)) {
        const prioritized_field = pick_first_text_field(result, ["stdout", "output", "text", "message", "summary"]) ||
...
//# sourceMappingURL=AgentChatToolFormatter.js.map