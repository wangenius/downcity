...
description: Call City from the studio with User Gate.
---

Studio clients use `User Gate`. The common paths are `client.ai.*` for AIService and `client.service(id).action(name).invoke()` for custom services.
