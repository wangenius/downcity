 ...
 * InfraRuntime 配置类型模块。
 *
 * 包含创建 InfraRuntime 实例和健康检查所需的所有类型定义。
 */
...
/**
 * 创建 InfraRuntime 实例时传入的顶层配置。
 *
 * 默认只需要传入 Drizzle db。InfraRuntime 会自动初始化内置 env/products 能力。
 */
export interface InfraRuntimeOptions {
  /**
   * Drizzle database 对象。
   *
