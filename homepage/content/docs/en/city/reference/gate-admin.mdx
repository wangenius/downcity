...
`Admin Gate` should only run in trusted environments.
...
The most accurate mental model is: `Admin Gate` is the trusted-side bridge into City management.
...
If `User Gate` owns "how a user-context bay call reaches City," then `Admin Gate` owns "how the trusted side prepares that call environment."
...
<div className="dc-doc-flow" aria-label="Admin Gate call flow">
...
4. the client calls City through `User Gate`

## Relationship with the accounts service

`Admin Gate` does not replace the accounts service, and the service does not replace `Admin Gate`.
...
- your backend uses `Admin Gate` to request `user_token`
- the frontend receives `bay_id + user_token`

### Pattern B: the accounts service owns login

- the frontend uses a guest `User Gate` to call `accounts.login/register`
- the accounts service returns `user_token` directly
- the frontend switches to a normal `User Gate`

So:

- `Admin Gate` owns trusted-side management actions
...
## Admin Gate can also call service-provided admin services

Besides `bays.*` and `env.*`, `Admin Gate` can call admin-side services exposed by official packages.
...
In other words, `Admin Gate` is not only for built-in bays/env endpoints. It is also the trusted-side entry into the same unified service route space.

## Error handling

When `Admin Gate` receives a non-2xx HTTP response, it throws an `Error` with two extra fields:
...
## What `Admin Gate` does not manage right now

These do not belong to `Admin Gate` yet:
...
In other words, `Admin Gate` owns env maintenance; runtime model definitions and mounting still belong to the `City` runtime layer.
