...
That is why auth, usage, and payment do not become separate systems. They become extension points inside the same City.
...
- the capability should be reused across many studios
- it is more than before or after logic and needs its own service or tables
- you want studio-side callers to reach it through a stable service boundary
...
## How studio-side callers reach services

Once mounted, services still look like normal services from the studio side:
...
Services add capability into City. The studio-side calling surface is still [@downcity/gate](/en/docs/city/packages/gate).

## Read next

- For unified studio-side service calls, read [@downcity/gate](/en/docs/city/packages/gate)
- For auth capability, read [@downcity/services](/en/docs/city/packages/services)
- For usage recording, read [@downcity/services](/en/docs/city/packages/services)
- For one-time Stripe topups, read [@downcity/services](/en/docs/city/packages/services)
