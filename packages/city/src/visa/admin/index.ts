...
import type {
  AdminVisaAccessOptions,
  AdminModelRecord,
  AdminServiceSummary,
} from "./types.js";

export class AdminVisaAccess {
  ...
  constructor(options: AdminVisaAccessOptions) {
    if (!options || typeof options !== "object") {
      throw new TypeError("Admin Visa options are required");
    }

    this.base_url = normalizeBaseURL(options.base_url, "base_url");
    this.secret = requiredString(
      options.admin_secret_key ?? process.env.DOWNCITY_CITY_ADMIN_SECRET_KEY,
      "admin_secret_key",
    );
  ...
  /** 获取 Service 调用器（与 User Visa 共用同一路由） */
