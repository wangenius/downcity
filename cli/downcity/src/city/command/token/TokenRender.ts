 ...
 * 打印 token 列表。
 */
export function printTokenList(tokens: AuthTokenSummary[], json = false): void {
  if (json === true) {
    printResult({
      asJson: true,
      success: true,
 ...
 * 渲染单个 token 详情。
 */
export function emitTokenDetail(token: AuthTokenSummary): void {
  emitCliBlock({
 ...
 * 渲染 token 创建成功后的接入说明。
 */
export function emitTokenSetupGuide(token: AuthIssuedToken): void {
  emitCliBlock({
    tone: "info",
    title: "Token ready",
    facts: [
