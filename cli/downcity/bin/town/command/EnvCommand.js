 ...
 */
function formatDotenvEntries(entries) {
    if (entries.length === 0)
        return "";
    return `${entries
 ...
 */
function isAgentShellExecution() {
    return Boolean(String(process.env.DC_AGENT_PATH || "").trim() ||
        String(process.env.DC_AGENT_ID || "").trim());
}
/**
 * 限制 `town env copy` 只能由本机 CLI 执行。
 */
function assertEnvCopyAllowedFromLocalCli() {
    if (!isAgentShellExecution())
        return;
    throw new Error("town env copy can only be run from the local CLI, not from an agent shell.");
}
async function listKeysEntries() {
...
//# sourceMappingURL=EnvCommand.js.map