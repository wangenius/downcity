...
- binding the project to its selected City AIService model
...
- it does not own the City AIService model catalog
- it does not persist City-level channel accounts
- it is not the Console UI or the global control plane

Those concerns are split between Town and City: Town manages local Agents and Agent plugins, while City manages models, channel accounts, and Service capabilities.
...
- chat with a project through Town
...
Town provides the user-facing local runtime path. This path is best when your goal is "turn this repository into an agent runtime and use it directly."
...
3. [Agent CLI product docs](/en/products-docs/town-cli/agent/agent)
