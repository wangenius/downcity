...
- `AgentRuntime`、`SessionFacade`、`Memory`、`Chat` 等宿主 runtime 状态不应该放进沙箱
...
也不应该把整个 `AgentRuntime` 都塞进容器或 VM。

原因很简单：

- `AgentRuntime` 是宿主 runtime 中心
- `SessionFacade` 是统一执行主轴
...
```text
宿主侧保留 AgentRuntime / Session / Chat / Memory / Logs
...
- `shell` plugin runtime 的协议和状态编排
- `task` plugin runtime 的 run 编排
- 审计、落盘、回 chat、日志
...
```text
`shell` plugin runtime 和 `task` plugin runtime 仍然在宿主侧，沙箱只是它们的执行后端。
```
...
- 容器不直接承担 chat 语义
- 中途状态查询仍然走宿主侧 `shell` plugin runtime
...
```mermaid
flowchart TD
  T["Task trigger"] --> TS["task plugin runtime (宿主)"]
  TS --> R["create run dir"]
