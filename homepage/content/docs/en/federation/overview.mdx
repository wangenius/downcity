...
Federation generates an Ed25519 Key Ring on first startup and publishes verification keys through `/.well-known/jwks.json`. Product backends can use `FedBureau` to identify requests belonging to their configured `city_id`.
